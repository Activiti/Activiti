...
public class DemoApplication implements CommandLineRunner {
    ...
    public void processText() {
        ...
        // these attributes could be handled as plain variables as the variables are in
        // activiti-api-basic-process-example
        // or the document could be mapped directly to a bean as in
        // activiti-api-basic-full-example-bean
        ...
        ProcessInstance processInstance =
                processRuntime.start(
                        ProcessPayloadBuilder.start()
                                ...
                                .withVariable(
                                        "content",
                                        objectMapper.convertValue(content, JsonNode.class))
                                .build());
        ...
        if (tasks.getTotalItems() > 0) {
            for (Task t : tasks.getContent()) {
                ...
                if (variableInstance.getName().equals("content")) {
                    LinkedHashMap contentToProcess =
                            objectMapper.convertValue(
                                    variableInstance.getValue(), LinkedHashMap.class);
                    logger.info(
                    ...
                    taskRuntime.complete(
                            TaskPayloadBuilder.complete()
                                    .withTaskId(t.getId())
                                    .withVariable("content", contentToProcess)
                                    .build());
                }
            }
    ...
    public Connector tagTextConnector() {
        return integrationContext -> {
            LinkedHashMap contentToTag =
                    (LinkedHashMap) integrationContext.getInBoundVariables().get("content");
    ...
    public Connector discardTextConnector() {
        return integrationContext -> {
            LinkedHashMap contentToDiscard =
                    (LinkedHashMap) integrationContext.getInBoundVariables().get("content");
