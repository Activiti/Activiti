...
public class EventSubscriptionManager {
  ...
  protected void removeObsoleteEventSubscriptionsImpl(
      ProcessDefinitionEntity processDefinition, String eventHandlerType) {
    ...
    List<EventSubscriptionEntity> subscriptionsToDelete =
        eventSubscriptionEntityManager.findEventSubscriptionsByTypeAndProcessDefinitionId(
            eventHandlerType, processDefinition.getId(), processDefinition.getTenantId());

    subscriptionsToDelete.forEach(
  ...
  protected void removeObsoleteMessageEventSubscriptions(
    ...
    if (previousProcessDefinition != null) {
      removeObsoleteEventSubscriptionsImpl(
          previousProcessDefinition, MessageEventHandler.EVENT_HANDLER_TYPE);
    }
  }
  ...
  protected void insertMessageEvent(
      MessageEventDefinition messageEventDefinition,
        ...
        .filter(
            eventSubscriptionEntity ->
                eventSubscriptionEntity.getProcessInstanceId() == null
                    || eventSubscriptionEntity.getProcessInstanceId().isEmpty())
        .forEach(
            _item -> { // processInstanceId != null or not empty -> it's a message related to an
                       // execution
    ...
    MessageEventSubscriptionEntity newSubscription =
        commandContext.getEventSubscriptionEntityManager().createMessageEventSubscription();
