 ...
 *
 * <p>This class will lazily instantiate the referenced classes when needed at runtime.
 */
public class ClassDelegate extends AbstractBpmnActivityBehavior
  ...
  public ClassDelegate(
      Class<?> clazz, List<FieldDeclaration> fieldDeclarations, Expression skipExpression) {
  ...
  public void notify(
    ...
    // Note that we can't wrap it in the delegate interceptor like usual here due to being executed
    // when the context is already removed.
    transactionDependentExecutionListenerInstance.notify(
      ...
      if (Context.getProcessEngineConfiguration().isEnableProcessDefinitionInfoCache()) {
        ...
        if (taskElementProperties != null
          ...
          String overrideClassName =
              taskElementProperties.get(DynamicBpmnConstants.SERVICE_TASK_CLASS_NAME).asText();
  ...
  public void trigger(DelegateExecution execution, String signalName, Object signalData) {
    ...
    if (activityBehaviorInstance instanceof TriggerableActivityBehavior) {
      ((TriggerableActivityBehavior) activityBehaviorInstance)
          .trigger(execution, signalName, signalData);
    } else {
  ...
  public void completing(DelegateExecution execution, DelegateExecution subProcessInstance)
    ...
    if (activityBehaviorInstance instanceof SubProcessActivityBehavior) {
      ((SubProcessActivityBehavior) activityBehaviorInstance)
          .completing(execution, subProcessInstance);
    } else {
      throw new ActivitiException(
  ...
  public static Object defaultInstantiateDelegate(
      Class<?> clazz, List<FieldDeclaration> fieldDeclarations) {
  ...
  public static void applyFieldDeclaration(
      List<FieldDeclaration> fieldDeclarations, Object target) {
  ...
  public static void applyFieldDeclaration(
      FieldDeclaration declaration, Object target, boolean throwExceptionOnMissingField) {
    Method setterMethod =
  ...
  /**
   * returns the class name this {@link ClassDelegate} is configured to. Comes in handy if you want
   * to check which delegates you already have e.g. in a list of listeners
   */
