...
public class BPMNLayout extends mxGraphLayout {
    ...
    /**
     * Does a depth first search starting at the specified cell. Makes sure the specified swimlane
     * is never left by the algorithm.
     */
    ...
    protected mxRectangle horizontalLayout(
            TreeNode node, double x0, double y0, mxRectangle bounds) {
    ...
    protected void attachParent(TreeNode node, double height) {
        ...
        node.contour.upperHead =
                createLine(node.height, 0, createLine(x, y1, node.contour.upperHead));
        node.contour.lowerHead =
                createLine(node.height, 0, createLine(x, y2, node.contour.lowerHead));
    }
            ...
            if (bounds == null) {
                ...
                bounds =
                        new mxRectangle(
                                ...
                                Math.max(
                                        bounds.getX() + bounds.getWidth(), g.getX() + g.getWidth()),
                                Math.max(
