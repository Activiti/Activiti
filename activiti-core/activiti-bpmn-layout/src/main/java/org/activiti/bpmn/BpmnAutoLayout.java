...
public class BpmnAutoLayout {
    ...
    protected void layout(FlowElementsContainer flowElementsContainer) {
        ...
        // Subprocesses are handled in a new instance of BpmnAutoLayout, hence they instantiations
        // of
        // new maps here.
        ...
        associations =
                new HashMap<
                        String,
                        Association>(); // Associations are gathered and processed afterwards,
        // because we must
        // be sure we already found source and target
        textAnnotations =
                new HashMap<
                        String, TextAnnotation>(); // Text Annotations are gathered and processed
        // afterwards, because we
        // must be sure we already found the parent.

        sequenceFlows =
                new HashMap<String, SequenceFlow>(); // Sequence flow are gathered and processed
        // afterwards,because we
        // mustbe sure we already found source and target
        boundaryEvents =
                new ArrayList<
                        BoundaryEvent>(); // Boundary events are gathered and processed afterwards,
        // because we
    ...
    protected void handleActivity(FlowElement flowElement) {
        Object activityVertex =
                graph.insertVertex(
                        cellParent, flowElement.getId(), "", 0, 0, taskWidth, taskHeight);
    ...
    protected void handleBoundaryEvents() {
        for (BoundaryEvent boundaryEvent : boundaryEvents) {
            ...
            mxCell boundaryPort =
                    new mxCell(null, geometry, "shape=ellipse;perimter=ellipsePerimeter");
    ...
    protected void handleAssociations() {
        ...
        for (Association association : associations.values()) {
            ...
            Object associationEdge =
                    graph.insertEdge(
                            cellParent, association.getId(), "", sourceVertex, targetVertex, style);
    ...
    protected void createDiagramInterchangeInformation(
            BaseElement element, List<mxPoint> waypoints) {
     ...
     * #handleSubProcess(FlowElement)}), the elements have x and y coordinates relative to the
     * bounds of the subprocess (thinking the subprocess is on (0,0). This however, does not work
     * for nested subprocesses, as they need to take in account the x and y coordinates for each of
     * the parent subproceses.
     *
        ...
        for (FlowElement flowElement : subProcess.getFlowElements()) {
            ...
            if (flowElement instanceof SequenceFlow) {
                List<GraphicInfo> graphicInfos =
                        bpmnModel.getFlowLocationMap().get(flowElement.getId());
