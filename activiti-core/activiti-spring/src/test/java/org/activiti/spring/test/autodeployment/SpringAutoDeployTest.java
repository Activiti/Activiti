...
public class SpringAutoDeployTest extends AbstractTestCase {
    ...
    public void testBasicActivitiSpringIntegration() {
        ...
        assertThat(processDefinitionKeys)
            .isEqualTo(expectedProcessDefinitionKeys);
    }
    ...
    public void testResourceRedeploymentAfterProcessDefinitionChange()
        throws Exception {
        createAppContext(CTX_PATH);
        ...
        String filePath =
            "org/activiti/spring/test/autodeployment/autodeploy.a.bpmn20.xml";
        String originalBpmnFileContent = IoUtil.readFileAsString(filePath);
        String updatedBpmnFileContent = originalBpmnFileContent.replace(
            "flow1",
            "fromStartToEndFlow"
        );
        assertThat(
            updatedBpmnFileContent.length() > originalBpmnFileContent.length()
        )
            .isTrue();
        ...
        try {
            applicationContext =
                new ClassPathXmlApplicationContext(CTX_NO_DROP_PATH);
            repositoryService =
                (RepositoryService) applicationContext.getBean(
                    "repositoryService"
                );
        } finally {
    ...
    private boolean waitUntilFileIsWritten(String filePath, int expectedBytes)
        throws Exception {
