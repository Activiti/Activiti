...
public class ActivitiInboundGateway extends MessagingGatewaySupport {
    ...
    public void execute(
        ...
        stringObjectMap.putAll(
            headerMapper.toHeaders(execution.getVariables())
        );
        MessageBuilder<?> mb = MessageBuilder
            .withPayload(execution)
            .copyHeaders(stringObjectMap);
        ...
        if (null != reply) {
            ...
            for (String k : vars.keySet()) {
                processEngine
                    .getRuntimeService()
                    .setVariable(execution.getId(), k, vars.get(k));
            }
