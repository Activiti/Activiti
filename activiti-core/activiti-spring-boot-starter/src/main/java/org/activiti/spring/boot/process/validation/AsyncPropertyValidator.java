...
public class AsyncPropertyValidator extends ProcessLevelValidator {
    ...
    protected void validateFlowElementsInContainer(
            FlowElementsContainer container, List<ValidationError> errors, Process process) {
        for (FlowElement flowElement : container.getFlowElements()) {
            ...
            if ((flowElement instanceof FlowNode) && ((FlowNode) flowElement).isAsynchronous()) {
                addWarning(
                        ...
                        process,
                        flowElement,
                        "Async property is not available when asyncExecutor is disabled.");
            }
                                ...
                                .forEach(
                                        event -> {
                                            if (event instanceof TimerEventDefinition) {
                                                addWarning(
                                                        errors,
                                                        Problems.EVENT_TIMER_ASYNC_NOT_AVAILABLE,
                                                        process,
                                                        flowElement,
                                                        "Timer event is not available when"
                                                                + " asyncExecutor is disabled.");
                                            } else if ((event instanceof SignalEventDefinition)
                                                    && ((SignalEventDefinition) event).isAsync()) {
                                                addWarning(
                                                        errors,
                                                        Problems.SIGNAL_ASYNC_NOT_AVAILABLE,
                                                        process,
                                                        flowElement,
                                                        "Async property is not available when"
                                                                + " asyncExecutor is disabled.");
                                            }
                                        });
            }
