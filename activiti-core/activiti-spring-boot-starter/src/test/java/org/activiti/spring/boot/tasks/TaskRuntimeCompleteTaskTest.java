    ...
    public void letsDebugg(){
        ...
        ProcessInstance processInstance = processRuntime.start(
            ProcessPayloadBuilder
                .start()
                .withProcessDefinitionKey("Process_KzwZAEl-")
                .build());
        ...
        Task completedTask = taskRuntime.complete(TaskPayloadBuilder.complete().withTaskId(task2.getId()).build());



        Page<Task> tasksAfterCompletion = taskRuntime.tasks(Pageable.of(0,
                                                         50));

        assertThat(tasksAfterCompletion.getContent()).hasSize(0);

        List<ProcessInstance> processInstanceList = processRuntime.processInstances(
            Pageable.of(0,
                        50),
            ProcessPayloadBuilder
                .processInstances()
