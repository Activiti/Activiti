...
public class TaskListenerTest extends PluggableActivitiTestCase {
    ...
    public void testTaskCompleteListener() {
        ...
        assertThat(
            runtimeService.getVariable(processInstance.getId(), "expressionValue")).isEqualTo(null);

        // Completing first task will change the description
        Task task = taskService.createTaskQuery().singleResult();
        taskService.complete(task.getId());

        assertThat(runtimeService.getVariable(processInstance.getId(), "greeting")).isEqualTo(
            "Hello from The Process");
        assertThat(runtimeService.getVariable(processInstance.getId(), "shortName")).isEqualTo(
            "Act");
    }
    ...
    public void testTaskListenerWithExpression() {
        ProcessInstance processInstance = runtimeService.startProcessInstanceByKey(
            "taskListenerProcess");
        assertThat(runtimeService.getVariable(processInstance.getId(), "greeting2")).isEqualTo(
            null);
    ...
    public void testAllEventsTaskListener() {
        ...
        String eventsReceived = (String) runtimeService.getVariable(task.getProcessInstanceId(),
            "events");
    ...
    public void testTaskListenersOnDeleteByComplete() {
        ...
        assertThat(TaskDeleteListener.getCurrentMessages().get(0)).isEqualTo(
            "Delete Task Listener executed.");
