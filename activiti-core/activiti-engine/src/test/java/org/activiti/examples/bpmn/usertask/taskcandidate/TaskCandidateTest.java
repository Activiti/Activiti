...
public class TaskCandidateTest extends PluggableActivitiTestCase {
    ...
    public void testMultipleCandidateGroups() {
        ...
        assertThat(
            taskService.createTaskQuery().taskCandidateUser(KERMIT, KERMITSGROUPS).list()).hasSize(
            1);
        assertThat(
            taskService.createTaskQuery().taskCandidateUser(GONZO, GONZOSGROUPS).list()).hasSize(1);
        assertThat(
            taskService.createTaskQuery().taskCandidateGroup("management").count()).isEqualTo(1);
        assertThat(
            taskService.createTaskQuery().taskCandidateGroup("accountancy").count()).isEqualTo(1);
        ...
        assertThat(taskService.createTaskQuery().taskCandidateUser(KERMIT, KERMITSGROUPS).list()
            .isEmpty()).isTrue();
        assertThat(taskService.createTaskQuery().taskCandidateUser(GONZO, GONZOSGROUPS).list()
            .isEmpty()).isTrue();
        assertThat(
            taskService.createTaskQuery().taskCandidateGroup("management").count()).isEqualTo(0);
    ...
    public void testMultipleCandidateUsers() {
        ...
        tasks = taskService.createTaskQuery().taskInvolvedUser(KERMIT).includeProcessVariables()
        ...
        tasks = taskService.createTaskQuery().taskInvolvedUser(GONZO).includeProcessVariables()
            .includeTaskLocalVariables().list();
        assertThat(tasks).hasSize(1);
        assertThat(task.getProcessVariables()).hasSize(1);
        assertThat(task.getTaskLocalVariables()).hasSize(1);
    ...
    public void testCandidateExpressionTwoParams() {
        ...
        runtimeService.startProcessInstanceByKey("candidateWithExpression", params);
        assertThat(taskService.createTaskQuery().taskCandidateUser(KERMIT, KERMITSGROUPS)
            .count()).isEqualTo(1);
