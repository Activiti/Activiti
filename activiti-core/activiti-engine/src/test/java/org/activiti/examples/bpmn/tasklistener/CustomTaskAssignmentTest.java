...
public class CustomTaskAssignmentTest extends PluggableActivitiTestCase {
    ...
    public void testCandidateGroupAssignment() {
        ...
        assertThat(taskService.createTaskQuery().taskCandidateUser("kermit", asList("management"))
            .count()).isEqualTo(1);
        assertThat(
            taskService.createTaskQuery().taskCandidateUser("fozzie", null).count()).isEqualTo(0);
    }
    ...
    public void testCandidateUserAssignment() {
        runtimeService.startProcessInstanceByKey("customTaskAssignment");
        assertThat(
            taskService.createTaskQuery().taskCandidateUser("kermit", null).count()).isEqualTo(1);
        assertThat(
            taskService.createTaskQuery().taskCandidateUser("fozzie", null).count()).isEqualTo(1);
        assertThat(
            taskService.createTaskQuery().taskCandidateUser("gonzo", null).count()).isEqualTo(0);
    }
