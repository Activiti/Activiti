...
public class ExecutionQueryEscapeClauseTest extends AbstractEscapeClauseTestCase {
  ...
  public void testQueryLikeByQueryVariableValue() {
    Execution execution =
        runtimeService.createExecutionQuery().variableValueLike("var1", "%\\%%").singleResult();
    assertThat(execution).isNotNull();
    assertThat(execution.getId()).isEqualTo(processInstance1.getId());

    execution =
        runtimeService.createExecutionQuery().variableValueLike("var1", "%\\_%").singleResult();
    assertThat(execution).isNotNull();
  ...
  public void testQueryLikeByQueryProcessVariableValue() {
    ...
    execution =
        runtimeService
            .createExecutionQuery()
            .onlyChildExecutions()
            .processVariableValueLike("var1", "%\\_%")
            .singleResult();
    assertThat(execution).isNotNull();
  }
  ...
  public void testQueryLikeIgnoreCaseByQueryProcessVariableValue() {
    Execution execution =
        runtimeService
            .createExecutionQuery()
            .onlyChildExecutions()
            .processVariableValueLikeIgnoreCase("var1", "%\\%%")
            .singleResult();
    assertThat(execution).isNotNull();
    ...
    execution =
        runtimeService
            .createExecutionQuery()
            .onlyChildExecutions()
            .processVariableValueLikeIgnoreCase("var1", "%\\_%")
            .singleResult();
    assertThat(execution).isNotNull();
  }
}
