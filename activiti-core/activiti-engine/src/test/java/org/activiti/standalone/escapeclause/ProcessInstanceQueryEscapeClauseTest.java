...
public class ProcessInstanceQueryEscapeClauseTest extends AbstractEscapeClauseTestCase {
    ...
    public void testQueryByProcessInstanceNameLike() {
        ...
        assertThat(
            runtimeService.createProcessInstanceQuery().processInstanceNameLike("%\\%%").singleResult()
        ).isNotNull();
        assertThat(
            runtimeService.createProcessInstanceQuery().processInstanceNameLike("%\\%%").singleResult().getId()
        ).isEqualTo(processInstance1.getId());

        assertThat(
            runtimeService.createProcessInstanceQuery().processInstanceNameLike("%\\_%").singleResult()
        ).isNotNull();
        assertThat(
            runtimeService.createProcessInstanceQuery().processInstanceNameLike("%\\_%").singleResult().getId()
