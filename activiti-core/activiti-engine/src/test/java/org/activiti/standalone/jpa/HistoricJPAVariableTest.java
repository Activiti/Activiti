...
public class HistoricJPAVariableTest extends AbstractActivitiTestCase {
    ...
    public void testGetJPAEntityAsHistoricVariable() {
        ...
        this.processInstanceId = runtimeService.startProcessInstanceByKey("JPAVariableProcess",
            variables).getId();
        ...
        HistoricVariableInstance historicVariableInstance = historyService.createHistoricVariableInstanceQuery()
            .processInstanceId(processInstanceId).variableName("simpleEntityFieldAccess")
            .singleResult();

        Object value = historicVariableInstance.getValue();
        assertThat(value).isInstanceOf(FieldAccessJPAEntity.class);
        assertThat(simpleEntityFieldAccess.getValue()).isEqualTo(
            ((FieldAccessJPAEntity) value).getValue());
    }
    ...
    public void testGetJPAEntityAsHistoricLog() {
        ...
        ProcessInstanceHistoryLog log = historyService.createProcessInstanceHistoryLogQuery(
                processInstanceId)
            .includeVariables()
            .singleResult();
    ...
    @Deployment
        (resources = {
            "org/activiti/standalone/jpa/HistoricJPAVariableTest.testGetJPAEntityAsHistoricLog.bpmn20.xml"})
    public void testGetJPAUpdateEntityAsHistoricLog() {
        ...
        for (Task task : taskService.createTaskQuery().includeProcessVariables().list()) {
            taskService.setVariable(task.getId(), "simpleEntityFieldAccess",
