...
public class CustomMybatisXMLMapperTest extends ResourceActivitiTestCase {
    ...
    public void testSelectOneTask() {
        ...
        CustomTask customTask = managementService.executeCommand(new Command<CustomTask>() {
            ...
            public CustomTask execute(CommandContext commandContext) {
                return (CustomTask) commandContext.getDbSqlSession()
    ...
    public void testSelectTaskList() {
        ...
        List<CustomTask> tasks = managementService.executeCommand(new Command<List<CustomTask>>() {
            ...
            public List<CustomTask> execute(CommandContext commandContext) {
                return (List<CustomTask>) commandContext.getDbSqlSession()
    ...
    public void testCustomQueryOrderBy() {
        ...
        List<CustomTask> tasks = new CustomTaskQuery(managementService).orderByTaskPriority().desc()
            .list();
    ...
    public void testAttachmentQuery() {
        ...
        String attachmentId = taskService.createAttachment("image/png", taskId, null, "attachment1",
            "", "http://activiti.org/").getId();
        taskService.createAttachment("image/jpeg", taskId, null, "attachment2",
            "Attachment Description", "http://activiti.org/");

        Authentication.setAuthenticatedUserId("gonzo");

        taskService.createAttachment("image/png", taskId, null, "zattachment3",
            "Attachment Description", "http://activiti.org/");

        Authentication.setAuthenticatedUserId("fozzie");

        for (int i = 0; i < 15; i++) {
            taskService.createAttachment(null, createTask(i + "", null, null, 0), null,
                "attachmentName" + i, "", "http://activiti.org/" + i);
        }

        assertThat(new AttachmentQuery(managementService).attachmentId(attachmentId).singleResult()
            .getId()).isEqualTo(attachmentId);

        assertThat(
            new AttachmentQuery(managementService).attachmentName("attachment1").singleResult()
                .getName()).isEqualTo("attachment1");
        ...
        assertThat(
            new AttachmentQuery(managementService).attachmentType("image/jpeg").count()).isEqualTo(
            1);
        attachments = new AttachmentQuery(managementService).attachmentType("image/jpeg").list();
        assertThat(attachments).hasSize(1);

        assertThat(
            new AttachmentQuery(managementService).orderByAttachmentName().desc().list().get(0)
                .getName()).isEqualTo("zattachment3");
