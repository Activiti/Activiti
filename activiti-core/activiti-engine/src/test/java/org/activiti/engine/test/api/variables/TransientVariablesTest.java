...
public class TransientVariablesTest extends PluggableActivitiTestCase {
    ...
    public void testTransientVariableDeletion() {
        String processInstanceId = runtimeService.startProcessInstanceByKey("transientVarsTest")
            .getId();
        long count = historyService.createHistoricVariableInstanceQuery()
            .processInstanceId(processInstanceId).variableName("response").count();
        assertThat(count).isEqualTo(0);
        count = historyService.createHistoricVariableInstanceQuery()
            .processInstanceId(processInstanceId).variableName("status").count();
        assertThat(count).isEqualTo(0);
    }
    ...
    public void testTaskCompleteWithTransientVariables() {
        ...
        ProcessInstance processInstance = runtimeService.startProcessInstanceByKey(
            "transientVarsTest", persistentVars);
        ...
        String combinedVar = (String) runtimeService.getVariable(processInstance.getId(),
            "combinedVar");
        assertThat(combinedVar).isEqualTo("Hello WorldABC123");

        assertThat(
            runtimeService.getVariable(processInstance.getId(), "persistentVar1")).isNotNull();
        assertThat(
            runtimeService.getVariable(processInstance.getId(), "persistentVar2")).isNotNull();

        assertThat(
            runtimeService.getVariable(processInstance.getId(), "unusedTransientVar")).isNull();
        assertThat(runtimeService.getVariable(processInstance.getId(), "transientVar")).isNull();
        assertThat(
            runtimeService.getVariable(processInstance.getId(), "secondTransientVar")).isNull();
        assertThat(
            runtimeService.getVariable(processInstance.getId(), "thirdTransientVar")).isNull();
    }
    ...
    public void testTransientVariableShadowsPersistentVariable() {
        ProcessInstance processInstance = runtimeService.startProcessInstanceByKey(
            "transientVarsTest", singletonMap("theVar", "Hello World"));
        Task task = taskService.createTaskQuery().processInstanceId(processInstance.getId())
            .singleResult();
    ...
    public void testTriggerWithTransientVars() {
        ...
        Execution executionInWait1 = runtimeService.createExecutionQuery().activityId("wait1")
            .singleResult();
        runtimeService.trigger(executionInWait1.getId(),
            singletonMap("persistentVar", "persistentValue01"));

        Execution executionInWait2 = runtimeService.createExecutionQuery().activityId("wait2")
            .singleResult();
        runtimeService.trigger(executionInWait2.getId(),
    ...
    /**
     * Mimics a service task that fetches data from a server and stored the whole thing in a
     * transient variable.
     */
    public static class FetchDataServiceTask implements JavaDelegate {

        public void execute(DelegateExecution execution) {
            execution.setTransientVariable("response",
