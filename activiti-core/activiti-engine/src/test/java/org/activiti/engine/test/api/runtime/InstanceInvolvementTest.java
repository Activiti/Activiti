...
public class InstanceInvolvementTest extends PluggableActivitiTestCase {
    ...
    public void testUserMultipleTimesinvolvedWithProcessInstance() {
        ...
        assertThat(
            runtimeService.createProcessInstanceQuery().involvedUser("kermit").count()).isEqualTo(
            1L);
    }

    private void assertNoInvolvement(String userId) {
        assertThat(
            runtimeService.createProcessInstanceQuery().involvedUser(userId).count()).isEqualTo(0L);
    }

    private void assertInvolvement(String userId, String instanceId) {
        ProcessInstance involvedInstance = runtimeService.createProcessInstanceQuery()
            .involvedUser(userId).singleResult();
    ...
    private boolean containsIdentityLink(List<IdentityLink> identityLinks, String userId,
        String type) {
