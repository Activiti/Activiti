 ...
 */
public class HistoricTaskQueryTest extends PluggableActivitiTestCase {
    ...
    public void testQuery() {
        if (processEngineConfiguration.getHistoryLevel().isAtLeast(HistoryLevel.ACTIVITY)) {
            HistoricTaskInstance task = historyService.createHistoricTaskInstanceQuery().taskAssignee(GONZO).singleResult();
            Map<String, Object> variableMap = task.getTaskLocalVariables();
            assertThat(variableMap).hasSize(0);
            ...
            task = historyService.createHistoricTaskInstanceQuery().taskAssignee(KERMIT).singleResult();
            assertThat(task.getProcessVariables()).hasSize(0);
            assertThat(task.getTaskLocalVariables()).hasSize(0);
            ...
            tasks = historyService.createHistoricTaskInstanceQuery().taskInvolvedUser(KERMIT).orderByTaskCreateTime().asc().list();
            assertThat(tasks).hasSize(3);
            ...
            task = historyService.createHistoricTaskInstanceQuery().taskAssignee(KERMIT).taskVariableValueEquals("localVar",
                "test").singleResult();
            assertThat(task.getProcessVariables()).hasSize(0);
            assertThat(task.getTaskLocalVariables()).hasSize(0);
            ...
            task = historyService.createHistoricTaskInstanceQuery().finished().singleResult();
