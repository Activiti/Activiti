...
public class CallActivityAdvancedTest extends PluggableActivitiTestCase {
    ...
    public void testCallSimpleSubProcess() {
        ...
        if (processEngineConfiguration.getHistoryLevel().isAtLeast(HistoryLevel.ACTIVITY)) {
            ...
            HistoricProcessInstance historicProcess = historyService.createHistoricProcessInstanceQuery()
                .processInstanceId(taskInSubProcess.getProcessInstanceId()).singleResult();
            ...
            List<HistoricActivityInstance> historicInstances = historyService.createHistoricActivityInstanceQuery()
                .processInstanceId(taskInSubProcess.getProcessInstanceId()).list();

            // Should contain a start-event, the task and an end-event
            assertThat(historicInstances).hasSize(3);
            Set<String> expectedActivities = new HashSet<String>(
                asList("theStart", "task", "theEnd"));

            for (HistoricActivityInstance act : historicInstances) {
                expectedActivities.remove(act.getActivityId());
            }
            assertThat(expectedActivities).as(
                "Not all expected activities were found in the history").isEmpty();
        }
    }
    ...
    public void testCallSimpleSubProcessWithExpressions() {
        ...
        // variable
        // into this task
        taskService.setVariable(taskBeforeSubProcess.getId(), "simpleSubProcessExpression",
            "simpleSubProcess");
        taskService.complete(taskBeforeSubProcess.getId());
        Task taskInSubProcess = taskQuery.singleResult();
        assertThat(taskInSubProcess.getName()).isEqualTo("Task in subprocess");
    ...
    /**
     * Test case for a possible tricky case: reaching the end event of the subprocess leads to an
     * end event in the super process instance.
     */
    @Deployment(resources = {
    ...
    public void testTimerOnCallActivity() {
        ...
        ProcessInstance pi2 = runtimeService.createProcessInstanceQuery()
            .superProcessInstanceId(pi1.getId()).singleResult();
        ...
        if (processEngineConfiguration.getHistoryLevel().isAtLeast(HistoryLevel.AUDIT)) {
            assertThat(
                historyService.createHistoricProcessInstanceQuery().processInstanceId(pi2.getId())
                    .singleResult()
                    .getDeleteReason()).startsWith(DeleteReason.BOUNDARY_EVENT_INTERRUPTING);
            assertHistoricTasksDeleteReason(pi2, DeleteReason.BOUNDARY_EVENT_INTERRUPTING,
                "Task in subprocess");
            assertHistoricActivitiesDeleteReason(pi1, DeleteReason.BOUNDARY_EVENT_INTERRUPTING,
                "callSubProcess");
            assertHistoricActivitiesDeleteReason(pi2, DeleteReason.BOUNDARY_EVENT_INTERRUPTING,
                "task");
        }
    }
