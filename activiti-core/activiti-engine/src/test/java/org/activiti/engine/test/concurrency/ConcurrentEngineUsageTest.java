...
public class ConcurrentEngineUsageTest extends PluggableActivitiTestCase {
    ...
    public void testConcurrentUsage() throws Exception {
        ...
        if (!processEngineConfiguration.getDatabaseType().equals("h2")
            ...
            ThreadPoolExecutor executor = new ThreadPoolExecutor(10, 10, 1000,
                TimeUnit.MILLISECONDS, new ArrayBlockingQueue<Runnable>(numberOfThreads));
            ...
            if (processEngineConfiguration.getHistoryLevel().isAtLeast(HistoryLevel.ACTIVITY)) {
                ...
                assertThat(historyService.createHistoricProcessInstanceQuery().finished()
                    .count()).isEqualTo(numberOfProcessesPerThread * numberOfThreads);
                assertThat(
                    historyService.createHistoricTaskInstanceQuery().finished().count()).isEqualTo(
                    totalNumberOfTasks);
            }
        }
    }
    ...
    protected void retryStartProcess(String runningUser) {
        ...
        while (retries > 0 && !success) {
            try {
                runtimeService.startProcessInstanceByKey("concurrentProcess",
                    singletonMap("assignee", runningUser));
                success = true;
            ...
            while (numberOfProcesses > 0 || tasksAvailable) {
                if (numberOfProcesses > 0 && !finishTask) {
                    ...
                    if (numberOfProcesses == 0) {
                        ...
                        // all started
                        tasksAvailable =
                            taskService.createTaskQuery().taskAssignee(drivingUser).count() > 0;
                    }
                    numberOfProcesses = numberOfProcesses - 1;
                } else {
                    // Finish a task
                    List<Task> taskToComplete = taskService.createTaskQuery()
                        .taskAssignee(drivingUser).listPage(0, 1);
