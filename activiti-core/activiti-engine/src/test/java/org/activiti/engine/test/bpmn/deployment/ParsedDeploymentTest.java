...
public class ParsedDeploymentTest extends PluggableActivitiTestCase {
    ...
    private static final String IDR_XML_NAME =
        "idr." + ResourceNameUtil.BPMN_RESOURCE_SUFFIXES[0];
    ...
    private static final String EN_XML_NAME =
        "en." + ResourceNameUtil.BPMN_RESOURCE_SUFFIXES[1];
    ...
    public void testCreateAndQuery() throws UnsupportedEncodingException {
        ...
        ParsedDeploymentBuilder builder = builderFactory.getBuilderForDeployment(
            entity
        );
        ...
        ProcessDefinitionEntity id1 = getProcessDefinitionEntityFromList(
            processDefinitions,
            ID1_ID
        );
        ProcessDefinitionEntity id2 = getProcessDefinitionEntityFromList(
            processDefinitions,
            ID2_ID
        );
        ...
        assertThat(
            parsedDeployment.getResourceForProcessDefinition(id1).getName()
        )
            .isEqualTo(IDR_XML_NAME);
        assertThat(
            parsedDeployment.getResourceForProcessDefinition(id2).getName()
        )
            .isEqualTo(IDR_XML_NAME);

        ProcessDefinitionEntity en1 = getProcessDefinitionEntityFromList(
            processDefinitions,
            EN1_ID
        );
        ProcessDefinitionEntity en2 = getProcessDefinitionEntityFromList(
            processDefinitions,
            EN2_ID
        );
        ...
        assertThat(
            parsedDeployment.getResourceForProcessDefinition(en1).getName()
        )
            .isEqualTo(EN_XML_NAME);
        assertThat(
            parsedDeployment.getResourceForProcessDefinition(en2).getName()
        )
            .isEqualTo(EN_XML_NAME);
    }

    private ProcessDefinitionEntity getProcessDefinitionEntityFromList(
        List<ProcessDefinitionEntity> list,
        String idString
    ) {
