...
public class StartTimerEventTest extends PluggableActivitiTestCase {
    ...
    public void testFixedDateStartTimerEvent() throws Exception {
        ...
        processEngineConfiguration.getClock().setCurrentTime(
            new SimpleDateFormat("dd/MM/yyyy hh:mm:ss").parse("15/11/2036 11:12:30"));
        waitForJobExecutorToProcessAllJobs(5000L,
            200L);
    ...
    public void testTimerShouldNotBeRemovedWhenUndeployingOldVersion() throws Exception {
        // Deploy test process
        String processXml = new String(IoUtil.readInputStream(getClass().getResourceAsStream(
    ...
    public void testOldJobsDeletedOnRedeploy() {
        ...
        for (int i = 0; i < 3; i++) {
            ...
            repositoryService.createDeployment()
                .addClasspathResource(
                    "org/activiti/engine/test/bpmn/event/timer/StartTimerEventTest.testOldJobsDeletedOnRedeploy.bpmn20.xml")
                .deploy();
        ...
        // Cleanup
        for (ProcessDefinition processDefinition : repositoryService.createProcessDefinitionQuery()
            .processDefinitionKey("timer").orderByProcessDefinitionVersion().desc().list()) {
    ...
    public void testTimersRecreatedOnDeploymentDelete() {
        ...
        repositoryService.createDeployment()
            .addClasspathResource(
                "org/activiti/engine/test/bpmn/event/timer/StartTimerEventTest.testTimersRecreatedOnDeploymentDelete_v1.bpmn20.xml")
            .deploy().getId();
        ...
        String deployment2 = repositoryService.createDeployment()
            .addClasspathResource(
                "org/activiti/engine/test/bpmn/event/timer/StartTimerEventTest.testTimersRecreatedOnDeploymentDelete_v2.bpmn20.xml")
            .deploy().getId();
        ...
        String deployment3 = repositoryService.createDeployment()
            .addClasspathResource(
                "org/activiti/engine/test/bpmn/event/timer/StartTimerEventTest.testTimersRecreatedOnDeploymentDelete_v3.bpmn20.xml")
            .deploy().getId();
        ...
        String deployment4 = repositoryService.createDeployment()
            .addClasspathResource(
                "org/activiti/engine/test/bpmn/event/timer/StartTimerEventTest.testTimersRecreatedOnDeploymentDelete_v4.bpmn20.xml")
            .deploy().getId();
        ...
        assertThat(repositoryService.createDeploymentQuery().count()).isEqualTo(2);
        assertThat(repositoryService.createProcessDefinitionQuery().count()).isEqualTo(2);
        assertThat(managementService.createTimerJobQuery().count()).isEqualTo(
            0); // v3 is still active
    ...
    public void testTimersRecreatedOnDeploymentDeleteWithTenantId() {
        ...
        assertThat(repositoryService.createDeploymentQuery().deploymentTenantId(testTenant)
            .count()).isEqualTo(1);
        assertThat(
            repositoryService.createProcessDefinitionQuery().processDefinitionTenantId(testTenant)
                .count()).isEqualTo(1);
        assertThat(
            managementService.createTimerJobQuery().jobTenantId(testTenant).count()).isEqualTo(1);
        ...
        assertThat(
            repositoryService.createProcessDefinitionQuery().processDefinitionTenantId(testTenant)
                .count()).isEqualTo(2);
        assertThat(
            managementService.createTimerJobQuery().jobTenantId(testTenant).count()).isEqualTo(0);
    ...
    public void testMultipleStartEvents() {
        ...
        String deploymentId = repositoryService.createDeployment()
            .addClasspathResource(
                "org/activiti/engine/test/bpmn/event/timer/StartTimerEventTest.testMultipleStartEvents.bpmn20.xml")
            .deploy().getId();
    ...
    private void validateTaskCounts(long taskACount,
        ...
        long taskCCount,
        long taskDCount) {
        assertThat(taskService.createTaskQuery().taskName("Task A").count()).as(
            "task A counts are incorrect").isEqualTo(taskACount);
        assertThat(taskService.createTaskQuery().taskName("Task B").count()).as(
            "task B counts are incorrect").isEqualTo(taskBCount);
        assertThat(taskService.createTaskQuery().taskName("Task C").count()).as(
            "task C counts are incorrect").isEqualTo(taskCCount);
        assertThat(taskService.createTaskQuery().taskName("Task D").count()).as(
            "task D counts are incorrect").isEqualTo(taskDCount);
    }
