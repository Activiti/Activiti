...
public class CancelCallActivityByMessageTest extends PluggableActivitiTestCase {
    ...
    public void testCancelCallActivityByMessage() throws Exception {
        ...
        ActivitiEntityEvent entityEvent = (ActivitiEntityEvent) myEventListener.getEventsReceived()
            .get(0);
        ...
        ActivitiActivityEvent activityEvent = (ActivitiActivityEvent) myEventListener.getEventsReceived()
            .get(3);
        ...
        assertThat(((ActivitiCancelledEvent) activityEvent).getCause()).isEqualTo(
            "boundary event (cancelBoundaryMessageEvent)");

        ActivitiEntityEvent taskCancelledEvent = (ActivitiEntityEvent) myEventListener.getEventsReceived()
            .get(15);
        assertThat(taskCancelledEvent.getType()).isEqualTo(ActivitiEventType.ENTITY_DELETED);
        assertThat(((TaskEntity) taskCancelledEvent.getEntity()).getName()).isEqualTo(
            taskEntity.getName());

        ActivitiCancelledEvent processCancelledEvent = (ActivitiCancelledEvent) myEventListener.getEventsReceived()
            .get(16);
        assertThat(processCancelledEvent.getType()).isEqualTo(ActivitiEventType.PROCESS_CANCELLED);
        assertThat(processCancelledEvent.getExecutionId()).isEqualTo(
            processCancelledEvent.getProcessInstanceId());
