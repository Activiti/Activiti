...
public class MultiTenantProcessEngineTest {
  ...
  private void setupProcessEngine(boolean sharedExecutor) {
    ...
    config.registerTenant(
        "acme", createDataSource("jdbc:h2:mem:activiti-mt-acme;DB_CLOSE_DELAY=1000", "sa", ""));
    config.registerTenant(
        "starkindustries",
  ...
  private void startProcessInstances(String userId) {
    ...
    ProcessInstance processInstance =
        processEngine.getRuntimeService().startProcessInstanceByKey("oneTaskProcess", vars);
    List<Task> tasks =
        processEngine
            .getTaskService()
            .createTaskQuery()
    ...
    System.out.println(
        "Got "
            + processEngine.getHistoryService().createHistoricProcessInstanceQuery().count()
