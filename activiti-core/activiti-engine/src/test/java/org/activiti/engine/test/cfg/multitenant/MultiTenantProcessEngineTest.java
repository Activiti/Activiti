...
public class MultiTenantProcessEngineTest {
    ...
    private void setupProcessEngine(boolean sharedExecutor) {
        ...
        config.registerTenant("alfresco",
            createDataSource("jdbc:h2:mem:activiti-mt-alfresco;DB_CLOSE_DELAY=1000", "sa", ""));
        config.registerTenant("acme",
            createDataSource("jdbc:h2:mem:activiti-mt-acme;DB_CLOSE_DELAY=1000", "sa", ""));
        config.registerTenant("starkindustries",
    ...
    protected void runProcessInstanceTest() throws InterruptedException {
        ...
        config.registerTenant("dailyplanet",
        ...
        config.getClock().setCurrentTime(
            new Date(config.getClock().getCurrentTime().getTime() + (2 * 60 * 60 * 1000)));
    ...
    private void startProcessInstances(String userId) {
        ...
        ProcessInstance processInstance = processEngine.getRuntimeService()
            .startProcessInstanceByKey("oneTaskProcess", vars);
        List<Task> tasks = processEngine.getTaskService().createTaskQuery()
            .processInstanceId(processInstance.getId()).list();
        System.out.println("Got " + tasks.size() + " tasks");

        System.out.println(
            "Got " + processEngine.getHistoryService().createHistoricProcessInstanceQuery().count()
    ...
    private void assertData(String userId, long nrOfActiveProcessInstances, long nrOfActiveJobs) {
        ...
        assertThat(
            processEngine.getRuntimeService().createExecutionQuery().onlyProcessInstanceExecutions()
                .count()).isEqualTo(nrOfActiveProcessInstances);
        assertThat(
            processEngine.getHistoryService().createHistoricProcessInstanceQuery().unfinished()
                .count()).isEqualTo(nrOfActiveProcessInstances);
        assertThat(processEngine.getManagementService().createTimerJobQuery().count()).isEqualTo(
            nrOfActiveJobs);
