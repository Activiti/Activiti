...
public class ProcessInstanceIdentityLinksTest extends PluggableActivitiTestCase {
    ...
    public void testParticipantUserLink() {
        ...
        String processInstanceId =
                runtimeService.createProcessInstanceQuery().singleResult().getId();
    ...
    public void testCandidateGroupLink() {
        ...
        if (processEngineConfiguration.getHistoryLevel().isAtLeast(HistoryLevel.AUDIT)) {
            List<Event> processInstanceEvents =
                    runtimeService.getProcessInstanceEvents(processInstanceId);
            Event processIsntanceEvent =
                    findProcessInstanceEvent(processInstanceEvents, Event.ACTION_DELETE_GROUP_LINK);
