...
/**
 */
public class ProcessInstanceIdentityLinksTest
    extends PluggableActivitiTestCase {

    @Deployment(
        resources = "org/activiti/engine/test/api/runtime/IdentityLinksProcess.bpmn20.xml"
    )
    public void testParticipantUserLink() {
        ...
        assertThat(identityLink.getType())
            .isEqualTo(IdentityLinkType.PARTICIPANT);
        assertThat(identityLink.getProcessInstanceId())
    ...
    public void testCandidateGroupLink() {
        ...
        if (
            ...
            List<Event> processInstanceEvents = runtimeService.getProcessInstanceEvents(
                processInstanceId
            );
            assertThat(processInstanceEvents).hasSize(1);
            Event processInstanceEvent = processInstanceEvents.get(0);
            assertThat(processInstanceEvent.getAction())
                .isEqualTo(Event.ACTION_ADD_GROUP_LINK);
            List<String> processInstanceEventMessageParts = processInstanceEvent.getMessageParts();
            assertThat(processInstanceEventMessageParts.get(0))
                .isEqualTo("muppets");
            assertThat(processInstanceEventMessageParts.get(1))
                .isEqualTo(IdentityLinkType.PARTICIPANT);
        ...
        if (
            ...
            Event processIsntanceEvent = findProcessInstanceEvent(
                processInstanceEvents,
                Event.ACTION_DELETE_GROUP_LINK
            );
            assertThat(processIsntanceEvent.getAction())
                .isEqualTo(Event.ACTION_DELETE_GROUP_LINK);
    ...
    private Event findProcessInstanceEvent(
        List<Event> processInstanceEvents,
        String action
    ) {
        ...
        throw new AssertionFailedError(
            "no process instance event found with action " + action
        );
    }
    ...
    public void testCustomLinkGroupLink() {
        ...
        runtimeService.deleteGroupIdentityLink(
            processInstanceId,
            "muppets",
            "playing"
        );
