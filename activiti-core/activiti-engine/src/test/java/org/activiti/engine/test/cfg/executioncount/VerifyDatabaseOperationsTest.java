...
public class VerifyDatabaseOperationsTest extends PluggableActivitiTestCase {
    ...
    protected void setUp() throws Exception {
        ...
        // Enable flags
        this.oldExecutionTreeFetchValue = processEngineConfiguration.getPerformanceSettings()
            .isEnableEagerExecutionTreeFetching();
        this.oldExecutionRelationshipCountValue = processEngineConfiguration.getPerformanceSettings()
        ...
        processEngineConfiguration.getPerformanceSettings()
            .setEnableEagerExecutionTreeFetching(true);
        processEngineConfiguration.getPerformanceSettings()
            .setEnableExecutionRelationshipCounts(true);
        processEngineConfiguration.setEnableProcessDefinitionInfoCache(false);
        ((DefaultHistoryManager) processEngineConfiguration.getHistoryManager()).setHistoryLevel(
            HistoryLevel.AUDIT);
        ...
        newDbSqlSessionFactory.setDatabaseTablePrefix(
            oldDbSqlSessionFactory.getDatabaseTablePrefix());
        newDbSqlSessionFactory.setTablePrefixIsSchema(
            oldDbSqlSessionFactory.isTablePrefixIsSchema());
    ...
    protected void tearDown() throws Exception {
        ...
        ((CommandExecutorImpl) processEngineConfiguration.getCommandExecutor()).setFirst(
            oldFirstCommandInterceptor);
    ...
    public void testVariablesAndPassthrough() {
        deployStartProcessInstanceAndProfile("process-variables-servicetask01.bpmn20.xml",
    ...
    public void testOneTaskProcess() {
        deployStartProcessInstanceAndProfile("process-usertask-01.bpmn20.xml",
            "process-usertask-01", false);
        Task task = taskService.createTaskQuery().singleResult();
        taskService.complete(task.getId());
        stopProfiling();

        assertExecutedCommands("StartProcessInstanceCmd", "org.activiti.engine.impl.TaskQueryImpl",
            "CompleteTaskCmd");
    ...
    protected void assertExecutedCommands(String... commands) {
        ...
        for (String command : commands) {
            assertThat(getStatsForCommand(command, allStats)).as(
                "Could not get stats for " + command).isNotNull();
        }
    }
    ...
    protected void assertDatabaseSelects(String commandClass, Object... expectedSelects) {
        ...
        for (int i = 0; i < expectedSelects.length; i += 2) {
            ...
            assertThat(stats.getDbSelects().get(dbSelect)).as("Wrong select count for " + dbSelect)
                .isEqualTo(count);
        }
    }
    ...
    protected void assertDatabaseInserts(String commandClass, Object... expectedInserts) {
        ...
        if (expectedInserts.length / 2 != stats.getDbInserts().size()) {
            fail("Unexpected number of database inserts : " + stats.getDbInserts().size()
                + ", but expected " + expectedInserts.length / 2);
        }
        ...
        for (int i = 0; i < expectedInserts.length; i += 2) {
            ...
            assertThat(stats.getDbInserts()
                .get("org.activiti.engine.impl.persistence.entity." + dbInsert)).as(
                "Insert count for " + dbInsert + "not correct").isEqualTo(count);
        }
    }
    ...
    protected void assertDatabaseDeletes(String commandClass, Object... expectedDeletes) {
        ...
        for (int i = 0; i < expectedDeletes.length; i += 2) {
            ...
            assertThat(stats.getDbDeletes()
                .get("org.activiti.engine.impl.persistence.entity." + dbDelete)).as(
                "Delete count for " + dbDelete + "not correct").isEqualTo(count);
        }
    }
    ...
    protected CommandStats getStatsForCommand(String commandClass,
        Map<String, CommandStats> allStats) {
    ...
    protected ActivitiProfiler deployStartProcessInstanceAndProfile(String path,
        String processDefinitionKey) {
        return deployStartProcessInstanceAndProfile(path, processDefinitionKey, true);
    }

    protected ActivitiProfiler deployStartProcessInstanceAndProfile(String path,
        String processDefinitionKey, boolean stopProfilingAfterStart) {
        deploy(path);
    ...
    protected void deploy(String path) {
        repositoryService.createDeployment()
            .addClasspathResource("org/activiti/engine/test/cfg/executioncount/" + path).deploy();
    }
