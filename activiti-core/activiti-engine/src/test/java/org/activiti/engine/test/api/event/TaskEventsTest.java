...
public class TaskEventsTest extends PluggableActivitiTestCase {
    ...
    public void testEventFiringOrdering() {
        // We need to add a special listener that copies the Task values - to record its state when
        // the
        // event fires,
        ...
        try {
            ...
            // verify script listener has done its job, on create before ActivitiEntityEvent was
            // fired
            assertThat(taskFromEvent.getAssignee())
                    .as(
                            "The ScriptTaskListener must set this value before the dispatchEvent"
                                    + " fires.")
