...
public class IntermediateTimerEventRepeatCompatibilityTest extends TimerEventCompatibilityTest {
    ...
    public void testRepeatWithEnd() throws Throwable {
        ...
        nextTimeCal.add(
                Calendar.HOUR,
                1); // after 1 hour the event must be triggered and the flow will go to the next
        // step
        processEngineConfiguration.getClock().setCurrentTime(nextTimeCal.getTime());
        ...
        nextTimeCal.add(
                Calendar.HOUR,
                1); // after 1H 40 minutes from process start, the timer will trigger because of the
        // endDate
        processEngineConfiguration.getClock().setCurrentTime(nextTimeCal.getTime());
