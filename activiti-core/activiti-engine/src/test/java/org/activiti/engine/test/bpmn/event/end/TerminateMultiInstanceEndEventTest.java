...
public class TerminateMultiInstanceEndEventTest extends PluggableActivitiTestCase {
    ...
    public void testMultiInstanceEmbeddedSubprocess() {
        ...
        Task aTask = taskService.createTaskQuery().processInstanceId(processInstance.getId())
            .singleResult();
        taskService.complete(aTask.getId());

        List<Task> bTasks = taskService.createTaskQuery().processInstanceId(processInstance.getId())
            .list();
        ...
        bTasks = taskService.createTaskQuery().processInstanceId(processInstance.getId())
            .taskName("B").list();
        assertThat(bTasks).hasSize(6);

        taskService.complete(bTasks.get(0).getId(), singletonMap("myVar", "toEnd"));

        Task afterMiTask = taskService.createTaskQuery().processInstanceId(processInstance.getId())
            .singleResult();
    ...
    public void testTerminateMiCallactivity() {
        ...
        Task taskA = taskService.createTaskQuery().processInstanceId(processInstance.getId())
            .singleResult();
        ...
        List<Task> afterMiTasks = taskService.createTaskQuery()
    ...
    public void testTerminateNestedMiEmbeddedSubprocess() {
        ...
        List<Task> aTasks = taskService.createTaskQuery().processInstanceId(processInstance.getId())
            .taskName("A").list();
        ...
        List<Task> afterInnerMiTasks = taskService.createTaskQuery().taskName("AfterInnerMi")
            .list();
        ...
        while (nextTasks != null && nextTasks.size() > 0) {
            ...
            nextTasks = taskService.createTaskQuery().processInstanceId(processInstance.getId())
                .list();
        }
    ...
    public void testTerminateNestedMiEmbeddedSubprocess2() {
        ...
        List<Task> aTasks = taskService.createTaskQuery().processInstanceId(processInstance.getId())
            .taskName("A").list();
        assertThat(aTasks).hasSize(12);
        List<Task> afterInnerMiTasks = taskService.createTaskQuery()
            .processInstanceId(processInstance.getId()).taskName("AfterInnerMi").list();
