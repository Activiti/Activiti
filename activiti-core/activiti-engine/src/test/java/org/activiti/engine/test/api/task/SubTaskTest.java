...
public class SubTaskTest extends PluggableActivitiTestCase {
    ...
    public void testSubTask() {
        ...
        assertThat(
            historyService.createHistoricTaskInstanceQuery().taskParentTaskId(subTaskId).list()
                .isEmpty()).isTrue();
    ...
    public void testSubTaskDeleteOnProcessInstanceDelete() {
        ...
        if (processEngineConfiguration.getHistoryLevel().isAtLeast(HistoryLevel.ACTIVITY)) {
            List<HistoricTaskInstance> historicTasks = historyService.createHistoricTaskInstanceQuery()
                .taskAssignee("test").list();
            assertThat(historicTasks).hasSize(3);

            historyService.deleteHistoricProcessInstance(processInstance.getId());

            historicTasks = historyService.createHistoricTaskInstanceQuery().taskAssignee("test")
                .list();
