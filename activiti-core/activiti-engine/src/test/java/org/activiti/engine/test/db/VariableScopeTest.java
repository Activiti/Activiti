...
public class VariableScopeTest extends PluggableActivitiTestCase {
    ...
    public void testVariableScope() {
        ...
        assertThat(runtimeService.getVariable(subProcessTask.getExecutionId(), "test")).isEqualTo(
            "test2");
        assertThat(
            runtimeService.getVariables(subProcessTask.getExecutionId()).get("test")).isEqualTo(
            "test2");
    ...
    public void testVariableNamesScope() {
        ...
        Task subProcessTask = taskService.createTaskQuery().processInstanceId(pi.getId())
            .singleResult();
        runtimeService.setVariableLocal(pi.getProcessInstanceId(), "mainProcessLocalVariable",
            "Hello World");

        assertThat(subProcessTask.getName()).isEqualTo("Task in subprocess");

        runtimeService.setVariableLocal(subProcessTask.getExecutionId(), "subProcessLocalVariable",
            "Hello SubProcess");

        // Returns a set of local variablenames of pi
        List<String> result = processEngineConfiguration.getCommandExecutor()
        ...
        result = processEngineConfiguration.getCommandExecutor()
    ...
    public void testModeledVariableScope() {
        ...
        for (Task subProcTask : subProcessTasks) {
            if (subProcTask.getName().equals("Task in subprocess2")) {
                ...
                // of variable test --> test3
                assertThat(
                    runtimeService.getVariable(subProcTask.getExecutionId(), "test")).isEqualTo(
                    "test3");
                assertThat(runtimeService.getVariables(subProcTask.getExecutionId())
                    .get("test")).isEqualTo("test3");
