...
public class HistoricVariableInstanceTest extends PluggableActivitiTestCase {
    ...
    public void testSimple() {
        if (processEngineConfiguration.getHistoryLevel().isAtLeast(HistoryLevel.FULL)) {
            ...
            HistoricVariableInstanceEntity historicVariable = (HistoricVariableInstanceEntity) variables.getFirst();
    ...
    public void testHistoricVariableInstanceQuery() {
        if (processEngineConfiguration.getHistoryLevel().isAtLeast(HistoryLevel.FULL)) {
            ...
            assertThat(
                historyService.createHistoricVariableInstanceQuery().orderByProcessInstanceId().asc().count()
            ).isEqualTo(4);
            assertThat(
                historyService.createHistoricVariableInstanceQuery().orderByProcessInstanceId().asc().list()
            ).hasSize(4);
            assertThat(
                historyService.createHistoricVariableInstanceQuery().orderByVariableName().asc().count()
            ).isEqualTo(4);
            assertThat(historyService.createHistoricVariableInstanceQuery().orderByVariableName().asc().list()).hasSize(
                4
            );
            ...
            assertThat(
                historyService.createHistoricVariableInstanceQuery().variableNameLike("myVar1").count()
            ).isEqualTo(2);
            assertThat(historyService.createHistoricVariableInstanceQuery().variableNameLike("myVar1").list()).hasSize(
                2
            );
            ...
            assertThat(
                historyService.createHistoricVariableInstanceQuery().variableValueEquals("myVar", "test123").count()
            ).isEqualTo(1);
            assertThat(
                historyService.createHistoricVariableInstanceQuery().variableValueEquals("myVar", "test123").list()
            ).hasSize(1);
            assertThat(
                historyService.createHistoricVariableInstanceQuery().variableValueEquals("myVar1", "test456").count()
            ).isEqualTo(1);
            assertThat(
                historyService.createHistoricVariableInstanceQuery().variableValueEquals("myVar1", "test456").list()
            ).hasSize(1);
            assertThat(
                historyService.createHistoricVariableInstanceQuery().variableValueEquals("myVar", "test666").count()
            ).isEqualTo(1);
            assertThat(
                historyService.createHistoricVariableInstanceQuery().variableValueEquals("myVar", "test666").list()
            ).hasSize(1);
            assertThat(
                historyService.createHistoricVariableInstanceQuery().variableValueEquals("myVar1", "test666").count()
            ).isEqualTo(1);
            assertThat(
                historyService.createHistoricVariableInstanceQuery().variableValueEquals("myVar1", "test666").list()
            ).hasSize(1);
    ...
    public void testHistoricVariableQuery2() {
        ...
        List<String> expectedVariableNames = asList(
            "executionVar0",
            "executionVar1",
            "startVar",
            "taskVar0",
        ...
        historicVariableInstances = historyService
            .createHistoricVariableInstanceQuery()
            .executionId(tasks.getFirst().getExecutionId())
            .orderByVariableName()
            .asc()
            .list();
        assertThat(historicVariableInstances).hasSize(2);
        assertThat(historicVariableInstances.getFirst().getVariableName()).isEqualTo("executionVar0");
        assertThat(historicVariableInstances.get(1).getVariableName()).isEqualTo("taskVar0");
        historicVariableInstances = historyService
            .createHistoricVariableInstanceQuery()
            .executionId(tasks.get(1).getExecutionId())
            .orderByVariableName()
            .asc()
            .list();
        assertThat(historicVariableInstances).hasSize(2);
        assertThat(historicVariableInstances.getFirst().getVariableName()).isEqualTo("executionVar1");
        ...
        assertThat(historicVariableInstances.getFirst().getVariableName()).isEqualTo("taskVar0");
        historicVariableInstances = historyService
            .createHistoricVariableInstanceQuery()
            .taskId(tasks.get(1).getId())
            .list();
        assertThat(historicVariableInstances).hasSize(1);
        assertThat(historicVariableInstances.getFirst().getVariableName()).isEqualTo("taskVar1");
    ...
    public void testHistoricVariableQueryByExecutionIds() {
        ...
        assertThat(
            historyService.createHistoricVariableInstanceQuery().executionIds(testProcessInstanceIds).list().size()
        )
            .isEqualTo(
                (int) historyService.createHistoricVariableInstanceQuery().executionIds(testProcessInstanceIds).count()
            )
            .isEqualTo(2);

        List<HistoricVariableInstance> historicVariableInstances = historyService
            .createHistoricVariableInstanceQuery()
            .executionIds(testProcessInstanceIds)
            .list();
        assertThat(historicVariableInstances)
            .hasSize(2)
            .extracting("name", "value")
            .containsExactly(tuple("startVar", "hello"), tuple("startVar", "hello"));
    ...
    public void testHistoricVariableQueryByExecutionIdsForScope() {
        ...
        assertThat("processVar").isEqualTo(historicVariableInstances.getFirst().getVariableName());
        assertThat("processVar").isEqualTo(historicVariableInstances.getFirst().getValue());
        ...
        assertThat("executionVar").isEqualTo(historicVariableInstances.getFirst().getVariableName());
        assertThat("executionVar").isEqualTo(historicVariableInstances.getFirst().getValue());
    ...
    public void testHistoricVariableQueryByTaskIds() {
        ...
        List<Task> tasks = taskService.createTaskQuery().processInstanceId(processInstanceId).list();
        taskService.setVariableLocal(tasks.getFirst().getId(), "taskVar1", "hello1");
        taskService.setVariableLocal(tasks.get(1).getId(), "taskVar2", "hello2");

        Set<String> taskIds = new HashSet<String>();
        taskIds.add(tasks.getFirst().getId());
        ...
        assertThat(historicVariableInstances).hasSize(2);
        assertThat(historicVariableInstances.getFirst().getVariableName()).isEqualTo("taskVar1");
        assertThat(historicVariableInstances.getFirst().getValue()).isEqualTo("hello1");
    ...
    public void testHistoricVariableQueryByTaskIdsForScope() {
        ...
        List<HistoricVariableInstance> historicVariableInstances = historyService
            ...
            .list();
        assertThat(2).isEqualTo(historicVariableInstances.size());
        assertThat("taskVar").isEqualTo(historicVariableInstances.getFirst().getVariableName());
        assertThat("taskVar").isEqualTo(historicVariableInstances.getFirst().getValue());
    ...
    public void testNativeHistoricVariableInstanceQuery() {
        if (processEngineConfiguration.getHistoryLevel().isAtLeast(HistoryLevel.FULL)) {
            ...
            assertThat(managementService.getTableName(HistoricVariableInstanceEntity.class)).isEqualTo(
                "ACT_HI_VARINST"
            );
            ...
            // paging
            assertThat(
                historyService.createNativeHistoricVariableInstanceQuery().sql(baseQuerySql).listPage(0, 3)
            ).hasSize(3);
            assertThat(
                historyService.createNativeHistoricVariableInstanceQuery().sql(baseQuerySql).listPage(1, 3)
            ).hasSize(2);
            assertThat(
                historyService
                    .createNativeHistoricVariableInstanceQuery()
        ...
        if (processEngineConfiguration.getHistoryLevel().isAtLeast(HistoryLevel.FULL)) {
            ...
            assertThat(
                historyService
                    ...
                    .sql(sqlWithConditions)
                    .parameter("name", "myVar")
                    .parameter("type", "VariableUpdate")
                    .singleResult()
            ).isNotNull();

            sqlWithConditions = baseQuerySql + " where NAME_ like #{name}";
            assertThat(
                historyService.createNativeHistoricDetailQuery().sql(sqlWithConditions).parameter("name", "var%").list()
            ).hasSize(2);
