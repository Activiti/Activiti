...
public class FailedJobRetryCmdTest extends PluggableActivitiTestCase {
    ...
    public void testFailedServiceTask() {
        ...
        assertThat(managementService.createDeadLetterJobQuery().withException().count()).isEqualTo(
            1);
    ...
    protected Job fetchJob(String processInstanceId) {
        return managementService.createTimerJobQuery().processInstanceId(processInstanceId)
            .singleResult();
    }
    ...
    protected ExecutionEntity refreshExecutionEntity(String executionId) {
        return (ExecutionEntity) runtimeService.createExecutionQuery().executionId(executionId)
            .singleResult();
    }
