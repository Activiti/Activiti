...
public class OptimisticLockingExceptionTest extends PluggableActivitiTestCase {
    ...
    public void testOptimisticLockExceptionForConcurrentJoin() throws Exception {
        ...
        for (int i = 0; i < 10; i++) {
            ...
            Execution execution1 = runtimeService.createExecutionQuery().activityId("wait1")
                .processInstanceId(processInstance.getId()).singleResult();
            Execution execution2 = runtimeService.createExecutionQuery().activityId("wait2")
                .processInstanceId(processInstance.getId()).singleResult();
            ...
            while (t1.getException() == null && t2.getException() == null &&
                runtimeService.createProcessInstanceQuery()
                    .processInstanceId(processInstance.getId()).count() == 1) {
                Thread.sleep(250L);
                totalWaitTime += 250L;
            ...
            boolean processInstanceEnded = runtimeService.createProcessInstanceQuery()
                .processInstanceId(processInstance.getId()).count() == 0;
    ...
    /**
     * Simple extension of the triggerCommand, that adds a {@link OptimisticLockingTestCommandContextCloseListener}
     * to the current {@link CommandContext} before doing the regular trigger.
     */
    ...
    /**
     * {@link CommandContextCloseListener} that halts the closing of the {@link CommandContext}
     * until all threads are synchronized.
     */
    public static class OptimisticLockingTestCommandContextCloseListener implements
        CommandContextCloseListener {

        public static CyclicBarrier TEST_BARRIER_BEFORE_CLOSE = new CyclicBarrier(
            3); // 2 threads for triggering the wait state, one for the unit test thread
