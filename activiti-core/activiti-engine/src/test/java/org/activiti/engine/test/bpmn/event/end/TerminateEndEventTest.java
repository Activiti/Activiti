...
public class TerminateEndEventTest extends PluggableActivitiTestCase {
    ...
    public void testProcessTerminate() throws Exception {
        ...
        assertHistoricActivitiesDeleteReason(
                pi, DeleteReason.TERMINATE_END_EVENT, "preNormalTerminateTask");
    ...
    public void testTerminateWithSubProcessTerminateAll() throws Exception {
        ...
        assertHistoricTasksDeleteReason(
                pi, DeleteReason.TERMINATE_END_EVENT, "check before termination");
        assertHistoricActivitiesDeleteReason(
                pi, DeleteReason.TERMINATE_END_EVENT, "preTerminateEnd");
    ...
    public void testTerminateWithCallActivity() throws Exception {
        ...
        assertHistoricTasksDeleteReason(pi, null, "check before termination");
        assertHistoricTasksDeleteReason(
                subProcessInstance, DeleteReason.TERMINATE_END_EVENT, "Perform Sample");
        assertHistoricActivitiesDeleteReason(pi, null, "preTerminateEnd");
        assertHistoricActivitiesDeleteReason(
                subProcessInstance, DeleteReason.TERMINATE_END_EVENT, "task");
    }
    ...
    public void testTerminateInSubProcessWithBoundary() throws Exception {
        ...
        ProcessInstance pi =
                runtimeService.startProcessInstanceByKey("terminateEndEventWithBoundary");
        ...
        assertHistoricProcessInstanceDeleteReason(pi, DeleteReason.TERMINATE_END_EVENT);
        assertHistoricTasksDeleteReason(
                pi, DeleteReason.TERMINATE_END_EVENT, "check before normal end");
        ...
        // 'preEndInnerTask' task in subprocess should have been terminated, only outerTask should
        // exist
        assertThat(taskService.createTaskQuery().processInstanceId(pi.getId()).count())
                .isEqualTo(1);
    ...
    public void testTerminateInSubProcessConcurrentMultiInstance() throws Exception {
        ...
        List<Task> tasks = taskService.createTaskQuery().processInstanceId(pi.getId()).list();
        assertThat(tasks)
                .hasSize(4); // 3 user tasks in MI  +1 (preNormalEnd) = 4 (2 were killed because it
        // went directly
        // to the terminate end event)
    ...
    public void testTerminateInCallActivityConcurrentCallActivity() throws Exception {
        // GIVEN - process instance starts and creates 2 subProcessInstances (with 2 user tasks -
        // preTerminate and my task)
        ProcessInstance pi =
                runtimeService.startProcessInstanceByKey(
    ...
    public void testMiCallActivityParallel() {
        ProcessInstance processInstance =
                runtimeService.startProcessInstanceByKey("testMiCallActivity");
    ...
    public void testTerminateNestedSubprocesses() {
        ProcessInstance processInstance =
                runtimeService.startProcessInstanceByKey("TestTerminateNestedSubprocesses");
    ...
    public void testTerminateNestedMiSubprocesses() {
        ProcessInstance processInstance =
                runtimeService.startProcessInstanceByKey("TestTerminateNestedMiSubprocesses");
    ...
    @Deployment
    public void testTerminateNestedMiSubprocessesTerminateAll3() { // Same as 1, but sequential
        // Multi-Instance
        ProcessInstance processInstance =
                runtimeService.startProcessInstanceByKey("TestTerminateNestedMiSubprocesses");
        Task task =
    ...
    @Deployment
    public void testTerminateNestedMiSubprocessesTerminateAll4() { // Same as 2, but sequential
        // Multi-Instance
        ProcessInstance processInstance =
                runtimeService.startProcessInstanceByKey("TestTerminateNestedMiSubprocesses");
        taskService.complete(taskService.createTaskQuery().taskName("A").singleResult().getId());
    ...
    public void testNestedCallActivities() {
        ProcessInstance processInstance =
                runtimeService.startProcessInstanceByKey("TestNestedCallActivities");
    ...
    public void testNestedCallActivitiesTerminateAll() {
        ...
        // Now the tricky one. 'Before A' leads to 'callActivity A', which calls subprocess02 which
        // terminates
        processInstance = runtimeService.startProcessInstanceByKey("TestNestedCallActivities");
        tasks =
                assertTaskNames(
    ...
    // Unit test for ACT-4101 : NPE when there are multiple routes to terminateEndEvent, and both
    // are
    // reached
    @Deployment
    public void testThreeExecutionsArrivingInTerminateEndEvent() {
        ...
        ProcessInstance processInstance =
                runtimeService.startProcessInstanceByKey("skybox_image_pull_request", variableMap);
    ...
    protected void assertHistoricActivitiesDeleteReason(
            ProcessInstance processInstance, String expectedDeleteReason, String... activityIds) {
        if (processEngineConfiguration.getHistoryLevel().isAtLeast(HistoryLevel.AUDIT)) {
            for (String activityId : activityIds) {
                List<HistoricActivityInstance> historicActiviyInstances =
                        historyService
