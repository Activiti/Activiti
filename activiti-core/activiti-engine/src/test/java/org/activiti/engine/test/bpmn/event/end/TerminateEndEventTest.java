...
public class TerminateEndEventTest extends PluggableActivitiTestCase {
    ...
    public void testProcessTerminate() throws Exception {
        ...
        assertHistoricActivitiesDeleteReason(pi, DeleteReason.TERMINATE_END_EVENT,
            "preNormalTerminateTask");
    ...
    public void testTerminateWithSubProcessTerminateAll() throws Exception {
        ...
        assertHistoricTasksDeleteReason(pi, DeleteReason.TERMINATE_END_EVENT,
    ...
    public void testTerminateWithCallActivity() throws Exception {
        ...
        assertHistoricProcessInstanceDeleteReason(pi, DeleteReason.TERMINATE_END_EVENT);
        assertHistoricTasksDeleteReason(pi, null, "check before termination");
        assertHistoricTasksDeleteReason(subProcessInstance, DeleteReason.TERMINATE_END_EVENT,
            "Perform Sample");
        assertHistoricActivitiesDeleteReason(pi, null, "preTerminateEnd");
        assertHistoricActivitiesDeleteReason(subProcessInstance, DeleteReason.TERMINATE_END_EVENT,
            "task");
    }
    ...
    public void testTerminateInExclusiveGatewayWithCallActivity() throws Exception {
        ProcessInstance pi = runtimeService.startProcessInstanceByKey(
            "terminateEndEventExample-terminateAfterExclusiveGateway");
    ...
    @Deployment
    public void testTerminateInExclusiveGatewayWithMultiInstanceSubProcessTerminateAll()
        throws Exception {
        ProcessInstance pi = runtimeService.startProcessInstanceByKey(
            "terminateEndEventExample-terminateAfterExclusiveGateway");
    ...
    public void testTerminateInSubProcessWithBoundary() throws Exception {
        ...
        ProcessInstance pi = runtimeService.startProcessInstanceByKey(
            "terminateEndEventWithBoundary");

        assertThat(taskService.createTaskQuery().processInstanceId(pi.getId()).count()).isEqualTo(
            3);
        ...
        Task task = taskService.createTaskQuery().processInstanceId(pi.getId())
            .taskDefinitionKey("preTermInnerTask").singleResult();
        taskService.complete(task.getId());
        ...
        // complete outerTask
        task = taskService.createTaskQuery().processInstanceId(pi.getId())
            .taskDefinitionKey("outerTask").singleResult();
        taskService.complete(task.getId());
    ...
    public void testTerminateInSubProcessConcurrentTerminateAll2() throws Exception {
        ...
        Task task = taskService.createTaskQuery().processInstanceId(pi.getId())
            .taskName("User Task").singleResult();
        taskService.complete(task.getId());
    ...
    public void testTerminateInSubProcessConcurrentMultiInstance() throws Exception {
        ...
        List<Task> tasks = taskService.createTaskQuery().processInstanceId(pi.getId()).list();
        assertThat(tasks).hasSize(
            4); // 3 user tasks in MI  +1 (preNormalEnd) = 4 (2 were killed because it went directly to the terminate end event)
    ...
    public void testTerminateInCallActivityConcurrentCallActivity() throws Exception {
        ...
        ProcessInstance pi = runtimeService.startProcessInstanceByKey(
            "terminateEndEventInCallActivityConcurrentCallActivity");
        assertThat(runtimeService.createProcessInstanceQuery().superProcessInstanceId(pi.getId())
            .list()).hasSize(2);
    ...
    @Deployment
    public void testTerminateInSubProcessSequentialConcurrentMultiInstanceTerminateAll()
        throws Exception {
        ProcessInstance pi = runtimeService.startProcessInstanceByKey("terminateEndEventExample");
        assertProcessEnded(pi.getId());
        assertHistoricProcessInstanceDetails(pi);
    ...
    public void testTerminateNestedSubprocesses() {
        ProcessInstance processInstance = runtimeService.startProcessInstanceByKey(
            "TestTerminateNestedSubprocesses");
    ...
    public void testTerminateNestedMiSubprocesses() {
        ProcessInstance processInstance = runtimeService.startProcessInstanceByKey(
            "TestTerminateNestedMiSubprocesses");
    ...
    public void testTerminateNestedMiSubprocessesSequential() {
        ...
        assertThat(
            taskService.createTaskQuery().processInstanceId(processInstance.getId()).taskName("I")
        ...
        assertThat(
            taskService.createTaskQuery().processInstanceId(processInstance.getId()).taskName("H")
                .singleResult()).isNotNull();
    }
    ...
    public void testNestedCallActivities() {
        ProcessInstance processInstance = runtimeService.startProcessInstanceByKey(
            "TestNestedCallActivities");

        // Verify the tasks
        List<Task> tasks = assertTaskNames(processInstance,
            asList("B", "B", "B", "B", "Before A", "Before A", "Before A", "Before A", "Before B",
                "Before C"));

        // Completing 'before c'
        taskService.complete(tasks.get(9).getId());
        tasks = assertTaskNames(processInstance,
            asList("After C", "B", "B", "B", "B", "Before A", "Before A", "Before A", "Before A",
                "Before B"));
        ...
        tasks = assertTaskNames(processInstance,
            asList("After A", "After C", "B", "B", "B", "B", "Before A", "Before A", "Before A",
                "Before B"));
    ...
    private List<Task> assertTaskNames(ProcessInstance processInstance, List<String> taskNames) {
        ...
        for (int i = 0; i < taskNames.size(); i++) {
            assertThat(tasks.get(i).getName()).as("Task name at index " + i + " does not match")
                .isEqualTo(taskNames.get(i));
        }
        return tasks;
    }
    ...
    public void testParseTerminateEndEventDefinitionWithExtensions() {
        org.activiti.engine.repository.Deployment deployment = repositoryService.createDeployment()
            .addClasspathResource(
                "org/activiti/engine/test/bpmn/event/end/TerminateEndEventTest.parseExtensionElements.bpmn20.xml")
            .deploy();
    ...
    public void testThreeExecutionsArrivingInTerminateEndEvent() {
        ...
        ProcessInstance processInstance = runtimeService.startProcessInstanceByKey(
            "skybox_image_pull_request", variableMap);
    ...
    protected void assertHistoricProcessInstanceDeleteReason(ProcessInstance processInstance,
        ...
        if (processEngineConfiguration.getHistoryLevel().isAtLeast(HistoryLevel.AUDIT)) {
            ...
            if (expectedDeleteReason == null) {
                ...
                assertThat(historicProcessInstance.getDeleteReason()
            ...
            for (String activityId : activityIds) {
                ...
                for (HistoricActivityInstance historicActiviyInstance : historicActiviyInstances) {
                    ...
                    if (expectedDeleteReason == null) {
                        ...
                        assertThat(historicActiviyInstance.getDeleteReason()
                            .startsWith(expectedDeleteReason)).isTrue();
                    }
                }
            }
