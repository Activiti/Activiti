...
public class TerminateEndEventTest extends PluggableActivitiTestCase {
  ...
  public void testTerminateInExclusiveGatewayWithMultiInstanceSubProcessTerminateAll()
    ...
    for (int i = 0; i < 5; i++) {
      ...
      assertThat(
              runtimeService.createProcessInstanceQuery().processInstanceId(pi.getId()).count() > 0)
          .isTrue();
    }
  ...
  public void testTerminateInSubProcessConcurrentMultiInstance() throws Exception {
    ...
    assertThat(tasks)
        .hasSize(
            4); // 3 user tasks in MI  +1 (preNormalEnd) = 4 (2 were killed because it went directly
  ...
  public void testTerminateInSubProcessConcurrentMultiInstance2() throws Exception {
    ...
    List<Task> tasks =
        taskService.createTaskQuery().processInstanceId(pi.getId()).taskName("User Task").list();
  ...
  @Deployment
  public void
      testTerminateNestedMiSubprocessesTerminateAll3() { // Same as 1, but sequential Multi-Instance
    ProcessInstance processInstance =
        runtimeService.startProcessInstanceByKey("TestTerminateNestedMiSubprocesses");
    Task task =
        taskService
  ...
  @Deployment
  public void
      testTerminateNestedMiSubprocessesTerminateAll4() { // Same as 2, but sequential Multi-Instance
    ProcessInstance processInstance =
        runtimeService.startProcessInstanceByKey("TestTerminateNestedMiSubprocesses");
    taskService.complete(taskService.createTaskQuery().taskName("A").singleResult().getId());
    Task task =
  ...
  // Unit test for ACT-4101 : NPE when there are multiple routes to terminateEndEvent, and both are
  // reached
  @Deployment
