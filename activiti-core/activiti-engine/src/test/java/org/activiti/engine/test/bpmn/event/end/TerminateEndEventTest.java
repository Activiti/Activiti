...
public class TerminateEndEventTest extends PluggableActivitiTestCase {
    ...
    public void testTerminateWithSubProcessWithUserTaskWithBoundaryEvent() throws Exception {
        ...
        HistoricActivityInstance subprocess11 = historyService
    ...
    public void testTerminateNestedMiSubprocessesSequential() {
        ...
        assertThat(
            taskService.createTaskQuery().processInstanceId(processInstance.getId()).taskName("I").singleResult()
        ).isNotNull();

        // Should have 3 instances of E active after each other
        for (int i = 0; i < 3; i++) {
            assertThat(
                taskService.createTaskQuery().processInstanceId(processInstance.getId()).taskName("D").count()
            ).isEqualTo(1);
            assertThat(
                taskService.createTaskQuery().processInstanceId(processInstance.getId()).taskName("F").count()
            ).isEqualTo(1);
        ...
        assertThat(
            taskService.createTaskQuery().processInstanceId(processInstance.getId()).taskName("H").singleResult()
        ).isNotNull();
    }
    ...
    @Deployment
    public void testTerminateNestedMiSubprocessesTerminateAll3() {
        // Same as 1, but sequential Multi-Instance
        ProcessInstance processInstance = runtimeService.startProcessInstanceByKey("TestTerminateNestedMiSubprocesses");
        Task task = taskService
            .createTaskQuery()
            .processInstanceId(processInstance.getId())
    ...
    @Deployment
    public void testTerminateNestedMiSubprocessesTerminateAll4() {
        // Same as 2, but sequential Multi-Instance
        ProcessInstance processInstance = runtimeService.startProcessInstanceByKey("TestTerminateNestedMiSubprocesses");
        taskService.complete(taskService.createTaskQuery().taskName("A").singleResult().getId());
        Task task = taskService
            .createTaskQuery()
