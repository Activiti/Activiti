...
public class TerminateEndEventTest extends PluggableActivitiTestCase {
    ...
    public void testTerminateInSubProcessWithBoundary() throws Exception {
        ...
        // complete outerTask
        task =
            taskService.createTaskQuery().processInstanceId(pi.getId()).taskDefinitionKey("outerTask").singleResult();
        taskService.complete(task.getId());
    ...
    public void testTerminateNestedMiSubprocessesSequential() {
        ...
        for (int i = 0; i < 3; i++) {
            assertThat(taskService.createTaskQuery().processInstanceId(processInstance.getId()).taskName("D").count())
                .isEqualTo(1);
            assertThat(taskService.createTaskQuery().processInstanceId(processInstance.getId()).taskName("F").count())
                .isEqualTo(1);
