...
public class HistoricTaskAndVariablesQueryTest extends PluggableActivitiTestCase {
    ...
    public void testQuery() {
        if (processEngineConfiguration.getHistoryLevel().isAtLeast(HistoryLevel.ACTIVITY)) {
            HistoricTaskInstance task = historyService.createHistoricTaskInstanceQuery()
                .includeTaskLocalVariables().taskAssignee(GONZO).singleResult();
            ...
            runtimeService.startProcessInstanceByKey("oneTaskProcess",
                singletonMap("processVar", true));

            task = historyService.createHistoricTaskInstanceQuery().includeProcessVariables()
                .taskAssignee(KERMIT).singleResult();
            ...
            tasks = historyService.createHistoricTaskInstanceQuery().includeTaskLocalVariables()
                .taskInvolvedUser(KERMIT).orderByTaskCreateTime().asc().list();
            assertThat(tasks).hasSize(3);
            ...
            task = historyService.createHistoricTaskInstanceQuery().includeTaskLocalVariables()
                .taskAssignee(KERMIT).taskVariableValueEquals("localVar",
                    "test").singleResult();
            assertThat(task.getProcessVariables()).hasSize(0);
            assertThat(task.getTaskLocalVariables()).hasSize(1);
            assertThat(task.getTaskLocalVariables().get("localVar")).isEqualTo("test");
            ...
            task = historyService.createHistoricTaskInstanceQuery().includeTaskLocalVariables()
                .includeProcessVariables().taskAssignee(KERMIT).singleResult();
            assertThat(task.getProcessVariables()).hasSize(2);
            assertThat(task.getTaskLocalVariables()).hasSize(1);
            assertThat(task.getTaskLocalVariables().get("localVar")).isEqualTo("test");
            assertThat(task.getProcessVariables().get("processVar")).isEqualTo(true);
            ...
            task = historyService.createHistoricTaskInstanceQuery().taskAssignee(GONZO)
                .singleResult();
    ...
    public void testOrQuery() {
        if (processEngineConfiguration.getHistoryLevel().isAtLeast(HistoryLevel.ACTIVITY)) {
            ...
            task = historyService.createHistoricTaskInstanceQuery().includeProcessVariables().or()
                .taskAssignee(GONZO).taskVariableValueEquals("localVar",
            ...
            task = historyService.createHistoricTaskInstanceQuery().includeTaskLocalVariables().or()
                .taskAssignee(KERMIT).taskVariableValueEquals("localVar",
                    "nonExisting").endOr().singleResult();
            assertThat(task.getProcessVariables()).hasSize(0);
            assertThat(task.getTaskLocalVariables()).hasSize(1);
            ...
            task = historyService.createHistoricTaskInstanceQuery().includeTaskLocalVariables()
                .taskAssignee(KERMIT).or().taskVariableValueEquals("localVar",
                    "test")
                .taskVariableValueEquals("localVar",
                    "nonExisting").endOr().singleResult();
            assertThat(task.getProcessVariables()).hasSize(0);
            ...
            task = historyService.createHistoricTaskInstanceQuery().includeTaskLocalVariables()
                .includeProcessVariables().or().taskAssignee(KERMIT)
                .taskVariableValueEquals("localVar",
                    "nonExisting")
                .endOr().singleResult();
            assertThat(task.getProcessVariables()).hasSize(2);
            ...
            taskService.complete(task.getId());
            task = historyService.createHistoricTaskInstanceQuery().includeTaskLocalVariables().or()
                .finished().taskVariableValueEquals("localVar",
                    "nonExisting").endOr().singleResult();
            variableMap = task.getTaskLocalVariables();
            assertThat(variableMap).hasSize(2);
            assertThat(task.getProcessVariables()).hasSize(0);
    ...
    public void testOrQueryMultipleVariableValues() {
        if (processEngineConfiguration.getHistoryLevel().isAtLeast(HistoryLevel.ACTIVITY)) {
            ...
            HistoricTaskInstanceQuery query0 = historyService.createHistoricTaskInstanceQuery()
            ...
            HistoricTaskInstanceQuery query1 = historyService.createHistoricTaskInstanceQuery()
                .includeProcessVariables().or().processVariableValueEquals("anotherProcessVar",
                    123);
            for (int i = 0; i < 20; i++) {
    ...
    public void testCandidate() {
        if (processEngineConfiguration.getHistoryLevel().isAtLeast(HistoryLevel.ACTIVITY)) {
            ...
            tasks = historyService.createHistoricTaskInstanceQuery().taskCandidateGroupIn(groups)
                .list();
            assertThat(tasks).hasSize(1);
    ...
    public void testCandidateWithUserGroupProxy() {
        ...
        if (processEngineConfiguration.getHistoryLevel().isAtLeast(HistoryLevel.ACTIVITY)) {
            ...
            tasks = historyService.createHistoricTaskInstanceQuery().taskCandidateUser(KERMIT)
                .taskCandidateGroupIn(asList("management")).list();
            assertThat(tasks).hasSize(3);
            ...
            tasks = historyService.createHistoricTaskInstanceQuery().taskCandidateUser(GONZO)
                .taskCandidateGroupIn(asList("invalid")).list();
            assertThat(tasks).hasSize(0);
        }
    }
    ...
    public void testQueryWithPagingAndVariables() {
        if (processEngineConfiguration.getHistoryLevel().isAtLeast(HistoryLevel.AUDIT)) {
            List<HistoricTaskInstance> tasks = historyService.createHistoricTaskInstanceQuery()
                .includeProcessVariables().includeTaskLocalVariables().orderByTaskPriority().desc()
                .listPage(0,
                    1);
            assertThat(tasks).hasSize(1);
            ...
            tasks = historyService.createHistoricTaskInstanceQuery().includeProcessVariables()
                .includeTaskLocalVariables().orderByTaskPriority().asc().listPage(1,
                    2);
            ...
            tasks = historyService.createHistoricTaskInstanceQuery().includeProcessVariables()
                .includeTaskLocalVariables().orderByTaskPriority().asc().listPage(2,
                    4);
            assertThat(tasks).hasSize(1);
            task = tasks.get(0);
            variableMap = task.getTaskLocalVariables();
            ...
            tasks = historyService.createHistoricTaskInstanceQuery().includeProcessVariables()
                .includeTaskLocalVariables().orderByTaskPriority().asc().listPage(4,
                    2);
            assertThat(tasks).hasSize(0);
        }
    }
    ...
    public void testWithoutDueDateQuery() throws Exception {
        if (processEngineConfiguration.getHistoryLevel().isAtLeast(HistoryLevel.AUDIT)) {
            ...
            HistoricTaskInstance historicTask = historyService.createHistoricTaskInstanceQuery()
                .processInstanceId(processInstance.getId()).withoutTaskDueDate().singleResult();
            ...
            assertThat(historyService.createHistoricTaskInstanceQuery()
                .processInstanceId(processInstance.getId()).withoutTaskDueDate().count()).isEqualTo(
                0);
    ...
    public void testQueryWithIncludeTaskVariableAndTaskCategory() {
        ...
        tasks = historyService.createHistoricTaskInstanceQuery().taskAssignee(GONZO)
