...
public class DuplicateVariableInsertTest extends PluggableActivitiTestCase {
            ...
            public void run() {
                try {
                    managementService.executeCommand(
                        new SetVariableWithBarriersCommand(startBarrier, endBarrier,
    ...
    public void testDuplicateVariableInsertOnTask() throws Exception {
        ...
        Thread firstInsertThread = new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    managementService.executeCommand(
                        new SetTaskVariableWithBarriersCommand(startBarrier, endBarrier,
                            task.getId()));
                } catch (Exception e) {
                    exceptions.add(e);
                }
    ...
    private class SetVariableWithBarriersCommand implements Command<Void> {
        ...
        public SetVariableWithBarriersCommand(CyclicBarrier startBarrier, CyclicBarrier endBarrier,
            String executionId) {
        ...
        public Void execute(CommandContext commandContext) {
            ...
            new SetExecutionVariablesCmd(executionId, singletonMap("var", "12345"), false).execute(
                commandContext);
    ...
    private class SetTaskVariableWithBarriersCommand implements Command<Void> {
        ...
        public SetTaskVariableWithBarriersCommand(CyclicBarrier startBarrier,
            CyclicBarrier endBarrier, String taskId) {
            this.startBarrier = startBarrier;
            this.endBarrier = endBarrier;
            this.taskId = taskId;
        }
        ...
        public Void execute(CommandContext commandContext) {
            ...
            new SetTaskVariablesCmd(taskId, singletonMap("var", "12345"), false).execute(
                commandContext);
