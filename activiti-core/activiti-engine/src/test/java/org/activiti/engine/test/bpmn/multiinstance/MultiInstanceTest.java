...
public class MultiInstanceTest extends PluggableActivitiTestCase {
    ...
    private void checkAndCompleteTask(String expectedAssignee, int expectedLoopCounter,
        int nrOfLoops, String elementIndexVariable, Execution outerInstance) {
    ...
    private void checkOuterInstanceVariables(Execution outerInstance, int loopCounter,
        int nrOfLoops, String elementIndexVariable) {
    ...
    private void checkInnerInstanceVariables(Task task, int loopCounter,
        String elementIndexVariable) {
        Map<String, Object> localVariables = runtimeService.getVariablesLocal(
            task.getExecutionId());
        // these variables should be available only in the outer instance: see BPMN specification table 10.30, page 194
        assertThat(localVariables).doesNotContainKeys(NR_OF_INSTANCES_KEY,
            NR_OF_ACTIVE_INSTANCES_KEY, NR_OF_COMPLETED_INSTANCES_KEY);
    ...
    public void testParallelUserTasksExecutionAndTaskListeners() {
        ...
        assertThat(
            runtimeService.getVariable(processInstance.getId(), "taskListenerCounter")).isEqualTo(
            3);
        assertThat(runtimeService.getVariable(processInstance.getId(),
            "executionListenerCounter")).isEqualTo(3);
    ...
    public void testParallelScriptTasksCompletionCondition() {
        ...
        Execution waitStateExecution = runtimeService.createExecutionQuery().activityId("waitState")
            .singleResult();
        assertThat(waitStateExecution).isNotNull();
        int sum = (Integer) runtimeService.getVariable(processInstance.getId(), "sum");
    ...
    public void testParallelSubProcessCompletionCondition() {
        ...
        List<Task> subProcessTasks1 = taskService.createTaskQuery()
            .taskDefinitionKey("subProcessTask1").list();
        assertThat(subProcessTasks1).hasSize(2);

        List<Task> subProcessTasks2 = taskService.createTaskQuery()
            .taskDefinitionKey("subProcessTask2").list();
    ...
    public void testParallelSubProcessAllAutomatic() {
        String procId = runtimeService.startProcessInstanceByKey(
            "miParallelSubprocessAllAutomatics", singletonMap(NR_OF_LOOPS_KEY, 5)).getId();
    ...
    public void testParallelSubProcessAllAutomaticCompletionCondition() {
        String procId = runtimeService.startProcessInstanceByKey(
            "miParallelSubprocessAllAutomatics", singletonMap(NR_OF_LOOPS_KEY, 10)).getId();
    ...
    public void testSequentialCallActivityWithList() {
        ...
        Task task1 = taskService.createTaskQuery().processVariableValueEquals("element", "one")
            .singleResult();
        Task task2 = taskService.createTaskQuery().processVariableValueEquals("element", "two")
            .singleResult();
        ...
        Task task3 = taskService.createTaskQuery().processDefinitionKey("midProcess")
            .singleResult();
        assertThat(task3).isNotNull();
        taskService.complete(task3.getId(), null);

        Task task4 = taskService.createTaskQuery().processDefinitionKey("parentProcess")
            .singleResult();
    ...
    public void testSequentialCallActivityWithTimer() {
        String procId = runtimeService.startProcessInstanceByKey(
            "miSequentialCallActivityWithTimer").getId();
    ...
    public void testMultiInstanceParallelReceiveTask() {
        ...
        List<Execution> executions = runtimeService.createExecutionQuery()
            .activityId("theReceiveTask").list();
    ...
    public void testMultiInstanceSequentialReceiveTask() {
        runtimeService.startProcessInstanceByKey("multi-instance-receive");
        Execution execution = runtimeService.createExecutionQuery().activityId("theReceiveTask")
            .singleResult();
        ...
        while (execution != null) {
            runtimeService.trigger(execution.getId());
            execution = runtimeService.createExecutionQuery().activityId("theReceiveTask")
                .singleResult();
        }
    ...
    public void testEmptyCollectionOnSequentialEmbeddedSubprocess() {
        if (processEngineConfiguration.getHistoryLevel().isAtLeast(HistoryLevel.AUDIT)) {
            ...
            assertThat(
                historyService.createHistoricProcessInstanceQuery().finished().count()).isEqualTo(
                1L);
        }
    }
