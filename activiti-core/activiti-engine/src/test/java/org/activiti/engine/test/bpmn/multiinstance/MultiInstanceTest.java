...
public class MultiInstanceTest extends PluggableActivitiTestCase {
    ...
    String procId =
        runtimeService
            .startProcessInstanceByKey(
                processDefinitionKey, singletonMap(NR_OF_LOOPS_KEY, nrOfLoops))
            .getId();
  ...
  private void checkAndCompleteTask(
    ...
    checkOuterInstanceVariables(
        outerInstance, expectedLoopCounter, nrOfLoops, elementIndexVariable);

    taskService.complete(task.getId());
  }

  private void checkOuterInstanceVariables(
      Execution outerInstance, int loopCounter, int nrOfLoops, String elementIndexVariable) {
    Map<String, Object> localVariables = runtimeService.getVariablesLocal(outerInstance.getId());
    // this variable should be available only in the inner instance: see BPMN specification table
    // 10.30, page 194
    assertThat(localVariables).doesNotContainKey(elementIndexVariable);

    assertThat(localVariables)
        .containsKeys(
            NR_OF_INSTANCES_KEY, NR_OF_ACTIVE_INSTANCES_KEY, NR_OF_COMPLETED_INSTANCES_KEY);
  ...
  private void checkInnerInstanceVariables(
      Task task, int loopCounter, String elementIndexVariable) {
    Map<String, Object> localVariables = runtimeService.getVariablesLocal(task.getExecutionId());
    // these variables should be available only in the outer instance: see BPMN specification table
    // 10.30, page 194
    assertThat(localVariables)
        .doesNotContainKeys(
            NR_OF_INSTANCES_KEY, NR_OF_ACTIVE_INSTANCES_KEY, NR_OF_COMPLETED_INSTANCES_KEY);
  ...
  public void testSequentialUserTasksHistory() {
    String procId =
        runtimeService
            .startProcessInstanceByKey("miSequentialUserTasks", singletonMap(NR_OF_LOOPS_KEY, 4))
            .getId();
    for (int i = 0; i < 4; i++) {
    ...
    if (processEngineConfiguration.getHistoryLevel().isAtLeast(HistoryLevel.ACTIVITY)) {
      ...
      List<HistoricActivityInstance> historicActivityInstances =
          historyService.createHistoricActivityInstanceQuery().activityType("userTask").list();
  ...
  public void testSequentialUserTasksWithTimer() {
    String procId =
        runtimeService
            .startProcessInstanceByKey("miSequentialUserTasks", singletonMap(NR_OF_LOOPS_KEY, 3))
            .getId();
  ...
  public void testSequentialUserTasksCompletionCondition() {
    String procId =
        runtimeService
            .startProcessInstanceByKey("miSequentialUserTasks", singletonMap(NR_OF_LOOPS_KEY, 10))
            .getId();
  ...
  private void checkBuiltInOuterVariables(
      Execution outerExecution, int expetedActiveNumber, int expectedCompletedNumber) {
  ...
  public void testParallelUserTasksWithTimer() {
    String procId =
        runtimeService.startProcessInstanceByKey("miParallelUserTasksWithTimer").getId();
  ...
  public void testParallelUserTasksCompletionCondition() {
    String procId =
        runtimeService.startProcessInstanceByKey("miParallelUserTasksCompletionCondition").getId();
    List<Task> tasks = taskService.createTaskQuery().list();
  ...
  public void testParallelUserTasksBasedOnCollection() {
    ...
    String procId =
        runtimeService
            .startProcessInstanceByKey(
                "miParallelUserTasksBasedOnCollection", singletonMap("assigneeList", assigneeList))
            .getId();
  ...
  public void testParallelUserTasksExecutionAndTaskListeners() {
    ...
    Execution waitState =
        runtimeService.createExecutionQuery().activityId("waitState").singleResult();
    assertThat(waitState).isNotNull();

    assertThat(runtimeService.getVariable(processInstance.getId(), "taskListenerCounter"))
        .isEqualTo(3);
    assertThat(runtimeService.getVariable(processInstance.getId(), "executionListenerCounter"))
        .isEqualTo(3);
  ...
  public void testParallelScriptTasksHistory() {
    ...
    if (processEngineConfiguration.getHistoryLevel().isAtLeast(HistoryLevel.ACTIVITY)) {
      List<HistoricActivityInstance> historicActivityInstances =
          historyService.createHistoricActivityInstanceQuery().activityType("scriptTask").list();
      assertThat(historicActivityInstances).hasSize(4);
      for (HistoricActivityInstance hai : historicActivityInstances) {
        assertThat(hai.getStartTime()).isNotNull();
        assertThat(hai.getEndTime()).isNotNull();
  ...
  public void testSequentialSubProcessHistory() {
    ...
    // Validate history
    if (processEngineConfiguration.getHistoryLevel().isAtLeast(HistoryLevel.ACTIVITY)) {
      List<HistoricActivityInstance> onlySubProcessInstances =
          historyService.createHistoricActivityInstanceQuery().activityType("subProcess").list();
  ...
  public void testSequentialSubProcessWithTimer() {
    String procId =
        runtimeService.startProcessInstanceByKey("miSequentialSubprocessWithTimer").getId();
  ...
  public void testSequentialSubProcessCompletionCondition() {
    String procId =
        runtimeService
            .startProcessInstanceByKey("miSequentialSubprocessCompletionCondition")
            .getId();
  ...
  public void testNestedSequentialSubProcess() {
    String procId =
        runtimeService.startProcessInstanceByKey("miNestedSequentialSubProcess").getId();
  ...
  public void testNestedSequentialSubProcessWithTimer() {
    String procId =
        runtimeService.startProcessInstanceByKey("miNestedSequentialSubProcessWithTimer").getId();
  ...
  public void testParallelSubProcessHistory() {
    ...
    if (processEngineConfiguration.getHistoryLevel().isAtLeast(HistoryLevel.ACTIVITY)) {
      List<HistoricActivityInstance> historicActivityInstances =
          historyService.createHistoricActivityInstanceQuery().activityId("miSubProcess").list();
      assertThat(historicActivityInstances).hasSize(2);
      for (HistoricActivityInstance hai : historicActivityInstances) {
        assertThat(hai.getStartTime()).isNotNull();
        assertThat(hai.getEndTime()).isNotNull();
  ...
  public void testParallelSubProcessWithTimer() {
    String procId =
        runtimeService.startProcessInstanceByKey("miParallelSubprocessWithTimer").getId();
    List<Task> tasks = taskService.createTaskQuery().list();
  ...
  public void testParallelSubProcessCompletionCondition() {
    String procId =
        runtimeService.startProcessInstanceByKey("miParallelSubprocessCompletionCondition").getId();
    List<Task> tasks = taskService.createTaskQuery().list();
    assertThat(tasks).hasSize(4);

    List<Task> subProcessTasks1 =
        taskService.createTaskQuery().taskDefinitionKey("subProcessTask1").list();
    assertThat(subProcessTasks1).hasSize(2);

    List<Task> subProcessTasks2 =
        taskService.createTaskQuery().taskDefinitionKey("subProcessTask2").list();
  ...
  public void testParallelSubProcessAllAutomatic() {
    String procId =
        runtimeService
            .startProcessInstanceByKey(
                "miParallelSubprocessAllAutomatics", singletonMap(NR_OF_LOOPS_KEY, 5))
            .getId();

    for (int i = 0; i < 5; i++) {
      List<Execution> waitSubExecutions =
          runtimeService.createExecutionQuery().activityId("subProcessWait").list();
  ...
  public void testParallelSubProcessAllAutomaticCompletionCondition() {
    String procId =
        runtimeService
            .startProcessInstanceByKey(
                "miParallelSubprocessAllAutomatics", singletonMap(NR_OF_LOOPS_KEY, 10))
            .getId();
  ...
  @Deployment(
      resources =
          "org/activiti/engine/test/bpmn/multiinstance/MultiInstanceTest.testSequentialCallActivityWithList.bpmn20.xml")
  public void testSequentialCallActivityWithList() {
    ...
    Task task1 =
        taskService.createTaskQuery().processVariableValueEquals("element", "one").singleResult();
    Task task2 =
        taskService.createTaskQuery().processVariableValueEquals("element", "two").singleResult();
  ...
  public void testSequentialCallActivityWithTimer() {
    String procId =
        runtimeService.startProcessInstanceByKey("miSequentialCallActivityWithTimer").getId();
  ...
  public void testParallelCallActivityHistory() {
    ...
    if (processEngineConfiguration.getHistoryLevel().isAtLeast(HistoryLevel.ACTIVITY)) {
      ...
      List<HistoricActivityInstance> historicActivityInstances =
          historyService.createHistoricActivityInstanceQuery().activityType("callActivity").list();
  ...
  public void testNestedSequentialCallActivity() {
    String procId =
        runtimeService.startProcessInstanceByKey("miNestedSequentialCallActivity").getId();
  ...
  public void testNestedSequentialCallActivityWithTimer() {
    String procId =
        runtimeService.startProcessInstanceByKey("miNestedSequentialCallActivityWithTimer").getId();
  ...
  public void testNestedParallelCallActivity() {
    String procId =
        runtimeService.startProcessInstanceByKey("miNestedParallelCallActivity").getId();
  ...
  public void testNestedParallelCallActivityWithTimer() {
    String procId =
        runtimeService.startProcessInstanceByKey("miNestedParallelCallActivityWithTimer").getId();
  ...
  public void testNestedParallelCallActivityCompletionCondition() {
    String procId =
        runtimeService
            .startProcessInstanceByKey("miNestedParallelCallActivityCompletionCondition")
            .getId();

    assertThat(taskService.createTaskQuery().count()).isEqualTo(8);

    for (int i = 0; i < 2; i++) {
      ProcessInstance nextSubProcessInstance =
          runtimeService
              ...
              .get(0);
      List<Task> tasks =
          taskService.createTaskQuery().processInstanceId(nextSubProcessInstance.getId()).list();
      assertThat(tasks).hasSize(2);
      for (Task task : tasks) {
        taskService.complete(task.getId());
      }
  ...
  public void testSequentialServiceTaskWithClass() {
    ProcessInstance procInst =
        runtimeService.startProcessInstanceByKey(
            "multiInstanceServiceTask", singletonMap("result", 5));
  ...
  public void testSequentialServiceTaskWithClassAndCollection() {
    ...
    ProcessInstance procInst =
        runtimeService.startProcessInstanceByKey("multiInstanceServiceTask", vars);
    Integer result = (Integer) runtimeService.getVariable(procInst.getId(), "result");
  ...
  public void testMultiInstanceCallActivityWithErrorBoundaryEvent() {
    ...
    List<ProcessInstance> processInstances =
        runtimeService.createProcessInstanceQuery().processDefinitionKey("process").list();
  ...
  public void testMultiInstanceParallelReceiveTask() {
    ...
    List<Execution> executions =
        runtimeService.createExecutionQuery().activityId("theReceiveTask").list();
  ...
  public void testMultiInstanceSequentialReceiveTask() {
    runtimeService.startProcessInstanceByKey("multi-instance-receive");
    Execution execution =
        runtimeService.createExecutionQuery().activityId("theReceiveTask").singleResult();
  ...
  public void testNestedMultiInstanceTasks() {
    ...
    ProcessInstance processInstance =
        runtimeService.startProcessInstanceByKey("miNestedMultiInstanceTasks", variableMap);
  ...
  public void testSequentialEmptyCollection() {
    ...
    variableMap.put("collection", collection);
    ProcessInstance processInstance =
        runtimeService.startProcessInstanceByKey("testSequentialEmptyCollection", variableMap);
    assertThat(processInstance).isNotNull();
    Task task = taskService.createTaskQuery().singleResult();
    assertThat(task).isNull();
    assertProcessEnded(processInstance.getId());
  ...
  public void testParalellEmptyCollection() throws Exception {
    ...
    variableMap.put("collection", collection);
    ProcessInstance processInstance =
        runtimeService.startProcessInstanceByKey("testParalellEmptyCollection", variableMap);
    assertThat(processInstance).isNotNull();
    Task task = taskService.createTaskQuery().singleResult();
    assertThat(task).isNull();
    assertProcessEnded(processInstance.getId());
  ...
  public void testInfiniteLoopWithDelegateExpressionFix() {
    ...
    Map<Object, Object> originalBeans =
        processEngineConfiguration.getExpressionManager().getBeans();
    ...
    try {
      ...
      ProcessInstance processInstance =
          runtimeService.startProcessInstanceByKey("infiniteLoopTest", params);
      assertThat(processInstance).isNotNull();
  ...
  public void testZeroLoopCardinalityOnParallelUserTask() {
    if (processEngineConfiguration.getHistoryLevel().isAtLeast(HistoryLevel.AUDIT)) {
      ProcessInstance processInstance =
          runtimeService.startProcessInstanceByKey("parallelUserTaskMi");
      assertThat(
              historyService
                  .createHistoricProcessInstanceQuery()
                  .processInstanceId(processInstance.getId())
  ...
  public void testExecutionListenersOnMultiInstanceUserTask() {
    resetTestCounts();
    ProcessInstance processInstance =
        runtimeService.startProcessInstanceByKey("testExecutionListenersOnMultiInstanceUserTask");
  ...
  public void testEndTimeOnMiSubprocess() {
    ...
    ProcessInstance processInstance =
        runtimeService.startProcessInstanceByKey("multiInstanceSubProcessParallelTasks");
    ...
    List<HistoricActivityInstance> historicActivityInstances =
        historyService.createHistoricActivityInstanceQuery().activityId("subprocess1").list();
    assertThat(historicActivityInstances).hasSize(2);
    ...
    // Complete one of the user tasks. This should not trigger setting of end time of the
    // subprocess, but due to a bug it did exactly that
    taskService.complete(tasks.get(0).getId());
