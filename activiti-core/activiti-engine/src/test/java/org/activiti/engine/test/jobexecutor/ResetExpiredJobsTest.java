...
public class ResetExpiredJobsTest extends PluggableActivitiTestCase {
    ...
    public void testResetExpiredJobs() {
        ...
        List<JobEntity> expiredJobs = managementService.executeCommand(
            new FindExpiredJobsCmd(expiredJobsPagesSize));
        ...
        expiredJobs = managementService.executeCommand(
            new FindExpiredJobsCmd(expiredJobsPagesSize));
        assertThat(expiredJobs).hasSize(0);
        assertJobDetails(true);
        ...
        Date newDate = new Date(
            startOfTestTime.getTime() + processEngineConfiguration.getAsyncExecutor()
                .getAsyncJobLockTimeInMillis() + 10000);
