...
public class TaskInfoQueryTest extends PluggableActivitiTestCase {
    ...
    public void testTaskInfoQuery() {
        ...
        // Runtime
        TaskInfoQueryWrapper taskInfoQueryWrapper = new TaskInfoQueryWrapper(
        ...
        if (processEngineConfiguration.getHistoryLevel().isAtLeast(HistoryLevel.AUDIT)) {
            // History
            taskInfoQueryWrapper = new TaskInfoQueryWrapper(
                historyService.createHistoricTaskInstanceQuery());
            taskInfos = taskInfoQueryWrapper.getTaskInfoQuery().or().taskNameLike("%k1%")
                .taskDueAfter(new Date(now.getTime() + (3 * 24L * 60L * 60L * 1000L))).endOr()
                .list();
