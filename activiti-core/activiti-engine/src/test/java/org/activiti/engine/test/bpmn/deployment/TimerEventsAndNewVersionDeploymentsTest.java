...
public class TimerEventsAndNewVersionDeploymentsTest extends PluggableActivitiTestCase {
    ...
    public void testTimerRestoreOnDeploymentDelete1() {
        ...
        assertThat(job.getProcessDefinitionId()).isEqualTo(
            repositoryService.createProcessDefinitionQuery().deploymentId(deploymentId3)
                .singleResult().getId());
    ...
    public void testTimerRestoreOnDeploymentDelete2() {
        ...
        Job job = managementService.createTimerJobQuery().singleResult();
        assertThat(job.getProcessDefinitionId()).isEqualTo(
            repositoryService.createProcessDefinitionQuery().deploymentId(deploymentId1)
                .singleResult().getId());
