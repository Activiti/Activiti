...
/**
 * These are the first tests ever written for Activiti 6. Keeping them here for nostalgic reasons.
 */
public class Activiti6Test extends PluggableActivitiTestCase {
  ...
  public void testSimpleParallelGateway() {
    ProcessInstance processInstance =
        runtimeService.startProcessInstanceByKey("simpleParallelGateway");
    assertThat(processInstance).isNotNull();
    assertThat(processInstance.isEnded()).isFalse();
    ...
    List<Task> tasks =
        taskService
            .createTaskQuery()
            .processDefinitionKey("simpleParallelGateway")
            .orderByTaskName()
            .asc()
            .list();
  ...
  public void testLongServiceTaskLoop() {
    ...
    ProcessInstance processInstance =
        runtimeService.startProcessInstanceByKey("testLongServiceTaskLoop", vars);
    assertThat(processInstance).isNotNull();
    assertThat(processInstance.isEnded()).isTrue();
  ...
  public void testSimpleNonInterruptingTimerBoundaryEvent() {
    ...
    tasks =
        taskService
            ...
            .orderByTaskName()
            .desc()
            .list(); // Not the desc() here: Task B, Task A will be the result (task b being
    // associated with the child execution)
    for (Task task : tasks) {
      taskService.complete(task.getId());
    }
    assertThat(runtimeService.createExecutionQuery().count()).isEqualTo(0);
  ...
  public void testConditionsWithoutExclusiveGateway() {
    ...
    ProcessInstance processInstance =
        runtimeService.startProcessInstanceByKey("testConditions", singletonMap("input", 2));
    assertThat(processInstance).isNotNull();
    assertThat(processInstance.isEnded()).isFalse();
    ...
    processInstance =
        runtimeService.startProcessInstanceByKey("testConditions", singletonMap("input", 20));
    ...
    processInstance =
        runtimeService.startProcessInstanceByKey("testConditions", singletonMap("input", 200));
    task = taskService.createTaskQuery().singleResult();
    taskService.complete(task.getId());
  ...
  public void testNonInterruptingMoreComplex() {
    ProcessInstance processInstance =
        runtimeService.startProcessInstanceByKey("nonInterruptingTimer");
    assertThat(processInstance).isNotNull();
    assertThat(processInstance.isEnded()).isFalse();
  ...
  public void testNonInterruptingMoreComplex2() {
    ...
    ProcessInstance processInstance =
        runtimeService.startProcessInstanceByKey("nonInterruptingWithInclusiveMerge");
    assertThat(processInstance).isNotNull();
    assertThat(processInstance.isEnded()).isFalse();
  ...
  /**
   * Based on the process and use cases described in
   * http://www.bp-3.com/blogs/2013/09/joins-and-ibm-bpm-diving-deeper/
   */
  @Test
  @org.activiti.engine.test.Deployment(
      resources =
          "org/activiti/engine/test/api/v6/Activiti6Test.testInclusiveTrickyMerge.bpmn20.xml")
  public void testInclusiveTrickyMergeEasy() {

    // Use case 1 (easy):
    // "When C completes, depending on the data, we can immediately issue E no matter what the
    // status is of A or B."
    ProcessInstance processInstance =
