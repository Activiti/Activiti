...
public class ProcessInstanceEventsTest extends PluggableActivitiTestCase {
    ...
    public void testProcessInstanceEvents() throws Exception {
        ...
        assertThat(event.getType())
            .isEqualTo(ActivitiEventType.PROCESS_STARTED);
        assertThat(event.getProcessInstanceId())
            .isEqualTo(processInstance.getId());
        assertThat(event.getProcessDefinitionId())
            .isEqualTo(processInstance.getProcessDefinitionId());
    ...
    public void testSubProcessInstanceEvents() throws Exception {
        ...
        assertThat(event.getProcessInstanceId())
            .isEqualTo(processInstance.getId());
        assertThat(((ExecutionEntity) event.getEntity()).getId())
            .isNotEqualTo(processInstance.getId());
        ...
        assertThat(subProcessEntity.getSuperExecutionId())
            .isEqualTo(startChildExecutionId);
        ...
        assertThat(event.getProcessInstanceId())
            .isEqualTo(subProcessInstanceId);
        assertThat(event.getProcessDefinitionId())
            .isEqualTo(subProcessDefinitionId);
        assertThat(event).isInstanceOf(ActivitiProcessStartedEvent.class);
        assertThat(
            ((ActivitiProcessStartedEvent) event).getNestedProcessDefinitionId()
        )
    ...
    public void testProcessCompleted_NoEnd() throws Exception {
        ProcessInstance noEndProcess =
            this.runtimeService.startProcessInstanceByKey("noEndProcess");
        Task task = taskService
            .createTaskQuery()
            .processInstanceId(noEndProcess.getId())
            .singleResult();
    ...
    public void testProcessCompleted_ParallelGatewayTwoEnds() throws Exception {
        ...
        List<ActivitiEvent> events = listener.filterEvents(
    ...
    public void testProcessInstanceCancelledEvents_cancel() throws Exception {
        ...
        assertThat(
            ActivitiCancelledEvent.class.isAssignableFrom(
                    processCancelledEvent.getClass()
                )
        )
            .as(
                "The cause has to be the same as deleteProcessInstance method call"
            )
            .isTrue();
    ...
    public void testProcessInstanceCancelledEvents_cancelProcessHierarchy()
        throws Exception {
        ProcessInstance processInstance = runtimeService.startProcessInstanceByKey(
            "nestedSimpleSubProcess"
        );
        ProcessInstance subProcess = runtimeService
        ...
        processCancelledEvent =
            (ActivitiCancelledEvent) processCancelledEvents.get(1);
        assertThat(
            ActivitiCancelledEvent.class.isAssignableFrom(
                    processCancelledEvent.getClass()
                )
    ...
    public void testProcessInstanceCancelledEvents_complete() throws Exception {
        ...
        assertThat(taskCancelledEvents)
            .as(
                "There should be no ActivitiEventType.ACTIVITY_CANCELLED event."
            )
            .hasSize(0);
    }
    ...
    public void testProcessInstanceTerminatedEvents_complete()
        throws Exception {
        ProcessInstance processInstance = runtimeService.startProcessInstanceByKey(
            "oneTaskProcess"
        );
    ...
    @Deployment(
        resources = "org/activiti/engine/test/bpmn/event/end/TerminateEndEventTest.testProcessTerminate.bpmn"
    )
    public void testProcessInstanceTerminatedEvents() throws Exception {
        ProcessInstance pi = runtimeService.startProcessInstanceByKey(
            "terminateEndEventExample"
        );

        long executionEntities = runtimeService
        ...
        assertThat(activitiEvent.getProcessInstanceId())
        ...
        for (ActivitiEvent event : activityTerminatedEvents) {
            ...
            if (
                activityEvent.getActivityId().equals("preNormalTerminateTask")
            ) {
    ...
    public void testProcessInstanceTerminatedEvents_callActivity()
        throws Exception {
        ProcessInstance pi = runtimeService.startProcessInstanceByKey(
            "terminateEndEventExample"
        );
    ...
    public void testProcessInstanceTerminatedEvents_terminateInParentProcess()
        throws Exception {
        ProcessInstance pi = runtimeService.startProcessInstanceByKey(
        ...
        assertThat(processCancelledEvent.getProcessInstanceId())
            .isEqualTo(pi.getProcessInstanceId());
        assertThat(processCancelledEvent.getProcessDefinitionId())
            .contains("terminateParentProcess");

        List<ActivitiEvent> activityTerminatedEvents = listener.filterEvents(
            ActivitiEventType.ACTIVITY_CANCELLED
        );
        assertThat(activityTerminatedEvents)
            .as("3 activities must be cancelled.")
            .hasSize(3);
    ...
    public void testProcessCompletedEvents_callActivityErrorEndEvent()
        throws Exception {
        ProcessInstance pi = runtimeService.startProcessInstanceByKey(
        ...
        List<ProcessInstance> subProcesses = runtimeService
            .createProcessInstanceQuery()
        ...
        assertThat(processCompletedEvent.getExecutionId())
            .isEqualTo(subProcesses.get(0).getId());
    ...
    private class TestInitializedEntityEventListener
        ...
        public void onEvent(ActivitiEvent event) {
            if (
                ...
                ActivitiEventType.PROCESS_CANCELLED.equals(event.getType()) ||
