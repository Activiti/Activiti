...
public class ProcessInstanceEventsTest extends PluggableActivitiTestCase {
    ...
    public void testProcessInstanceEvents() throws Exception {
        ...
        assertThat(((ProcessInstance) event.getEntity()).getId()).isEqualTo(
        ...
        assertThat(event.getProcessDefinitionId()).isEqualTo(
        ...
        List<ActivitiEvent> processCancelledEvents = listener.filterEvents(
            ActivitiEventType.PROCESS_CANCELLED);
        assertThat(processCancelledEvents).hasSize(1);
        ActivitiCancelledEvent cancelledEvent = (ActivitiCancelledEvent) processCancelledEvents.get(
            0);
    ...
    public void testSubProcessInstanceEvents() throws Exception {
        ...
        assertThat(event.getType()).isEqualTo(ActivitiEventType.ENTITY_INITIALIZED);
        assertThat(event.getProcessInstanceId()).isEqualTo(processInstance.getId());
        assertThat(((ExecutionEntity) event.getEntity()).getId()).isNotEqualTo(
            processInstance.getId());
        ...
        ProcessDefinition subProcessDefinition = repositoryService.getProcessDefinition(
        ...
        assertThat(event).isInstanceOf(ActivitiProcessStartedEvent.class);
        assertThat(((ActivitiProcessStartedEvent) event).getNestedProcessDefinitionId()).isEqualTo(
            processDefinitionId);
        assertThat(((ActivitiProcessStartedEvent) event).getNestedProcessInstanceId()).isEqualTo(
            processInstance.getId());
    ...
    public void testProcessCompleted_StartEnd() throws Exception {
        ...
        assertThat(listener.filterEvents(ActivitiEventType.PROCESS_COMPLETED)).as(
            "ActivitiEventType.PROCESS_COMPLETED was expected 1 time.").hasSize(1);
    }
    ...
    public void testProcessCompleted_NoEnd() throws Exception {
        ProcessInstance noEndProcess = this.runtimeService.startProcessInstanceByKey(
            "noEndProcess");
        Task task = taskService.createTaskQuery().processInstanceId(noEndProcess.getId())
            .singleResult();
    ...
    public void testProcessCompleted_ParallelGatewayTwoEnds() throws Exception {
        ...
        assertThat(events).as("ActivitiEventType.PROCESS_COMPLETED was expected 1 time.")
            .hasSize(1);
    }
    ...
    public void testProcessInstanceCancelledEvents_cancel() throws Exception {
        ...
        List<ActivitiEvent> processCancelledEvents = listener.filterEvents(
            ActivitiEventType.PROCESS_CANCELLED);
        assertThat(processCancelledEvents).as(
            "ActivitiEventType.PROCESS_CANCELLED was expected 1 time.").hasSize(1);
        ActivitiCancelledEvent processCancelledEvent = (ActivitiCancelledEvent) processCancelledEvents.get(
            0);
        assertThat(
            ActivitiCancelledEvent.class.isAssignableFrom(processCancelledEvent.getClass())).as(
            "The cause has to be the same as deleteProcessInstance method call").isTrue();
        assertThat(processCancelledEvent.getProcessInstanceId()).as(
                "The process instance has to be the same as in deleteProcessInstance method call")
            .isEqualTo(processInstance.getId());
        assertThat(processCancelledEvent.getExecutionId()).as(
                "The execution instance has to be the same as in deleteProcessInstance method call")
            .isEqualTo(processInstance.getId());
        assertThat(processCancelledEvent.getCause()).as(
                "The cause has to be the same as in deleteProcessInstance method call")
            .isEqualTo("delete_test");

        List<ActivitiEvent> taskCancelledEvents = listener.filterEvents(
            ActivitiEventType.ACTIVITY_CANCELLED);
        assertThat(taskCancelledEvents).as(
            "ActivitiEventType.ACTIVITY_CANCELLED was expected 1 time.").hasSize(1);
        ActivitiActivityCancelledEvent activityCancelledEvent = (ActivitiActivityCancelledEvent) taskCancelledEvents.get(
            0);
        assertThat(ActivitiActivityCancelledEvent.class.isAssignableFrom(
            activityCancelledEvent.getClass())).as(
            "The cause has to be the same as deleteProcessInstance method call").isTrue();
        assertThat(activityCancelledEvent.getProcessInstanceId()).as(
                "The process instance has to be the same as in deleteProcessInstance method call")
            .isEqualTo(processInstance.getId());
        assertThat(activityCancelledEvent.getCause()).as(
                "The cause has to be the same as in deleteProcessInstance method call")
            .isEqualTo("delete_test");
    ...
    public void testProcessInstanceCancelledEvents_cancelProcessHierarchy() throws Exception {
        ...
        List<ActivitiEvent> processCancelledEvents = listener.filterEvents(
            ActivitiEventType.PROCESS_CANCELLED);
        assertThat(processCancelledEvents).as(
            "ActivitiEventType.PROCESS_CANCELLED was expected 2 times.").hasSize(2);
        ActivitiCancelledEvent processCancelledEvent = (ActivitiCancelledEvent) processCancelledEvents.get(
            0);
        assertThat(
            ActivitiCancelledEvent.class.isAssignableFrom(processCancelledEvent.getClass())).as(
        ...
        assertThat(this.taskService.createTaskQuery().processInstanceId(processInstance.getId())
            .count()).as("No task can be active for deleted process.").isEqualTo(0);
        ...
        assertThat(ActivitiActivityCancelledEvent.class.isAssignableFrom(
            ...
            "The cause has to be the same as deleteProcessInstance method call").isTrue();
        assertThat(activityCancelledEvent.getProcessInstanceId()).as(
            "The process instance has to point to the subprocess").isEqualTo(subProcess.getId());
        assertThat(activityCancelledEvent.getCause()).as(
                "The cause has to be the same as in deleteProcessInstance method call")
            .isEqualTo("delete_test");
    ...
    public void testProcessInstanceCancelledEvents_complete() throws Exception {
        ...
        List<ActivitiEvent> processCancelledEvents = listener.filterEvents(
            ActivitiEventType.PROCESS_CANCELLED);
        assertThat(processCancelledEvents).as(
                "There should be no ActivitiEventType.PROCESS_CANCELLED event after process complete.")
            .hasSize(0);
        List<ActivitiEvent> taskCancelledEvents = listener.filterEvents(
            ActivitiEventType.ACTIVITY_CANCELLED);
        assertThat(taskCancelledEvents).as(
            "There should be no ActivitiEventType.ACTIVITY_CANCELLED event.").hasSize(0);
    }
    ...
    public void testProcessInstanceTerminatedEvents_complete() throws Exception {
        ...
        List<ActivitiEvent> processTerminatedEvents = listener.filterEvents(
            ActivitiEventType.PROCESS_CANCELLED);
        assertThat(processTerminatedEvents).as(
                "There should be no ActivitiEventType.PROCESS_TERMINATED event after process complete.")
            .hasSize(0);
    }
    ...
    public void testProcessInstanceTerminatedEvents() throws Exception {
        ...
        long executionEntities = runtimeService.createExecutionQuery().processInstanceId(pi.getId())
            .count();
        assertThat(executionEntities).isEqualTo(3);

        Task task = taskService.createTaskQuery().processInstanceId(pi.getId())
            .taskDefinitionKey("preTerminateTask").singleResult();
        taskService.complete(task.getId());

        List<ActivitiEvent> processTerminatedEvents = listener.filterEvents(
            ActivitiEventType.PROCESS_CANCELLED);
        assertThat(processTerminatedEvents).as(
                "There should be exactly one ActivitiEventType.PROCESS_CANCELLED event after the task complete.")
            .hasSize(1);
        ActivitiProcessCancelledEventImpl activitiEvent = (ActivitiProcessCancelledEventImpl) processTerminatedEvents.get(
            0);
        assertThat(activitiEvent.getProcessInstanceId()).isEqualTo(pi.getProcessInstanceId());

        List<ActivitiEvent> activityTerminatedEvents = listener.filterEvents(
            ActivitiEventType.ACTIVITY_CANCELLED);
        assertThat(activityTerminatedEvents).as(
                "There should be exactly two ActivitiEventType.ACTIVITY_CANCELLED event after the task complete.")
            .hasSize(2);
        ...
        for (ActivitiEvent event : activityTerminatedEvents) {
            ...
            if (activityEvent.getActivityId().equals("preNormalTerminateTask")) {
                assertThat(activityEvent.getActivityId()).as("The user task must be terminated")
                    .isEqualTo("preNormalTerminateTask");
                assertThat(activityEvent.getCause()).as("The cause must be terminate end event")
                    .isEqualTo("Terminated by end event: EndEvent_2");
            } else if (activityEvent.getActivityId().equals("EndEvent_2")) {
                assertThat(activityEvent.getActivityId()).as("The end event must be terminated")
    ...
    public void testProcessInstanceTerminatedEvents_callActivity() throws Exception {
        ...
        Task task = taskService.createTaskQuery().processInstanceId(pi.getId())
            .taskDefinitionKey("preNormalEnd").singleResult();
        taskService.complete(task.getId());
        ...
        assertThat(processTerminatedEvents).as(
                "There should be exactly one ActivitiEventType.PROCESS_CANCELLED event after the task complete.")
            .hasSize(1);
        ActivitiProcessCancelledEventImpl processCancelledEvent = (ActivitiProcessCancelledEventImpl) processTerminatedEvents.get(
            0);
        assertThat(processCancelledEvent.getProcessInstanceId()).isNotEqualTo(
            pi.getProcessInstanceId());
        assertThat(processCancelledEvent.getProcessDefinitionId()).contains(
            "terminateEndEventSubprocessExample");
    ...
    public void testProcessInstanceTerminatedEvents_terminateInParentProcess() throws Exception {
        ...
        List<ActivitiEvent> processTerminatedEvents = listener.filterEvents(
            ActivitiEventType.PROCESS_CANCELLED);
        assertThat(processTerminatedEvents).as(
                "There should be exactly one ActivitiEventType.PROCESS_TERMINATED event after the task complete.")
            .hasSize(1);
        ActivitiProcessCancelledEventImpl processCancelledEvent = (ActivitiProcessCancelledEventImpl) processTerminatedEvents.get(
            0);
        assertThat(processCancelledEvent.getProcessInstanceId()).isEqualTo(
            pi.getProcessInstanceId());
        assertThat(processCancelledEvent.getProcessDefinitionId()).contains(
        ...
        for (ActivitiEvent event : activityTerminatedEvents) {
            ...
            if (activityEvent.getActivityId().equals("theTask")) {
                ...
                assertThat(activityEvent.getActivityId()).as(
                        "The user task must be terminated in the called sub process.")
                    .isEqualTo("theTask");
                assertThat(activityEvent.getCause()).as("The cause must be terminate end event")
                ...
                assertThat(activityEvent.getActivityId()).as("The call activity must be terminated")
    ...
    public void testProcessCompletedEvents_callActivityErrorEndEvent() throws Exception {
        ...
        List<ProcessInstance> subProcesses = runtimeService.createProcessInstanceQuery()
            .superProcessInstanceId(pi.getId()).list();
        ...
        List<ActivitiEvent> processCompletedEvents = listener.filterEvents(
            ActivitiEventType.PROCESS_COMPLETED_WITH_ERROR_END_EVENT);
        assertThat(processCompletedEvents).as(
                "There should be exactly an ActivitiEventType.PROCESS_COMPLETED_WITH_ERROR_END_EVENT event after the task complete.")
            .hasSize(1);
        ActivitiEntityEvent processCompletedEvent = (ActivitiEntityEvent) processCompletedEvents.get(
            0);
    ...
    public void testDeleteMultiInstanceCallActivityProcessInstance() {
        ...
        assertThat(this.listener.getEventsReceived().get(0).getType()).as(
                "Task cancelled event has to be fired.")
            .isEqualTo(ActivitiEventType.ACTIVITY_CANCELLED);
        assertThat(this.listener.getEventsReceived().get(2).getType()).as(
                "SubProcess cancelled event has to be fired.")
    ...
    private class TestInitializedEntityEventListener implements ActivitiEventListener {
        ...
        public void onEvent(ActivitiEvent event) {
            if (event instanceof ActivitiEntityEvent && ProcessInstance.class.isAssignableFrom(
                ((ActivitiEntityEvent) event).getEntity().getClass())) {
                // check whether entity in the event is initialized before adding to the list.
                assertThat(
                    ((ExecutionEntity) ((ActivitiEntityEvent) event).getEntity()).getId()).isNotNull();
