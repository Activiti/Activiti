...
public class ProcessInstanceEventsTest extends PluggableActivitiTestCase {
    ...
    public void testProcessInstanceEvents() throws Exception {
        ...
        ActivitiCancelledEvent cancelledEvent = (ActivitiCancelledEvent) processCancelledEvents.getFirst();
     ...
     */
    @Deployment(
        resources = { "org/activiti/engine/test/api/event/ProcessInstanceEventsTest.noneTaskProcess.bpmn20.xml" }
    )
    ...
    public void testProcessInstanceCancelledEvents_cancel() throws Exception {
        ...
        ActivitiCancelledEvent processCancelledEvent = (ActivitiCancelledEvent) processCancelledEvents.getFirst();
        ...
        ActivitiActivityCancelledEvent activityCancelledEvent =
            (ActivitiActivityCancelledEvent) taskCancelledEvents.getFirst();
        assertThat(ActivitiActivityCancelledEvent.class.isAssignableFrom(activityCancelledEvent.getClass()))
            .as("The cause has to be the same as deleteProcessInstance method call")
            .isTrue();
    ...
    public void testProcessInstanceCancelledEvents_cancelProcessHierarchy() throws Exception {
        ...
        assertThat(this.taskService.createTaskQuery().processInstanceId(processInstance.getId()).count())
    ...
    public void testProcessInstanceTerminatedEvents() throws Exception {
        ...
        List<ActivitiEvent> processTerminatedEvents = listener.filterEvents(ActivitiEventType.PROCESS_CANCELLED);
        assertThat(processTerminatedEvents)
            .as("There should be exactly one ActivitiEventType.PROCESS_CANCELLED event after the task complete.")
            .hasSize(1);
        ActivitiProcessCancelledEventImpl activitiEvent =
            (ActivitiProcessCancelledEventImpl) processTerminatedEvents.getFirst();
        ...
        assertThat(activityTerminatedEvents)
            .as("There should be exactly two ActivitiEventType.ACTIVITY_CANCELLED event after the task complete.")
            .hasSize(2);
    ...
    public void testProcessInstanceTerminatedEvents_terminateInParentProcess() throws Exception {
        ...
        List<ActivitiEvent> processTerminatedEvents = listener.filterEvents(ActivitiEventType.PROCESS_CANCELLED);
        assertThat(processTerminatedEvents)
            .as("There should be exactly one ActivitiEventType.PROCESS_TERMINATED event after the task complete.")
            .hasSize(1);
        ActivitiProcessCancelledEventImpl processCancelledEvent =
            (ActivitiProcessCancelledEventImpl) processTerminatedEvents.getFirst();
    ...
    public void testProcessCompletedEvents_callActivityErrorEndEvent() throws Exception {
        ...
        assertThat(processCompletedEvents)
            ...
            .hasSize(1);
        ActivitiEntityEvent processCompletedEvent = (ActivitiEntityEvent) processCompletedEvents.getFirst();
        assertThat(processCompletedEvent.getExecutionId()).isEqualTo(subProcesses.getFirst().getId());
    ...
    public void testDeleteMultiInstanceCallActivityProcessInstance() {
        ...
        assertThat(this.listener.getEventsReceived().getFirst().getType())
    ...
    private class TestInitializedEntityEventListener implements ActivitiEventListener {
        ...
        public List<ActivitiEvent> filterEvents(ActivitiEventType eventType) {
