...
public class SequentialMultiInstanceBehavior extends MultiInstanceActivityBehavior {
    ...
    /**
     * Handles the sequential case of spawning the instances. Will only create one instance, since
     * at most one instance can be active.
     */
    protected int createInstances(DelegateExecution multiInstanceExecution) {
        ...
        if (nrOfInstances == 0) {
            ...
            throw new ActivitiIllegalArgumentException(
                "Invalid number of instances: must be a non-negative integer value" + ", but was "
                    + nrOfInstances);
        }
     ...
     * AbstractBpmnActivityBehavior#leave(DelegateExecution)} method. Handles the completion of one
     * instance, and executes the logic for the sequential behavior.
     */
    public void leave(DelegateExecution childExecution) {
        ...
        int nrOfCompletedInstances =
            getLoopVariable(multiInstanceRootExecution, NUMBER_OF_COMPLETED_INSTANCES) + 1;
        int nrOfActiveInstances = getLoopVariable(multiInstanceRootExecution,
            NUMBER_OF_ACTIVE_INSTANCES);

        setLoopVariable(multiInstanceRootExecution, NUMBER_OF_COMPLETED_INSTANCES,
            nrOfCompletedInstances);
        setLoopVariable(childExecution, getCollectionElementIndexVariable(), loopCounter);
        logLoopDetails(childExecution, "instance completed", loopCounter, nrOfCompletedInstances,
        ...
        if (loopCounter >= nrOfInstances || completionConditionSatisfied(
            ...
            try {
                ...
                if (childExecution.getCurrentFlowElement() instanceof SubProcess) {
                    ...
                    setLoopVariable(executionToContinue, getCollectionElementIndexVariable(),
                        loopCounter);
                ...
                throw new ActivitiException(
                    "Could not execute inner activity behavior of multi instance behavior", e);
            }
        }
    }
}
