...
public class ProcessExecutionLogger {
    ...
    public synchronized void addDebugInfo(AbstractDebugInfo debugInfo,
            ...
            if (!deletedExecutions.containsKey(executionEntity.getId())) {
                if (executionEntity.getParentId() == null) {
                    ...
                    if (!parentMapping.containsKey(executionEntity.getParentId())) {
                        parentMapping.put(executionEntity.getParentId(),
    ...
    protected void internalPopulateExecutionTree(DebugInfoExecutionTreeNode parentNode,
        ...
        if (parentMapping.containsKey(parentNode.getId())) {
            for (ExecutionEntity childExecutionEntity : parentMapping.get(parentNode.getId())) {
                ...
                childNode.setActivityId(childExecutionEntity.getCurrentFlowElement() != null
                    ? childExecutionEntity.getCurrentFlowElement().getId() : null);
                childNode.setActivityName(childExecutionEntity.getCurrentFlowElement() != null
                    ? childExecutionEntity.getCurrentFlowElement().getName() : null);
