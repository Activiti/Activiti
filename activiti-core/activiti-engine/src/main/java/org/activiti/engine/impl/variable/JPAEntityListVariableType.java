 ...
 * configured by annotations are supported. Use of compound primary keys is not supported. <br> The
 * variable value should be of type {@link List} and can only contain objects of the same type.
 */
    ...
    public boolean isAbleToStore(Object value) {
        ...
        if (value instanceof List<?>) {
            ...
            if (list.size() > 0) {
                ...
                canStore = true;
                Class<?> entityClass = mappings.getEntityMetaData(list.get(0).getClass())
                    .getEntityClass();

                for (Object entity : list) {
                    canStore = entity != null && mappings.isJPAEntity(entity)
                        && mappings.getEntityMetaData(entity.getClass()).getEntityClass()
                        .equals(entityClass);
                    if (!canStore) {
