...
public class SignalEventHandler extends AbstractEventHandler {
    ...
    public void handleEvent(
        ...
        if (eventSubscription.getExecutionId() != null) {
            ...
            FlowElement flowElement =
                    process.getFlowElement(eventSubscription.getActivityId(), true);
            if (flowElement == null) {
                throw new ActivitiException(
            ...
            ExecutionEntity executionEntity =
                    processInstanceHelper.createProcessInstanceWithInitialFlowElement(
                            processDefinition, null, null, flowElement, process);
            DelegateExecution execution = executionEntity.getExecutions().get(0);
            dispatchActivitySignalledEvent(
                    execution, eventSubscription.getEventName(), payload, commandContext);

            processInstanceHelper.startProcessInstance(
                    executionEntity, commandContext, variables, flowElement, null);
        } else {
            throw new ActivitiException(
    ...
    protected void dispatchActivitySignalledEvent(
        ...
        if (commandContext.getProcessEngineConfiguration().getEventDispatcher().isEnabled()) {
            ...
            ActivitiSignalEvent signalEvent =
                    ActivitiEventBuilder.createActivitiySignalledEvent(
                            execution, signalName, payload);
