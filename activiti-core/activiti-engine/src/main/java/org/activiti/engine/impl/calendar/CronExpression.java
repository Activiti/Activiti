...
public class CronExpression implements Serializable, Cloneable {
     ...
     *     represent
     * @param clockReader The reader which will provide the current time
     * @param timeZone The time zone that will be used for calculations
     * @throws java.text.ParseException if the string expression cannot be parsed into a valid
     *     <CODE>
     *     CronExpression</CODE>
     */
    public CronExpression(String cronExpression, ClockReader clockReader, TimeZone timeZone)
        ...
        try {
            ...
            while (exprsTok.hasMoreTokens() && exprOn <= YEAR) {
                ...
                if (exprOn == DAY_OF_WEEK
                        ...
                        && expr.indexOf(",") >= 0) {
                    throw new ParseException(
                            "Support for specifying 'L' with other days of the week is not"
                                    + " implemented",
                            -1);
                }
                if (exprOn == DAY_OF_WEEK
    ...
    protected int checkNext(int pos, String s, int val, int type) throws ParseException {
        ...
        if (c == 'W') {
            ...
            if (val > 31)
                throw new ParseException(
                        "The 'W' option does not make sense with values larger than 31 (max number"
                                + " of days in a month)",
                        i);
            TreeSet<Integer> set = getSet(type);
            set.add(Integer.valueOf(val));
