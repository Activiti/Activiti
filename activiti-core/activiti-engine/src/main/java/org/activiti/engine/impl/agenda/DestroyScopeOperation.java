...
/**
 * Destroys a scope (for example a subprocess): this means that all child executions, tasks, jobs,
 * variables, etc within that scope are deleted.
 *
 * <p>The typical example is an interrupting boundary event that is on the boundary of a subprocess
 * and is triggered. At that point, everything within the subprocess would need to be destroyed.
 */
public class DestroyScopeOperation extends AbstractOperation {
  ...
  public void run() {
    ...
    ExecutionEntity scopeExecution =
        execution.isScope() ? execution : findFirstParentScopeExecution(execution);
    ...
    commandContext
        .getHistoryManager()
        .recordActivityEnd(scopeExecution, scopeExecution.getDeleteReason());
    executionEntityManager.delete(scopeExecution);
  }

  private void removeAllVariablesFromScope(
      ExecutionEntity scopeExecution, VariableInstanceEntityManager variableInstanceEntityManager) {
    Collection<VariableInstanceEntity> variablesForExecution =
        variableInstanceEntityManager.findVariableInstancesByExecutionId(scopeExecution.getId());
  ...
  private void deleteAllScopeJobs(
      ExecutionEntity scopeExecution, TimerJobEntityManager timerJobEntityManager) {
    Collection<TimerJobEntity> timerJobsForExecution =
        timerJobEntityManager.findJobsByExecutionId(scopeExecution.getId());
    ...
    Collection<JobEntity> jobsForExecution =
        jobEntityManager.findJobsByExecutionId(scopeExecution.getId());
    ...
    Collection<SuspendedJobEntity> suspendedJobsForExecution =
        suspendedJobEntityManager.findJobsByExecutionId(scopeExecution.getId());
    ...
    Collection<DeadLetterJobEntity> deadLetterJobsForExecution =
        deadLetterJobEntityManager.findJobsByExecutionId(scopeExecution.getId());
  ...
  private void deleteAllScopeTasks(
      ExecutionEntity scopeExecution, TaskEntityManager taskEntityManager) {
    Collection<TaskEntity> tasksForExecution =
        taskEntityManager.findTasksByExecutionId(scopeExecution.getId());
  ...
  private ExecutionEntityManager deleteAllChildExecutions(
      ExecutionEntityManager executionEntityManager, ExecutionEntity scopeExecution) {
    // Delete all child executions
    Collection<ExecutionEntity> childExecutions =
        executionEntityManager.findChildExecutionsByParentExecutionId(scopeExecution.getId());
    for (ExecutionEntity childExecution : childExecutions) {
      executionEntityManager.deleteExecutionAndRelatedData(
          childExecution, execution.getDeleteReason());
    }
