...
public class TerminateEndEventActivityBehavior extends FlowNodeActivityBehavior {
  ...
  protected void defaultTerminateEndEventBehaviour(
    ...
    // In case of a multi-instance subprocess, only one instance is terminated, the other instances
    // continue to exist.
    ...
    if (scopeExecutionEntity.isProcessInstanceType()
      ...
      commandContext
          .getHistoryManager()
          .recordProcessInstanceEnd(
              scopeExecutionEntity.getId(), deleteReason, execution.getCurrentActivityId());
      ...
      if (subProcess.hasMultiInstanceLoopCharacteristics()) {
        ...
        ExecutionEntity outgoingFlowExecution =
            executionEntityManager.createChildExecution(scopeExecutionEntity.getParent());
  ...
  protected void terminateMultiInstanceRoot(
    ...
    // When terminateMultiInstance is 'true', we look for the multi instance root and delete it from
    // there.
