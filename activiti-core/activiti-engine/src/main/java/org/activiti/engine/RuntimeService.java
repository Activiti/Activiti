...
public interface RuntimeService {
   ...
   * @throws ActivitiObjectNotFoundException when user does not have permission to start the process
   *     instance
   ...
   * {@link ProcessInstanceQuery#processInstanceBusinessKey(String)}. Providing such a business key
  ...
  /**
   * Similar to {@link #startProcessInstanceByKey(String, String, Map)}, but using a specific tenant
   * identifier.
   */
  ProcessInstance startProcessInstanceByKeyAndTenantId(
   ...
   *       started.
   *   <li>If no subscription to a message with the given name exists, {@link ActivitiException} is
   *       thrown
   * </ul>
   *
   * @param messageName the 'name' of the message as specified as an attribute on the bpmn20 {@code
   *     <message name="messageName" />} element.
  ...
  /**
   * Signals the process engine that a message is received and starts a new {@link ProcessInstance}.
   * See {@link #startProcessInstanceByMessage(String, Map)}. This method allows specifying a
   * business key.
   *
   * @param messageName the 'name' of the message as specified as an attribute on the bpmn20 {@code
   *     <message name="messageName" />} element.
  ...
  /**
   * Signals the process engine that a message is received and starts a new {@link ProcessInstance}.
   * See {@link #startProcessInstanceByMessage(String)}. In addition, this method allows specifying
   * a the payload of the message as a map of process variables.
   *
   * @param messageName the 'name' of the message as specified as an attribute on the bpmn20 {@code
   *     <message name="messageName" />} element.
  ...
  /**
   * Retrieves the {@link IdentityLink}s associated with the given process instance. Such an {@link
   * IdentityLink} informs how a certain user is involved with a process instance.
   */
   ...
   * @return the variable value or null if the variable is undefined or the value of the variable is
   *     null.
   * @throws ActivitiObjectNotFoundException when no execution is found for the given executionId.
   */
   ...
   * execution (and not searching parent scopes). Returns null when no variable value is found with
   * the given name or when the value is set to null.
   */
  ...
  /**
   * The variable value for an execution. Returns the value casted to given class when the variable
   * is set for the execution (and not searching parent scopes). Returns null when no variable value
   * is found with the given name or when the value is set to null.
   */
   ...
   * account. If you have many local DataObjects and you only need a few, consider using {@link
   * #getDataObjectsLocal(String, Collection)} for better performance.
   *
   * @param executionId id of execution, cannot be null.
   * @return the DataObjects or an empty map if no DataObjects are found.
   * @throws ActivitiObjectNotFoundException when no execution is found for the given executionId.
   ...
   * given execution (including parent scopes). Returns null when no DataObject value is found with
   * the given name or when the value is set to null.
   *
   * @param executionId id of execution, cannot be null.
  ...
  DataObject getDataObject(
      String executionId, String dataObjectName, String locale, boolean withLocalizationFallback);
   ...
   *
   * <p>If you have a process instance hierarchy, suspending one process instance form the hierarchy
   * will not suspend other process instances form that hierarchy.
   *
  ...
  /**
   * Similar to {@link #signalEventReceived(String, Map<String, Object>)}, but within the context of
   * one tenant.
   */
  ...
  /**
   * Notifies the process engine that a message event with name 'messageName' has been received and
   * has been correlated to an execution with id 'executionId'.
   *
  ...
  /**
   * Notifies the process engine that a message event with the name 'messageName' has been received
   ...
   * <li>The scope for a boundary message event is that of the execution for the Activity the event
   *     is attached to
   *
   *     <p>Variables are set according to the algorithm as documented for {@link
   *     VariableScope#setVariables(Map)}, applied separately to each variable.
   ...
   * @throws ActivitiException if an exception occurs when dispatching the event or when the {@link
   *     ActivitiEventDispatcher} is disabled.
