...
public class UserTaskActivityBehavior extends TaskActivityBehavior {
    ...
    public void execute(DelegateExecution execution) {
        ...
        if (StringUtils.isNotEmpty(activeTaskDueDate)) {
            Object dueDate =
                    expressionManager.createExpression(activeTaskDueDate).getValue(execution);
            if (dueDate != null) {
        ...
        if (StringUtils.isNotEmpty(activeTaskSkipExpression)) {
            Expression skipExpression =
                    expressionManager.createExpression(activeTaskSkipExpression);
            skipUserTask =
    ...
    protected Map<String, Object> calculateOutBoundVariables(
            DelegateExecution execution, Map<String, Object> taskVariables) {
        CommandContext commandContext = Context.getCommandContext();
        if (commandContext.getProcessEngineConfiguration().isCopyVariablesToLocalForTasks()) {
    ...
    private void propagateVariablesToProcess(
            DelegateExecution execution, CommandContext commandContext) {
        ...
        if (processInstanceEntity != null) {
            ...
            Map<String, Object> outboundVariables =
                    calculateOutBoundVariables(execution, taskVariables);
    ...
    protected void handleAssignments(
        ...
        if (userTask.getCustomUserIdentityLinks() != null
            ...
            for (String customUserIdentityLinkType :
                    ...
                    if (value instanceof String) {
                        ...
                        while (userIdSet.hasNext()) {
                            task.addUserIdentityLink(
                                    (String) userIdSet.next(), customUserIdentityLinkType);
                        }
                    } else {
                        throw new ActivitiException(
                                "Expression did not resolve to a string or collection of strings");
        ...
        if (userTask.getCustomGroupIdentityLinks() != null
            ...
            for (String customGroupIdentityLinkType :
                    ...
                    if (value instanceof String) {
                        ...
                        while (groupIdSet.hasNext()) {
                            task.addGroupIdentityLink(
                                    (String) groupIdSet.next(), customGroupIdentityLinkType);
                        }
                    } else {
                        throw new ActivitiException(
                                "Expression did not resolve to a string or collection of strings");
