...
public interface ExecutionQuery extends Query<ExecutionQuery, Execution> {
  ...
  /**
   * Only select executions which have the given process definition version. Particulary useful when
   * used in combination with {@link #processDefinitionKey(String)}
   */
   ...
   *
   * <p>Note that only process instances have a business key and as such, child executions will NOT
  ...
  /**
   * Only select executions which have at least one local variable with the given value. The type of
   * variable is determined based on the value, using types configured in {@link
   * ProcessEngineConfiguration#getVariableTypes()} . Byte-arrays and {@link Serializable} objects
   * (which are not primitive type wrappers) are not supported.
   */
   ...
   * to the given value. Byte-arrays and {@link Serializable} objects (which are not primitive type
   * wrappers) are not supported.
   */
   ...
   *
   * <p>(The messageName is specified using the 'name' attribute of the message element in the BPMN
   * 2.0 XML.)
   *
   ...
   *
   * @param userId the user id of the authenticated user that started the execution (cannot be null)
   */
