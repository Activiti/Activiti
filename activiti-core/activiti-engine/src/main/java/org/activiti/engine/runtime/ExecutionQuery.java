...
public interface ExecutionQuery extends Query<ExecutionQuery, Execution> {
    ...
    /**
     * Only select executions which have the given process definition version. Particulary useful
     * when used in combination with {@link #processDefinitionKey(String)}
     */
     ...
     *
     * <p>Note that only process instances have a business key and as such, child executions will
     * NOT be returned. If you want to return child executions of the process instance with the
     * given business key too, use the {@link #processInstanceBusinessKey(String, boolean)} method
     * with a boolean value of <i>true</i> instead.
     */
    ExecutionQuery processInstanceBusinessKey(String processInstanceBusinessKey);

    /**
     * Only executions with the given business key. Similar to {@link
     * #processInstanceBusinessKey(String)}, but allows to choose whether child executions are
     * returned or not.
     */
     ...
     * is determined based on the value, using types configured in {@link
     * ProcessEngineConfiguration#getVariableTypes()}. Byte-arrays and {@link Serializable} objects
    ...
    /**
     * Only select executions which have a local string variable with the given value, case
     * insensitive.
     *
     ...
     * of variable is determined based on the value, using types configured in {@link
     * ProcessEngineConfiguration#getVariableTypes()} . Byte-arrays and {@link Serializable} objects
     * (which are not primitive type wrappers) are not supported.
     */
    ...
    /**
     * Only select executions which have a local string variable which is not the given value, case
     * insensitive.
     *
     * <p>This method only works if your database has encoding/collation that supports
     * case-sensitive queries. For example, use "collate UTF-8" on MySQL and for MSSQL, select one
     ...
     * Booleans, Byte-arrays and {@link Serializable} objects (which are not primitive type
    ...
    /**
     * Only select executions which have a local variable value like the given value. This be used
     * on string variables only.
     *
     * @param name variable name, cannot be null.
     * @param value variable value, cannot be null. The string can include the wildcard character
     *     '%' to express like-strategy: starts with (string%), ends with (%string) or contains
     *     (%string%).
     */
    ExecutionQuery variableValueLike(String name, String value);

    /**
     * Only select executions which have a local variable value like the given value (case
     * insensitive). This be used on string variables only.
     *
     * @param name variable name, cannot be null.
     * @param value variable value, cannot be null. The string can include the wildcard character
     *     '%' to express like-strategy: starts with (string%), ends with (%string) or contains
     ...
     * set to the given value. Byte-arrays and {@link Serializable} objects (which are not primitive
     * type wrappers) are not supported.
     */
    ExecutionQuery processVariableValueEquals(String variableName, Object variableValue);

    /**
     * Only select executions which are part of a process that have at least one variable with the
     * given value. Byte-arrays and {@link Serializable} objects (which are not primitive type
     * wrappers) are not supported.
     */
    ...
    /**
     * Only select executions which are part of a process that have a variable with the given name,
     * but with a different value than the passed value. Byte-arrays and {@link Serializable}
    ...
    /**
     * Only select executions which are part of a process that have a local string variable with the
    ...
    /**
     * Only select executions which are part of a process that have a local string variable which is
     * not the given value, case insensitive.
     *
     * <p>This method only works if your database has encoding/collation that supports
     * case-sensitive queries. For example, use "collate UTF-8" on MySQL and for MSSQL, select one
     * of the case-sensitive Collations available (<a
    ...
    /**
     * Only select executions which are part of a process that have at least one variable like the
     * given value. Byte-arrays and {@link Serializable} objects (which are not primitive type
     * wrappers) are not supported.
     */
    ...
    /**
     * Only select executions which are part of a process that have at least one variable like the
     * given value (case insensitive). Byte-arrays and {@link Serializable} objects (which are not
     * primitive type wrappers) are not supported.
     */
     ...
     *
     * <p>(The signalName is specified using the 'name' attribute of the signal element in the BPMN
     * 2.0 XML.)
     *
     ...
     *
     * <p>(The messageName is specified using the 'name' attribute of the message element in the
     * BPMN 2.0 XML.)
     *
     ...
     *
     * @param beforeTime executions started before this time will be returned (cannot be null)
     */
     ...
     *
     * @param afterTime executions started after this time will be returned (cannot be null)
     */
     ...
     *
     * @param userId the user id of the authenticated user that started the execution (cannot be
     *     null)
     */
