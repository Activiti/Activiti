...
public class ActivitiEventDispatcherImpl implements ActivitiEventDispatcher {
     ...
     * <p>
     * If not an entity-related event, the process-definition will be retrieved based on the
    ...
    protected BpmnModel extractBpmnModelFromEvent(ActivitiEvent event) {
        ...
        if (result == null && event.getProcessDefinitionId() != null) {
            ProcessDefinition processDefinition = ProcessDefinitionUtil.getProcessDefinition(
                event.getProcessDefinitionId(), true);
            if (processDefinition != null) {
                result = Context.getProcessEngineConfiguration().getDeploymentManager()
