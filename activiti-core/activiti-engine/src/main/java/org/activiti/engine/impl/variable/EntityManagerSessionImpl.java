...
public class EntityManagerSessionImpl implements EntityManagerSession {
    ...
    public EntityManagerSessionImpl(
        ...
        if (entityManager != null && (!handleTransactions || isTransactionActive())) {
            try {
                ...
                throw new ActivitiException(
                        "Error while flushing EntityManager, illegal state", ise);
            } catch (TransactionRequiredException tre) {
                throw new ActivitiException(
                        "Cannot flush EntityManager, an active transaction is required", tre);
            } catch (PersistenceException pe) {
                throw new ActivitiException(
                        "Error while flushing EntityManager: " + pe.getMessage(), pe);
            }
        }
    }
    ...
    public void close() {
        if (closeEntityManager && entityManager != null && !entityManager.isOpen()) {
            try {
                ...
                throw new ActivitiException(
                        "Error while closing EntityManager, may have already been closed or it is"
                                + " container-managed",
                        ise);
            }
        }
        ...
        if (entityManager == null) {
            ...
            if (handleTransactions) {
                ...
                transactionContext.addTransactionListener(
                        TransactionState.COMMITTED, jpaTransactionCommitListener);
                transactionContext.addTransactionListener(
                        TransactionState.ROLLED_BACK, jpaTransactionRollbackListener);
