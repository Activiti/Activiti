...
public class IntermediateCatchEventActivityBehavior extends AbstractBpmnActivityBehavior {
    ...
    /**
     * Specific leave method for intermediate events: does a normal leave(), except when behind an
     * event based gateway. In that case, the other events are cancelled (we're only supporting the
     * exclusive event based gateway type currently). and the process instance is continued through
     * the triggered event.
     */
    ...
    /**
     * Should be subclassed by the more specific types. For an intermediate catch without type, it's
     * simply leaving the event.
     */
    ...
    protected EventGateway getPrecedingEventBasedGateway(DelegateExecution execution) {
        ...
        if (currentFlowElement instanceof IntermediateCatchEvent) {
            IntermediateCatchEvent intermediateCatchEvent =
                    (IntermediateCatchEvent) currentFlowElement;
            List<SequenceFlow> incomingSequenceFlow = intermediateCatchEvent.getIncomingFlows();

            // If behind an event based gateway, there is only one incoming sequence flow that
            // originates
            // from said gateway
    ...
    protected void deleteOtherEventsRelatedToEventBasedGateway(
            DelegateExecution execution, EventGateway eventGateway) {
        ...
        // (for example a parallel gw going twice to the event based gateway, kinda silly, but
        // valid)
        // so we only take _one_ result of such a query for deletion.

        // Gather all activity ids for the events after the event based gateway that need to be
        // destroyed
        List<SequenceFlow> outgoingSequenceFlows = eventGateway.getOutgoingFlows();
        Set<String> eventActivityIds =
                new HashSet<String>(
                        outgoingSequenceFlows.size()
                                - 1); // -1, the event being triggered does not need to be deleted
        ...
        List<ExecutionEntity> executionEntities =
                executionEntityManager.findExecutionsByParentExecutionAndActivityIds(
                        execution.getParentId(), eventActivityIds);

        // Execute the cancel behaviour of the IntermediateCatchEvent
        for (ExecutionEntity executionEntity : executionEntities) {
            if (eventActivityIds.contains(executionEntity.getActivityId())
                    && execution.getCurrentFlowElement() instanceof IntermediateCatchEvent) {
                IntermediateCatchEvent intermediateCatchEvent =
                        (IntermediateCatchEvent) execution.getCurrentFlowElement();
                if (intermediateCatchEvent.getBehavior()
                        instanceof IntermediateCatchEventActivityBehavior) {
                    ((IntermediateCatchEventActivityBehavior) intermediateCatchEvent.getBehavior())
                            .eventCancelledByEventGateway(executionEntity);
                    eventActivityIds.remove(
                            executionEntity
                                    .getActivityId()); // We only need to delete ONE execution at
                    // the event.
                }
            }
        }
