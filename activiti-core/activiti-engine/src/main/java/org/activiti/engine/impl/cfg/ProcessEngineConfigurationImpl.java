...
public abstract class ProcessEngineConfigurationImpl extends ProcessEngineConfiguration {
     ...
     * <p>
     * By default null. If null, an {@link ArrayBlockingQueue} will be created of size {@link
     ...
     * <p>
     * Default value = 1, as this lowers the potential on optimistic locking exceptions. Change this
     ...
     * <p>
     * It is important that each async executor instance in a cluster of Activiti engines has a
     * different name!
     * <p>
     * (This property is only applicable when using the {@link DefaultAsyncJobExecutor}).
     */
     ...
     * <p>
     * During such a check, jobs that are expired are again made available, meaning the lock owner
     ...
     * <p>
     * And the changes to the java object will be reflected in the database. Otherwise, a manual
     ...
     * <p>
     * Unfortunately, this feature is not available on some platforms (JDK 6, JBoss), hence the
     ...
     * process instances).
     * <p>
     * The default setting is quite low, as not to surprise anyone with sudden memory spikes. Change
     * it to something higher if the environment Activiti runs in allows it.
     */
     ...
     * more information.
     * <p>
     * Set this flag to false to throw an exception at runtime when a field is injected and a
     ...
     * <p>
     * By default: 100 (75 for mssql server as it has a hard limit of 2000 parameters in a
     * statement)
     */
     ...
     * <p>
     * If true (default), the {@link ProcessEngineConfiguration#getDatabaseSchemaUpdate()} value
     * will be used to determine what needs to happen wrt the database schema.
     * <p>
     * If false, no validation or schema creation will be done. That means that the database schema
     * must have been created 'manually' before but the engine does not validate whether the schema
     * is correct. The {@link ProcessEngineConfiguration#getDatabaseSchemaUpdate()} value will not
     * be used.
     */
        ...
        if (dataSource == null) {
            if (dataSourceJndiName != null) {
                try {
                    ...
                    throw new ActivitiException(
                        "couldn't lookup datasource from " + dataSourceJndiName + ": "
                            + e.getMessage(), e);
                }
    ...
    public Configuration initMybatisConfiguration(Environment environment, Reader reader,
    ...
    public Configuration parseMybatisConfiguration(Configuration configuration,
    ...
    public void initDataManagers() {
        ...
        if (historicActivityInstanceDataManager == null) {
            historicActivityInstanceDataManager = new MybatisHistoricActivityInstanceDataManager(
                this);
        }
        ...
        if (historicProcessInstanceDataManager == null) {
            historicProcessInstanceDataManager = new MybatisHistoricProcessInstanceDataManager(
                this);
        }
    ...
    public void initEntityManagers() {
        ...
        if (eventLogEntryEntityManager == null) {
            eventLogEntryEntityManager = new EventLogEntryEntityManagerImpl(this,
                eventLogEntryDataManager);
        }
        if (eventSubscriptionEntityManager == null) {
            eventSubscriptionEntityManager = new EventSubscriptionEntityManagerImpl(this,
        ...
        if (historicDetailEntityManager == null) {
            historicDetailEntityManager = new HistoricDetailEntityManagerImpl(this,
                historicDetailDataManager);
        }
        if (historicIdentityLinkEntityManager == null) {
            historicIdentityLinkEntityManager = new HistoricIdentityLinkEntityManagerImpl(this,
        ...
        if (historicTaskInstanceEntityManager == null) {
            historicTaskInstanceEntityManager = new HistoricTaskInstanceEntityManagerImpl(this,
        ...
        if (suspendedJobEntityManager == null) {
            suspendedJobEntityManager = new SuspendedJobEntityManagerImpl(this,
                suspendedJobDataManager);
        }
        if (deadLetterJobEntityManager == null) {
            deadLetterJobEntityManager = new DeadLetterJobEntityManagerImpl(this,
        ...
        if (processDefinitionInfoEntityManager == null) {
            processDefinitionInfoEntityManager = new ProcessDefinitionInfoEntityManagerImpl(this,
        ...
        if (variableInstanceEntityManager == null) {
            variableInstanceEntityManager = new VariableInstanceEntityManagerImpl(this,
    ...
    public void initConfigurators() {
        ...
        if (enableConfiguratorServiceLoader) {
            ...
            ServiceLoader<ProcessEngineConfigurator> configuratorServiceLoader = ServiceLoader.load(
                ProcessEngineConfigurator.class, classLoader);
            ...
            if (nrOfServiceLoadedConfigurators > 0) {
                log.info("Found {} auto-discoverable Process Engine Configurator{}",
                    nrOfServiceLoadedConfigurators++,
            ...
            if (!allConfigurators.isEmpty()) {
                ...
                log.info("Found {} Process Engine Configurators in total:",
                    allConfigurators.size());
                for (ProcessEngineConfigurator configurator : allConfigurators) {
                    log.info("{} (priority:{})", configurator.getClass(),
    ...
    public void configuratorsBeforeInit() {
        for (ProcessEngineConfigurator configurator : allConfigurators) {
            log.info("Executing beforeInit() of {} (priority:{})", configurator.getClass(),
                configurator.getPriority());
    ...
    public void configuratorsAfterInit() {
        for (ProcessEngineConfigurator configurator : allConfigurators) {
            log.info("Executing configure() of {} (priority:{})", configurator.getClass(),
                configurator.getPriority());
    ...
    public void initBehaviorFactory() {
        if (activityBehaviorFactory == null) {
            ...
            ((AbstractBehaviorFactory) activityBehaviorFactory).setExpressionManager(
                expressionManager);
        }
    }
                ...
                if (defaultBpmnParseHandler.getHandledTypes().size() != 1) {
                    ...
                    throw new ActivitiException(
                        "The default BPMN parse handlers should only support one type, but "
                            + defaultBpmnParseHandler.getClass() + " supports "
                            + supportedTypes
                            + ". This is likely a programmatic error");
                } else {
                    Class<?> handledType = defaultBpmnParseHandler.getHandledTypes().iterator()
                        .next();
                    if (customParseHandlerMap.containsKey(handledType)) {
                        BpmnParseHandler newBpmnParseHandler = customParseHandlerMap.get(
                            handledType);
                        log.info("Replacing default BpmnParseHandler "
                            + defaultBpmnParseHandler.getClass().getName() + " with "
    ...
    public void initJobHandlers() {
        ...
        jobHandlers.put(activateProcessDefinitionHandler.getType(),
            activateProcessDefinitionHandler);
    ...
    public void initVariableTypes() {
        if (variableTypes == null) {
            ...
            variableTypes.addType(
                new JsonType(getMaxLengthString(), objectMapper, serializePOJOsInVariablesToJson,
                    jsonTypeConverter));
    ...
    public void initScriptingEngines() {
        ...
        if (scriptingEngines == null) {
            scriptingEngines = new ScriptingEngines(
                new ScriptBindingsFactory(this, resolverFactories));
        }
    }
    ...
    public void initJpa() {
        ...
        if (jpaEntityManagerFactory != null) {
            sessionFactories.put(EntityManagerSession.class,
                new EntityManagerSessionFactory(jpaEntityManagerFactory, jpaHandleTransaction,
            ...
            if (jpaType == null) {
                variableTypes.addType(new JPAEntityListVariableType(),
    ...
    public ProcessEngineConfigurationImpl setProcessDefinitionHelper(
