 ...
 * <p>
 * If multitenancy is needed and no data isolation is needed: the default {@link
 ...
 * <p>
 * - It needs a {@link TenantInfoHolder} to determine which tenant is currently 'active'. Ie for
 * which tenant a certain API call is executed.
 * <p>
 * - The {@link StrongUuidGenerator} is used by default. The 'regular' {@link DbIdGenerator} cannot
 * be used with this config.
 * <p>
 * - Adding tenants (also after boot!) is done using the {@link #registerTenant(String, DataSource)}
 * operations.
 * <p>
 * - Currently, this config does not work with the 'old' {@link JobExecutor}, but only with the
 ...
 */
@Deprecated
public class MultiSchemaMultiTenantProcessEngineConfiguration extends
    ...
    /**
     * Add a new {@link DataSource} for a tenant, identified by the provided tenantId, to the
     * engine. This can be done after the engine has booted up.
     * <p>
    ...
    public void initAsyncExecutor() {
        ...
        if (asyncExecutor instanceof TenantAwareAsyncExecutor) {
            for (String tenantId : tenantInfoHolder.getAllTenants()) {
                ((TenantAwareAsyncExecutor) asyncExecutor).addTenantAsyncExecutor(tenantId,
                    false); // false -> will be started later with all the other executors
            }
        }
    }
