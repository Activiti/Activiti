...
public class TimerJobEntityManagerImpl extends AbstractEntityManager<TimerJobEntity>
    ...
    public List<TimerJobEntity> findJobsByTypeAndProcessDefinitionId(
        ...
        return jobDataManager.findJobsByTypeAndProcessDefinitionId(
                jobHandlerType, processDefinitionId);
    }
    ...
    public List<TimerJobEntity> findJobsByTypeAndProcessDefinitionKeyNoTenantId(
        ...
        return jobDataManager.findJobsByTypeAndProcessDefinitionKeyNoTenantId(
                jobHandlerType, processDefinitionKey);
    }
    ...
    public List<TimerJobEntity> findJobsByTypeAndProcessDefinitionKeyAndTenantId(
        ...
        return jobDataManager.findJobsByTypeAndProcessDefinitionKeyAndTenantId(
                jobHandlerType, processDefinitionKey, tenantId);
    }
    ...
    protected boolean doInsert(TimerJobEntity jobEntity, boolean fireCreateEvent) {
        ...
        if (jobEntity.getExecutionId() != null) {
            ExecutionEntity execution =
                    getExecutionEntityManager().findById(jobEntity.getExecutionId());
            if (execution != null) {
                ...
                if (isExecutionRelatedEntityCountEnabled(execution)) {
                    CountingExecutionEntity countingExecutionEntity =
                            (CountingExecutionEntity) execution;
                    countingExecutionEntity.setTimerJobCount(
                            countingExecutionEntity.getTimerJobCount() + 1);
                }
            } else {
            ...
            getEventDispatcher()
                    .dispatchEvent(
                            ActivitiEventBuilder.createEntityEvent(
                                    ActivitiEventType.ENTITY_DELETED, this));
        }
    }
    ...
    protected Date calculateNextTimer(JobEntity timerEntity, VariableScope variableScope) {
        ...
        return businessCalendar.resolveDuedate(
                timerEntity.getRepeat(), timerEntity.getMaxIterations());
    }
