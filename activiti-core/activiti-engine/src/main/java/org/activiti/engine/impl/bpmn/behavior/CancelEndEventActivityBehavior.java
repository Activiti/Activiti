...
public class CancelEndEventActivityBehavior extends FlowNodeActivityBehavior {
    ...
    public void execute(DelegateExecution execution) {
        ...
        while (currentlyExaminedExecution != null && parentScopeExecution == null) {
            if (currentlyExaminedExecution.getCurrentFlowElement() instanceof SubProcess) {
                ...
                SubProcess subProcess =
                        (SubProcess) currentlyExaminedExecution.getCurrentFlowElement();
        ...
        if (subProcess.getLoopCharacteristics() != null) {
            List<? extends ExecutionEntity> multiInstanceExecutions =
                    parentScopeExecution.getExecutions();
        ...
        commandContext
                .getHistoryManager()
                .recordActivityEnd(parentScopeExecution, DeleteReason.TRANSACTION_CANCELED);
        ...
        if (CollectionUtil.isNotEmpty(childExecutions)) {
            for (ExecutionEntity childExecution : childExecutions) {
                if (!(childExecution.getId().equals(notToDeleteExecution.getId()))) {
                    deleteChildExecutions(
                            childExecution, notToDeleteExecution, commandContext, deleteReason);
                }
            }
        }
