...
public class CancelEndEventActivityBehavior extends FlowNodeActivityBehavior {
    ...
    public void execute(DelegateExecution execution) {
        ...
        if (parentScopeExecution == null) {
            throw new ActivitiException("No sub process execution found for cancel end event "
        ...
        if (cancelBoundaryEvent == null) {
            throw new ActivitiException("Could not find cancel boundary event for cancel end event "
                + executionEntity.getCurrentActivityId());
        }

        ExecutionEntity newParentScopeExecution = null;
        currentlyExaminedExecution = executionEntityManager.findById(
            parentScopeExecution.getParentId());
        ...
        if (subProcess.getLoopCharacteristics() != null) {
            ...
            for (ExecutionEntity executionEntityToDelete : executionsToDelete) {
                deleteChildExecutions(executionEntityToDelete, executionEntity, commandContext,
                    DeleteReason.TRANSACTION_CANCELED);
            }
        }
    ...
    protected void deleteChildExecutions(ExecutionEntity parentExecution,
