 ...
 *
 */
public class ProcessInstanceHistoryLogQueryImpl implements ProcessInstanceHistoryLogQuery,
    ...
    public ProcessInstanceHistoryLogQueryImpl(CommandExecutor commandExecutor,
    ...
    public ProcessInstanceHistoryLog execute(CommandContext commandContext) {
        ...
        HistoricProcessInstanceEntity historicProcessInstance = commandContext.getHistoricProcessInstanceEntityManager()
        ...
        ProcessInstanceHistoryLogImpl processInstanceHistoryLog = new ProcessInstanceHistoryLogImpl(
        ...
        if (includeTasks) {
            List<? extends HistoricData> tasks = commandContext.getHistoricTaskInstanceEntityManager()
                .findHistoricTaskInstancesByQueryCriteria(
                    new HistoricTaskInstanceQueryImpl(commandExecutor).processInstanceId(
                        processInstanceId));
        ...
        if (includeActivities) {
            List<HistoricActivityInstance> activities = commandContext.getHistoricActivityInstanceEntityManager()
                .findHistoricActivityInstancesByQueryCriteria(
                    new HistoricActivityInstanceQueryImpl(commandExecutor).processInstanceId(
        ...
        if (includeVariables) {
            List<HistoricVariableInstance> variables = commandContext.getHistoricVariableInstanceEntityManager()
                .findHistoricVariableInstancesByQueryCriteria(
                    new HistoricVariableInstanceQueryImpl(commandExecutor).processInstanceId(
                        processInstanceId), null);
        ...
        // Comment
        if (includeComments) {
            List<? extends HistoricData> comments = commandContext.getCommentEntityManager()
