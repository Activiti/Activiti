...
public class TimerStartEventJobHandler extends TimerEventHandler implements JobHandler {
    ...
    private static final Logger log = LoggerFactory.getLogger(TimerStartEventJobHandler.class);
    ...
    public void execute(JobEntity job, String configuration, ExecutionEntity execution,
        ...
        ProcessDefinitionEntity processDefinitionEntity = ProcessDefinitionUtil
            .getProcessDefinitionFromDatabase(
                job.getProcessDefinitionId()); // From DB -> need to get latest suspended state
        ...
        try {
            if (!processDefinitionEntity.isSuspended()) {
                ...
                if (activityId != null) {
                    ...
                    if (flowElement == null) {
                        throw new ActivitiException(
                            "Could not find matching FlowElement for activityId " + activityId);
                    }
                    ...
                    processInstanceHelper.createAndStartProcessInstanceWithInitialFlowElement(
                        processDefinitionEntity, null, null, flowElement, process, new HashMap<>(),
                        null, true);
                } else {
                    new StartProcessInstanceCmd(processDefinitionEntity.getKey(), null, null, null,
                        job.getTenantId()).execute(commandContext);
                }

            } else {
                log.debug("ignoring timer of suspended process definition {}",
