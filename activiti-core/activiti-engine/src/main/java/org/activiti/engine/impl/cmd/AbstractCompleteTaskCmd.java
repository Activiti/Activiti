...
public abstract class AbstractCompleteTaskCmd extends NeedsActiveTaskCmd<Void> {
    ...
    protected void executeTaskComplete(CommandContext commandContext, TaskEntity taskEntity,
        ...
        if (taskEntity.getDelegationState() != null && taskEntity.getDelegationState()
            .equals(DelegationState.PENDING)) {
        ...
        if (Authentication.getAuthenticatedUserId() != null
            ...
            commandContext.getIdentityLinkEntityManager()
                .involveUser(processInstanceEntity, Authentication.getAuthenticatedUserId(),
        ...
        if (eventDispatcher.isEnabled()) {
            if (variables != null) {
                eventDispatcher.dispatchEvent(ActivitiEventBuilder.createEntityWithVariablesEvent(
                    ActivitiEventType.TASK_COMPLETED, taskEntity, variables, localScope));
            } else {
                eventDispatcher.dispatchEvent(
                    ActivitiEventBuilder.createEntityEvent(ActivitiEventType.TASK_COMPLETED,
                        taskEntity));
            }
        }
