 ...
 *
 * <p>Note that it is impossible to retrieve the deployment resources through the results of this
 * operation, since that would cause a huge transfer of (possibly) unneeded bytes over the wire.
 *
 * <p>To retrieve the actual bytes of a deployment resource use the operations on the {@link
 * RepositoryService#getDeploymentResourceNames(String)} and {@link
...
public interface DeploymentQuery extends Query<DeploymentQuery, Deployment> {
  ...
  /**
   * Only select deployments where the deployment time is the latest value. Can only be used
   * together with the deployment key.
   */
