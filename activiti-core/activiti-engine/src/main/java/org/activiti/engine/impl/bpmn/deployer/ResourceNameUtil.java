...
public class ResourceNameUtil {
  ...
  public static String getProcessDiagramResourceName(
      String bpmnFileResource, String processKey, String diagramSuffix) {
  ...
  /**
   * Finds the name of a resource for the diagram for a process definition. Assumes that the process
   * definition's key and (BPMN) resource name are already set.
   *
   ...
   * containing only one process with key 'myProcess', then this method will look for an image
   * resources called'abc.myProcess.png' (or .jpg, or .gif, etc.) or 'abc.png' if the previous one
   * wasn't found.
   *
   * <p>Example 2: if the deployment contains a BPMN 2.0 xml resource called 'abc.bpmn20.xml'
   * containing three processes (with keys a, b and c), then this method will first look for an
   * image resource called 'abc.a.png' before looking for 'abc.png' (likewise for b and c). Note
   * that if abc.a.png, abc.b.png and abc.c.png don't exist, all processes will have the same image:
   * abc.png.
   *
   * @return name of an existing resource, or null if no matching image resource is found in the
   *     resources.
   */
