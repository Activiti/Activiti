...
public class BoundaryCancelEventActivityBehavior extends BoundaryEventActivityBehavior {
    ...
    public void trigger(DelegateExecution execution, String triggerName, Object triggerData) {
        ...
        List<CompensateEventSubscriptionEntity> eventSubscriptions = eventSubscriptionEntityManager.findCompensateEventSubscriptionsByExecutionId(
            subProcessExecution.getParentId());
        ...
        if (eventSubscriptions.isEmpty()) {
            ...
            String deleteReason =
                DeleteReason.BOUNDARY_EVENT_INTERRUPTING + "(" + boundaryEvent.getId() + ")";
            ...
            if (subProcessExecution.getCurrentFlowElement() instanceof Activity) {
                ...
                if (activity.getLoopCharacteristics() != null) {
                    ...
                    List<ExecutionEntity> miChildExecutions = executionEntityManager.findChildExecutionsByParentExecutionId(
                        miExecution.getId());
                    for (ExecutionEntity miChildExecution : miChildExecutions) {
                        if (subProcessExecution.getId().equals(miChildExecution.getId()) == false
