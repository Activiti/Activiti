...
public interface TaskInfoQuery<T extends TaskInfoQuery<?, ?>, V extends TaskInfo>
    ...
    /**
     * Only select tasks with the given task id (in practice, there will be maximum one of this
     * kind)
     */
     ...
     *
     * @throws ActivitiIllegalArgumentException When passed name list is empty or <code>null</code>
     *     or contains <code>null String</code>.
     */
     ...
     * upper/lower case: both the input parameter as the column value are lowercased when the query
     * is executed.
     */
     ...
     * example usage: descriptionLike(%activiti%)
     *
     * <p>This method, unlike the {@link #taskDescriptionLike(String)} method will not take in
     * account the upper/lower case: both the input parameter as the column value are lowercased
    ...
    /**
     * Only select tasks which were last assigned to an assignee like the given value. The syntax
     ...
     * including tasks which have been assigned to the given user (assignee) or owned by the given
     * user (owner).
     */
     ...
     *     #taskCandidateGroup(String)} or {@link #taskCandidateUser(String)} has been executed on
     *     the query instance. When passed group list is empty or <code>null</code>.
     */
     ...
     * example usage: processInstanceBusinessKeyLike("%activiti%").
     *
     * <p>This method, unlike the {@link #processInstanceBusinessKeyLike(String)} method will not
     * take in account the upper/lower case: both the input parameter as the column value are
     ...
     * of SQL: for example usage: taskDefinitionKeyLike("%activiti%"). The task definition key is
     * the id of the userTask: &lt;userTask id="xxx" .../&gt;
     */
    ...
    /**
     * Only select tasks which are part of a process instance which has a process definition key
     * like the given value. The syntax that should be used is the same as in SQL, eg. %activiti%.
     */
    ...
    /**
     * Only select tasks that have a process definition for which the key is present in the given
     * list *
     */
    ...
    /**
     * Only select tasks which are part of a process instance which has a process definition name
     * like the given value. The syntax that should be used is the same as in SQL, eg. %activiti%.
     */
    T processDefinitionNameLike(String processDefinitionNameLike);

    /**
     * Only select tasks which are part of a process instance whose definition belongs to the
    ...
    /**
     * Only select tasks which are part of a process instance whose definition does not belong to
     * the category which is present in the given list.
     *
     * @throws ActivitiIllegalArgumentException When passed category list is empty or <code>null
     *     </code> or contains <code>null String</code>.
    ...
    /**
     * Only select tasks which have a local task variable with the given name set to the given
     * value.
     */
     ...
     *
     * <p>This method only works if your database has encoding/collation that supports
     * case-sensitive queries. For example, use "collate UTF-8" on MySQL and for MSSQL, select one
     * of the case-sensitive Collations available (<a
     * href="http://msdn.microsoft.com/en-us/library/ms144250(v=sql.105).aspx" >MSDN Server
     * Collation Reference</a>).
     */
    ...
    /**
     * Only select tasks which have a local variable value less than or equal to the passed value
     * when they ended. Booleans, Byte-arrays and {@link Serializable} objects (which are not
     * primitive type wrappers) are not supported.
     *
     * @param name cannot be null.
    ...
    /**
     * Only select tasks which are part of a process that has a local string variable which is not
    ...
    /**
     * Only select tasks which are part of a process that has a string variable with the given
    ...
    /**
     * Only select tasks which have a global variable value greater than or equal to the passed
     * value when they ended. Booleans, Byte-arrays and {@link Serializable} objects (which are not
     * primitive type wrappers) are not supported.
     *
     * @param name cannot be null.
     * @param value cannot be null.
     ...
     *
     * <p>Calling endOr() will add all clauses to the regular query again. Calling or() after
     * endOr() has been called will result in an exception.
     */
    T or();
