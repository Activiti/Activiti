...
public interface ProcessInstanceQuery extends Query<ProcessInstanceQuery, ProcessInstance> {
  ...
  /**
   * Select the process instance that have as sub process instance the given process instance. Note
   * that there will always be maximum only <b>one</b> such process instance that can be the result
   * of this query.
   */
  ...
  /**
   * Only select process instances which have at least one global variable with the given value. The
   * type of variable is determined based on the value, using types configured in {@link
   * ProcessEngineConfiguration#getVariableTypes()}. Byte-arrays and {@link Serializable} objects
   * (which are not primitive type wrappers) are not supported.
   */
  ...
  /**
   * Only select process instances which have a global variable value like the given value. This be
   * used on string variables only.
   *
   * @param name variable name, cannot be null.
