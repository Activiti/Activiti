        ...
        while (!isInterrupted) {
            ...
            try {
                AcquiredJobEntities acquiredJobs =
                        commandExecutor.execute(new AcquireJobsCmd(asyncExecutor));
                ...
                // If the queue was full, we wait too (even if we got enough jobs back), as not
                // overload the
                // queue
                ...
                if (log.isDebugEnabled()) {
                    log.debug(
                            "Optimistic locking exception during async job acquisition. If you have"
                                + " multiple async executors running against the same database,"
                                + " this exception means that this thread tried to acquire a due"
                                + " async job, which already was acquired by another async executor"
                                + " acquisition thread.This is expected behavior in a clustered"
                                + " environment. You can ignore this message if you indeed have"
                                + " multiple async executor acquisition threads running against the"
                                + " same database. Exception message: {}",
