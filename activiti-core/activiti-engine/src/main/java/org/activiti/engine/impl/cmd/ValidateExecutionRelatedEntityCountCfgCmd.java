    ...
    public Void execute(CommandContext commandContext) {
        ...
        if (propertyEntity == null) {
            ...
            boolean propertyValue =
                    Boolean.valueOf(propertyEntity.getValue().toString().toLowerCase());
            if (!configProperty && propertyValue) {
                if (logger.isInfoEnabled()) {
                    logger.info(
                            "Configuration change: execution related entity counting feature was"
                                    + " enabled before, but now disabled. Updating all execution"
                                    + " entities.");
                }
                commandContext
                        .getProcessEngineConfiguration()
