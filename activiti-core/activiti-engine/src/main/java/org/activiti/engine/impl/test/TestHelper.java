...
public abstract class TestHelper {
    ...
    public static String annotationDeploymentSetUp(ProcessEngine processEngine, Class<?> testClass,
        String methodName) {
        ...
        try {
            ...
            log.warn(
                "Could not get method by reflection. This could happen if you are using @Parameters in combination with annotations.",
                e);
            return null;
        }
        Deployment deploymentAnnotation = method.getAnnotation(Deployment.class);
        if (deploymentAnnotation != null) {
            log.debug("annotation @Deployment creates deployment for {}.{}",
            ...
            DeploymentBuilder deploymentBuilder = processEngine.getRepositoryService()
                .createDeployment().name(testClass.getSimpleName() + "." + methodName);
    ...
    public static void annotationDeploymentTearDown(ProcessEngine processEngine,
        ...
        log.debug("annotation @Deployment deletes deployment for {}.{}", testClass.getSimpleName(),
            methodName);
    ...
    public static void annotationMockSupportSetup(Class<?> testClass, String methodName,
    ...
    protected static void handleMockServiceTaskAnnotation(ActivitiMockSupport mockSupport,
        ...
        mockSupport.mockServiceTaskWithClassDelegate(mockedServiceTask.originalClassName(),
            mockedServiceTask.mockedClassName());
    }
    ...
    protected static void handleNoOpServiceTasksAnnotation(ActivitiMockSupport mockSupport,
        ...
        if (noOpServiceTasks != null) {
            ...
            if ((ids == null || ids.length == 0) && (classes == null || classes.length == 0) && (
     ...
     * The return value will be the full classpath location of the type, plus a suffix built from
     * the name parameter: <code>BpmnDeployer.BPMN_RESOURCE_SUFFIXES</code>. The first resource
    ...
    public static String getBpmnProcessDefinitionResource(Class<?> type, String name) {
        ...
        return type.getName().replace('.', '/') + "." + name + "."
