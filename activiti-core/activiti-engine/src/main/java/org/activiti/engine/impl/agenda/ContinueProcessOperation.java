...
/**
 * Operation that takes the current {@link FlowElement} set on the {@link ExecutionEntity} and
 * executes the associated {@link ActivityBehavior}. In the case of async, schedules a {@link Job}.
 *
 * <p>Also makes sure the {@link ExecutionListener} instances are called.
 */
public class ContinueProcessOperation extends AbstractOperation {
    ...
    public ContinueProcessOperation(
    ...
    protected void executeProcessStartExecutionListeners() {
        ...
        executeExecutionListeners(
                process, execution.getParent(), ExecutionListener.EVENTNAME_START);
    }

    protected void continueThroughFlowNode(FlowNode flowNode) {

        // Check if it's the initial flow element. If so, we must fire the execution listeners for
        // the
        // process too
        ...
        // For a subprocess, a new child execution is created that will visit the steps of the
        // subprocess
    ...
    protected void createChildExecutionForSubProcess(SubProcess subProcess) {
        ...
        ExecutionEntity subProcessExecution =
                commandContext
                        ...
                        .createChildExecution(parentScopeExecution);
    ...
    protected void executeSynchronous(FlowNode flowNode) {
        ...
        // Execute any boundary events, sub process boundary events will be executed from the
        // activity
        // behavior
        if (!inCompensation
                && flowNode instanceof Activity) { // Only activities can have boundary events
    ...
    protected void continueThroughSequenceFlow(SequenceFlow sequenceFlow) {
        ...
        // Execution listener. Sequenceflow only 'take' makes sense ... but we've supported all
        // three
        // since the beginning
                    ...
                    .dispatchEvent(
                            ActivitiEventBuilder.createSequenceFlowTakenEvent(
                                    ...
                                    sourceFlowElement != null
                                            ? (String) sourceFlowElement.getName()
                                            : null,
                                    sourceFlowElement != null
                                            ? sourceFlowElement.getClass().getName()
                                            : null,
                                    sourceFlowElement != null
                                            ? ((FlowNode) sourceFlowElement).getBehavior()
                                            : null,
                                    ...
                                    targetFlowElement != null
                                            ? targetFlowElement.getClass().getName()
                                            : null,
                                    targetFlowElement != null
                                            ? ((FlowNode) targetFlowElement).getBehavior()
                                            : null));
        }
        ...
        logger.debug(
                "Sequence flow '{}' encountered. Continuing process by following it using execution"
                        + " {}",
                sequenceFlow.getId(),
                execution.getId());
    ...
    protected void executeBoundaryEvents(
            Collection<BoundaryEvent> boundaryEvents, ExecutionEntity execution) {
        ...
        for (BoundaryEvent boundaryEvent : boundaryEvents) {
            ...
            // A Child execution of the current execution is created to represent the boundary event
            // being
            // active
