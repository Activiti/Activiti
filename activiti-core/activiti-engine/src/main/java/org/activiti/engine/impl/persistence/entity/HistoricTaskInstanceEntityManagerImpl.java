 ...
 *
 */
public class HistoricTaskInstanceEntityManagerImpl extends
    AbstractEntityManager<HistoricTaskInstanceEntity> implements HistoricTaskInstanceEntityManager {
    ...
    public void deleteHistoricTaskInstancesByProcessInstanceId(String processInstanceId) {
        if (getHistoryManager().isHistoryLevelAtLeast(HistoryLevel.AUDIT)) {
            ...
            for (HistoricTaskInstanceEntity historicTaskInstanceEntity : taskInstances) {
                delete(
                    historicTaskInstanceEntity.getId()); // Needs to be by id (since that method is overridden, see below !)
            }
        }
    }
