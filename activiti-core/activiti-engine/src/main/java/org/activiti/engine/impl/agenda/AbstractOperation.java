...
/**
 * Abstract superclass for all operation interfaces (which are {@link Runnable} instances), exposing
 * some shared helper methods and member fields to subclasses.
 *
 * <p>An operations is a {@link Runnable} instance that is put on the {@link Agenda} during the
...
public abstract class AbstractOperation implements Runnable {
  ...
  /**
   * Helper method to match the activityId of an execution with a FlowElement of the process
   * definition referenced by the execution.
   */
  ...
  /**
   * Executes the execution listeners defined on the given element, with the given event type. Uses
   * the {@link #execution} of this operation instance as argument for the execution listener.
   */
  protected void executeExecutionListeners(
      HasExecutionListeners elementWithExecutionListeners, String eventType) {
    executeExecutionListeners(elementWithExecutionListeners, execution, eventType);
  }

  /**
   * Executes the execution listeners defined on the given element, with the given event type, and
   * passing the provided execution to the {@link ExecutionListener} instances.
   */
  protected void executeExecutionListeners(
