...
public class SendTaskParseHandler extends AbstractActivityBpmnParseHandler<SendTask> {
    ...
    protected void executeParse(BpmnParse bpmnParse, SendTask sendTask) {
        ...
        if (StringUtils.isNotEmpty(sendTask.getType())) {
            ...
            sendTask.getImplementationType()) && StringUtils.isNotEmpty(
            sendTask.getOperationRef())) {
            ...
            logger.warn("One of the attributes 'type' or 'operation' is mandatory on sendTask "
                + sendTask.getId());
        }
    }
