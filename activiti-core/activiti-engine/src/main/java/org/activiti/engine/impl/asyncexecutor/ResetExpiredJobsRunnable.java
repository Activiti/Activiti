 ...
 * <p>
 * When a job is executed, it is first locked (lock owner and lock time is set). A job is expired
 * when this lock time is exceeded. This can happen when an executor goes down before completing a
 * task.
 * <p>
...
public class ResetExpiredJobsRunnable implements Runnable {
    ...
    private static final Logger log = LoggerFactory.getLogger(ResetExpiredJobsRunnable.class);
    ...
    public synchronized void run() {
        ...
        while (!isInterrupted) {
            ...
            try {
                ...
                List<JobEntity> expiredJobs = asyncExecutor.getProcessEngineConfiguration()
                    .getCommandExecutor()
