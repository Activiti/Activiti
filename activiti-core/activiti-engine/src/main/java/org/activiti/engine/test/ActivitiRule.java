 ...
 * of the activitiRule. The processEngine will be initialized by default with the activiti.cfg.xml
 * resource on the classpath. To specify a different configuration file, pass the resource location
 * in {@link #ActivitiRule(String) the appropriate constructor}. Process engines will be cached
 * statically. Right before the first time the setUp is called for a given configuration resource,
 * the process engine will be constructed.
 * </p>
 *
 * <p>
 ...
 *
 * <p>
 * The activitiRule also lets you {@link ActivitiRule#setCurrentTime(Date) set the current time used
 * by the process engine}. This can be handy to control the exact time that is used by the engine in
 * order to verify e.g. e.g. due dates of timers. Or start, end and duration times in the history
 * service. In the tearDown, the internal clock will automatically be reset to use the current
