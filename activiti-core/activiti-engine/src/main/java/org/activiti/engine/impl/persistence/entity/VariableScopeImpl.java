...
public abstract class VariableScopeImpl extends AbstractEntity
    ...
    protected Map<String, VariableInstanceEntity>
            variableInstances; // needs to be null, the logic depends on it for checking if vars
    // were
    // already fetched
        ...
        for (String variableName : variableNames) {
            if (transientVariabes != null && transientVariabes.containsKey(variableName)) {
                requestedVariables.put(
                        variableName, transientVariabes.get(variableName).getValue());
                variableNamesToFetch.remove(variableName);
            } else if (usedVariablesCache.containsKey(variableName)) {
                requestedVariables.put(
                        variableName, usedVariablesCache.get(variableName).getValue());
                variableNamesToFetch.remove(variableName);
            }
        }
     ...
     *
     * <p>Note that the default Activiti way (because of backwards compatibility) is to fetch all
     * the variables when doing a get/set of variables. So this means 'true' is the default value
     * for this method, and in fact it will simply delegate to {@link #getVariable(String)}. This
     * can also be the most performant, if you're doing a lot of variable gets in the same
     * transaction (eg in service tasks).
     *
    ...
    public VariableInstance getVariableInstanceLocal(
            String variableName, boolean fetchAllVariables) {
        ...
        if (fetchAllVariables == true) {
            ...
            if (usedVariablesCache.containsKey(variableName)) {
                updateVariableInstance(
                        usedVariablesCache.get(variableName), value, sourceExecution);
            }
                ...
                if (variable != null) {
                    ...
                    if (parent != null) {
                        if (sourceExecution == null) {
                            ...
                            parent.setVariable(
                                    variableName, value, sourceExecution, fetchAllVariables);
                        }
    ...
    /**
     * The default {@link #setVariableLocal(String, Object)} fetches all variables (for historical
     * and backwards compatible reasons) while setting the variables.
     *
     * <p>Setting the fetchAllVariables parameter to true is the default behaviour (ie fetching all
     * variables) Setting the fetchAllVariables parameter to false does not do that.
     */
    public Object setVariableLocal(String variableName, Object value, boolean fetchAllVariables) {
        return setVariableLocal(
                variableName, value, getSourceActivityExecution(), fetchAllVariables);
    }
    ...
    public Object setVariableLocal(
        ...
        if (fetchAllVariables == true) {
            ...
            if (usedVariablesCache.containsKey(variableName)) {
                ...
                updateVariableInstance(
                        variableInstances.get(variableName), value, sourceActivityExecution);
            } else {
    ...
    protected void createVariableLocal(
        ...
        if (variableInstances.containsKey(variableName)) {
            throw new ActivitiException(
                    "variable '"
                            + variableName
                            + "' already exists. Use setVariableLocal if you want to overwrite the"
                            + " value");
        }
    ...
    protected void removeVariableLocal(
            String variableName, ExecutionEntity sourceActivityExecution) {
        ensureVariableInstancesInitialized();
    ...
    public void setTransientVariableLocal(String variableName, Object variableValue) {
        ...
        transientVariabes.put(
                variableName, new TransientVariableInstance(variableName, variableValue));
    }
