 ...
 * 2.0.
 *
 * <p>This class can be used by inheritance or aggregation.
 */
public class BpmnActivityBehavior implements Serializable {
   ...
   *
   * <p>More precisely: every sequence flow that has a condition which evaluates to true (or which
   ...
   * #performDefaultOutgoingBehavior(ExecutionEntity)}), but without checking the conditions on the
   * outgoing sequence flow.
   *
   * <p>This means that every outgoing sequence flow is selected for continuing the process
   * instance, regardless of having a condition or not. In case of multiple outgoing sequence flow,
   * multiple parallel paths of executions will be created.
   */
   ...
   * @param checkConditions Whether or not to check conditions before determining whether or not to
   *     take a transition.
   * @param throwExceptionIfExecutionStuck If true, an {@link ActivitiException} will be thrown in
   *     case no transition could be found to leave the activity.
   */
  protected void performOutgoingBehavior(
      ExecutionEntity execution, boolean checkConditions, boolean throwExceptionIfExecutionStuck) {
  ...
  private void propagateVariablesToParent(ExecutionEntity execution) {
    ...
    if (parentExecution != null) {
      parentExecution.setVariables(
          variablesCalculator.calculateOutPutVariables(
              buildMappingExecutionContext(execution), execution.getVariablesLocal()));
    }
  }
