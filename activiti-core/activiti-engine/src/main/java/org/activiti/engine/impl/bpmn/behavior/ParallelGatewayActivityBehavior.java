 ...
 * <p>
 * The Parallel Gateway can be used for splitting a path of execution into multiple paths of
 * executions (AND-split/fork behavior), one for every outgoing sequence flow.
 * <p>
 * The Parallel Gateway can also be used for merging or joining paths of execution (AND-join). In
 ...
 * sequence flow).
 * <p>
 * Note that there is a slight difference to spec (p. 436): "The parallel gateway is activated if
 ...
 * <p>
 * Note that a Parallel Gateway having one incoming and multiple outgoing sequence flow, is the same
 * as having multiple outgoing sequence flow on a given activity. However, a parallel gateway does
 * NOT check conditions on the outgoing sequence flow.
 */
public class ParallelGatewayActivityBehavior extends GatewayActivityBehavior {
        ...
        if (nbrOfExecutionsCurrentlyJoined == nbrOfExecutionsToJoin) {
            ...
            if (log.isDebugEnabled()) {
                log.debug("parallel gateway '{}' activates: {} of {} joined",
                    execution.getCurrentActivityId(), nbrOfExecutionsCurrentlyJoined,
            ...
            Context.getAgenda().planTakeOutgoingSequenceFlowsOperation((ExecutionEntity) execution,
                false); // false -> ignoring conditions on parallel gw
    ...
    protected boolean isChildOfMultiInstanceExecution(DelegateExecution executionEntity,
        ...
        if (parentExecution != null) {
            if (parentExecution.getId().equals(multiInstanceExecution.getId())) {
                ...
                boolean isNestedChild = isChildOfMultiInstanceExecution(parentExecution,
