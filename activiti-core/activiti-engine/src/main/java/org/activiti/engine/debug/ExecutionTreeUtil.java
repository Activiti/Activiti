...
public class ExecutionTreeUtil {
    ...
    public static ExecutionTree buildExecutionTree(DelegateExecution executionEntity) {
        ...
        while (parentExecution.getParentId() != null
    ...
    protected static void collectChildExecutions(
            ExecutionEntity rootExecutionEntity, List<ExecutionEntity> allExecutions) {
    ...
    public static ExecutionTree buildExecutionTree(Collection<ExecutionEntity> executions) {
        ...
        // Map the executions to their parents. Catch and store the root element (process instance
        // execution) while were at it
    ...
    protected static void fillExecutionTree(
        ...
        if (executionTree.getRoot() == null) {
            throw new ActivitiException(
                    "Programmatic error: the list of passed executions in the argument of the"
                            + " method should contain the process instance execution");
        }
        ...
        while (!executionsToHandle.isEmpty()) {
            ...
            if (parentMapping.containsKey(parentId)) {
                ...
                List<ExecutionTreeNode> childNodes =
                        new ArrayList<ExecutionTreeNode>(childExecutions.size());
