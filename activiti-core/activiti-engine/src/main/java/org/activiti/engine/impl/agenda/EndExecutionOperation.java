...
public class EndExecutionOperation extends AbstractOperation {
  ...
  protected void handleProcessInstanceExecution(ExecutionEntity processInstanceExecution) {
    ...
    if (superExecution != null) {
      ...
      try {
        ...
        logger.error(
            "Error while completing sub process of execution {}", processInstanceExecution, e);
        throw e;
      } catch (Exception e) {
        logger.error(
            "Error while completing sub process of execution {}", processInstanceExecution, e);
    ...
    if (activeExecutions == 0) {
      ...
      logger.debug(
          "Active executions found. Process instance {} will not be ended.", processInstanceId);
    }
  ...
  protected void handleRegularExecution() {
    ...
    logger.debug(
        "Parent execution found. Continuing process using execution {}", parentExecution.getId());
    ...
    if (getNumberOfActiveChildExecutionsForExecution(
      ...
      if (subProcess != null) {
        ...
        if (subProcess.isForCompensation()) {
          ...
          executionToContinue =
              handleSubProcessEnd(executionEntityManager, parentExecution, subProcess);
        }
      } else {
  ...
  protected void handleMultiInstanceSubProcess(
    ...
    List<ExecutionEntity> activeChildExecutions =
        getActiveChildExecutionsForExecution(executionEntityManager, parentExecution.getId());
