...
public abstract class ReflectUtil {
    ...
    public static Class<?> loadClass(String className) {
        ...
        if (clazz == null) {
            try {
                LOG.trace("Trying to load class with current thread context classloader: {}",
                    className);
    ...
    public static Method getSetter(String fieldName, Class<?> clazz, Class<?> fieldType) {
        String setterName =
            "set" + Character.toTitleCase(fieldName.charAt(0)) + fieldName.substring(1
            );
        try {
            ...
            throw new ActivitiException("Not allowed to access method " + setterName + " on class "
