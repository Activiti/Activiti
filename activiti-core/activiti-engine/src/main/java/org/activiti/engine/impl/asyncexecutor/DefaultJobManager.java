...
public class DefaultJobManager implements JobManager {
        ...
        if (eventDispatcher.isEnabled()) {
            eventDispatcher.dispatchEvent(
                    ActivitiEventBuilder.createEntityEvent(
                            ActivitiEventType.TIMER_SCHEDULED, timerJob));
        }
    }
    ...
    public void unacquire(Job job) {
        ...
        if (job instanceof JobEntity) {
            ...
            // We're not calling triggerExecutorIfNeeded here after the inser. The unacquire
            // happened
    ...
    protected void restoreExtraData(JobEntity timerEntity, VariableScope variableScope) {
        ...
        if (timerEntity.getJobHandlerType().equalsIgnoreCase(TimerStartEventJobHandler.TYPE)
            ...
            String endDateExpressionString =
                    TimerEventHandler.getEndDateFromConfiguration(
                            timerEntity.getJobHandlerConfiguration());
                    ...
                    if (endDateValue instanceof String) {
                        ...
                        throw new ActivitiException(
                                "Timer '"
                                        ...
                                        + "' was not configured with a valid duration/time, either"
                                        + " hand in a java.util.Date or a String in format"
    ...
    protected JobEntity internalCreateLockedAsyncJob(ExecutionEntity execution, boolean exclusive) {
        ...
        gregorianCalendar.add(
                Calendar.MILLISECOND, getAsyncExecutor().getAsyncJobLockTimeInMillis());
    ...
    protected JobEntity createExecutableJobFromOtherJob(AbstractJobEntity job) {
        ...
        if (isAsyncExecutorActive()) {
            ...
            gregorianCalendar.setTime(processEngineConfiguration.getClock().getCurrentTime());
            gregorianCalendar.add(
                    Calendar.MILLISECOND, getAsyncExecutor().getTimerLockTimeInMillis());
