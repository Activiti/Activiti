...
public interface ProcessDefinitionQuery extends Query<ProcessDefinitionQuery, ProcessDefinition> {
   ...
   *
   * <p>Can also be used without any other criteria (ie. query.latest().list()), which will then
   * give all the latest versions of all the deployed process definitions.
   *
  ...
  /**
   * Order by the app version of the process definitions (needs to be followed by {@link #asc()} or
   * {@link #desc()}).
   */
  ProcessDefinitionQuery orderByProcessDefinitionAppVersion();

  /**
   * Order by the name of the process definitions (needs to be followed by {@link #asc()} or {@link
   * #desc()}).
   */
