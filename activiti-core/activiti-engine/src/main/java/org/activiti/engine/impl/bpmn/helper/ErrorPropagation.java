...
public class ErrorPropagation {
    ...
    public static void propagateError(String errorRef, DelegateExecution execution) {
        Map<String, List<Event>> eventMap = findCatchingEventsForProcess(
            execution.getProcessDefinitionId(), errorRef);
        if (eventMap.size() > 0) {
            executeCatch(eventMap, execution, errorRef);
        } else if (!execution.getProcessInstanceId()
            ...
            ExecutionEntity processInstanceExecution = executionEntityManager.findById(
            ...
            if (processInstanceExecution != null) {
                ...
                while (parentExecution != null && eventMap.size() == 0) {
                    eventMap = findCatchingEventsForProcess(
                        parentExecution.getProcessDefinitionId(), errorRef);
                    if (eventMap.size() > 0) {

                        for (String processInstanceId : toDeleteProcessInstanceIds) {
                            ExecutionEntity processInstanceEntity = executionEntityManager.findById(
                                processInstanceId);
                            ...
                            executionEntityManager.deleteProcessInstanceExecutionEntity(
                                ...
                                execution.getCurrentFlowElement() != null
                                    ? execution.getCurrentFlowElement().getId() : null,
                            ...
                            if (Context.getProcessEngineConfiguration() != null
                                ...
                                .isEnabled()) {
                                Context.getProcessEngineConfiguration().getEventDispatcher()
                                    .dispatchEvent(ActivitiEventBuilder.createEntityEvent(
                        ...
                        } else if (!parentExecution.getId().equals(
                            parentExecution.getRootProcessInstanceId())) { // stop at the root
        ...
        if (eventMap.size() == 0) {
            throw new BpmnError(errorRef,
                "No catching boundary event found for error with errorCode '" + errorRef
                    + "', neither in same process nor in parent process");
        }
    }

    protected static void executeCatch(Map<String, List<Event>> eventMap,
        DelegateExecution delegateExecution, String errorId) {
        ...
        if (eventMap.containsKey(currentExecution.getActivityId())) {
            ...
            if (currentExecution.getParentId() != null && currentExecution.getParent()
                .isMultiInstanceRoot()) {
            ...
            while (matchingEvent == null && parentExecution != null) {
                ...
                if (parentExecution.getCurrentFlowElement() instanceof FlowElementsContainer) {
                    ...
                    currentContainer = ProcessDefinitionUtil.getProcess(
                ...
                if (matchingEvent == null) {
                    if (eventMap.containsKey(parentExecution.getActivityId())) {
                        ...
                        // Check for multi instance
                        if (parentExecution.getParentId() != null && parentExecution.getParent()
                            .isMultiInstanceRoot()) {
    ...
    protected static void executeEventHandler(Event event, ExecutionEntity parentExecution,
        ExecutionEntity currentExecution, String errorId) {
        if (Context.getProcessEngineConfiguration() != null
            ...
            if (bpmnModel != null) {
                ...
                Context.getProcessEngineConfiguration().getEventDispatcher().dispatchEvent(
                    ActivitiEventBuilder.createErrorEvent(ActivitiEventType.ACTIVITY_ERROR_RECEIVED,
                        event.getId(), errorId, errorCode, parentExecution.getId(),
        ...
        if (event instanceof StartEvent) {
            ...
            ExecutionEntity eventSubProcessExecution = executionEntityManager.createChildExecution(
                parentExecution);
    ...
    protected static Map<String, List<Event>> findCatchingEventsForProcess(
        ...
        List<EventSubProcess> subProcesses = process.findFlowElementsOfType(EventSubProcess.class,
            true);
            ...
            for (FlowElement flowElement : eventSubProcess.getFlowElements()) {
                if (flowElement instanceof StartEvent) {
                    ...
                    if (CollectionUtil.isNotEmpty(startEvent.getEventDefinitions())
                        ...
                        ErrorEventDefinition errorEventDef = (ErrorEventDefinition) startEvent.getEventDefinitions()
                            .get(0);
                        String eventErrorCode = retrieveErrorCode(bpmnModel,
                            errorEventDef.getErrorRef());
        ...
        for (BoundaryEvent boundaryEvent : boundaryEvents) {
            if (boundaryEvent.getAttachedToRefId() != null && CollectionUtil.isNotEmpty(
                boundaryEvent.getEventDefinitions()) && boundaryEvent.getEventDefinitions()
                .get(0) instanceof ErrorEventDefinition) {

                ErrorEventDefinition errorEventDef = (ErrorEventDefinition) boundaryEvent.getEventDefinitions()
                    .get(0);
                String eventErrorCode = retrieveErrorCode(bpmnModel, errorEventDef.getErrorRef());

                if (eventErrorCode == null || compareErrorCode == null || eventErrorCode.equals(
                    compareErrorCode)) {
    ...
    public static boolean mapException(Exception e, ExecutionEntity execution,
    ...
    protected static String findMatchingExceptionMapping(Exception e,
        List<MapExceptionEntry> exceptionMap) {
        ...
        for (MapExceptionEntry me : exceptionMap) {
            ...
            if (StringUtils.isNotEmpty(errorCode) && StringUtils.isEmpty(exceptionClass)
