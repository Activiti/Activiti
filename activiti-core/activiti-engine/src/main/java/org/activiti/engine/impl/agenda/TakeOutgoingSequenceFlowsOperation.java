...
public class TakeOutgoingSequenceFlowsOperation extends AbstractOperation {
  ...
  public TakeOutgoingSequenceFlowsOperation(
      CommandContext commandContext, ExecutionEntity executionEntity, boolean evaluateConditions) {
  ...
  protected void leaveFlowNode(FlowNode flowNode) {
    ...
    if (outgoingSequenceFlows.size() == 0
        && evaluateConditions) { // The elements that set this to false also have no support for
  ...
  protected void cleanupExecutions(FlowElement currentFlowElement) {
    if (execution.getParentId() != null && execution.isScope()) {
      ...
      // If the current activity is an activity, we need to remove any currently active boundary
      // events
      ...
      if (CollectionUtil.isNotEmpty(activity.getBoundaryEvents())) {
        ...
        for (ExecutionEntity childExecution : childExecutions) {
          if (childExecution.getCurrentFlowElement() == null
              || !notToDeleteEvents.contains(childExecution.getCurrentFlowElement().getId())) {
            commandContext
                .getExecutionEntityManager()
