...
public class ScopeUtil {
    ...
    public static void throwCompensationEvent(
        ...
        for (EventSubscriptionEntity eventSubscription : eventSubscriptions) {
            ...
            // check whether compensating execution is already created (which is the case when
            // compensating an embedded subprocess,
            // where the compensating execution is created when leaving the subprocess and holds
            // snapshot
            // data).
            if (eventSubscription.getConfiguration() != null) {
                compensatingExecution =
                        executionEntityManager.findById(eventSubscription.getConfiguration());
                compensatingExecution.setParent(compensatingExecution.getProcessInstance());
                compensatingExecution.setEventScope(false);
            } else {
                compensatingExecution =
                        executionEntityManager.createChildExecution((ExecutionEntity) execution);
        ...
        Collections.sort(
                eventSubscriptions,
                new Comparator<EventSubscriptionEntity>() {
    ...
    /**
     * Creates a new event scope execution and moves existing event subscriptions to this new
     * execution
     */
    public static void createCopyOfSubProcessExecutionForCompensation(
        ...
        List<EventSubscriptionEntity> eventSubscriptions =
                eventSubscriptionEntityManager.findEventSubscriptionsByExecutionAndType(
                        subProcessExecution.getId(), "compensate");
        ...
        if (CollectionUtil.isNotEmpty(compensateEventSubscriptions)) {
            ...
            ExecutionEntity processInstanceExecutionEntity =
                    subProcessExecution.getProcessInstance();
            ...
            new SubProcessVariableSnapshotter()
                    .setVariablesSnapshots(subProcessExecution, eventScopeExecution);

            // set event subscriptions to the event scope execution:
            for (CompensateEventSubscriptionEntity eventSubscriptionEntity :
                    compensateEventSubscriptions) {
