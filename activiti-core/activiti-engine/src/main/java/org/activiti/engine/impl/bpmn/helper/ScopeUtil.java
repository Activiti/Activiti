...
public class ScopeUtil {
    ...
    public static void throwCompensationEvent(
        List<CompensateEventSubscriptionEntity> eventSubscriptions, DelegateExecution execution,
        boolean async) {
        ...
        for (EventSubscriptionEntity eventSubscription : eventSubscriptions) {
            ...
            if (eventSubscription.getConfiguration() != null) {
                compensatingExecution = executionEntityManager.findById(
                    eventSubscription.getConfiguration());
                compensatingExecution.setParent(compensatingExecution.getProcessInstance());
                compensatingExecution.setEventScope(false);
            } else {
                compensatingExecution = executionEntityManager.createChildExecution(
    ...
    public static void createCopyOfSubProcessExecutionForCompensation(
        ...
        if (CollectionUtil.isNotEmpty(compensateEventSubscriptions)) {
            ...
            ExecutionEntity eventScopeExecution = Context.getCommandContext()
                .getExecutionEntityManager().createChildExecution(processInstanceExecutionEntity);
            ...
            new SubProcessVariableSnapshotter().setVariablesSnapshots(subProcessExecution,
                eventScopeExecution);
