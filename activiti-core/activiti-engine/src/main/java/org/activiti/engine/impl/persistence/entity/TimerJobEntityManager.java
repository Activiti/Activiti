...
public interface TimerJobEntityManager extends EntityManager<TimerJobEntity> {
  ...
  /**
   * Insert the {@link TimerJobEntity}, similar to {@link #insert(TimerJobEntity)}, but returns a
   * boolean in case the insert did not go through. This could happen if the execution related to
   * the {@link TimerJobEntity} has been removed (for example due to a task complete for a timer
   * boundary on that task).
   */
  boolean insertTimerJobEntity(TimerJobEntity timerJobEntity);

  /**
   * Returns the {@link TimerJobEntity} instances that are elegible to execute, meaning the due date
   * of the timer has been passed.
   */
   ...
   *
   * <p>This is for example used when deleting a process definition: it finds the {@link
   * TimerJobEntity} representing the timer start events.
   */
  List<TimerJobEntity> findJobsByTypeAndProcessDefinitionId(
      String type, String processDefinitionId);

  /**
   * The same as {@link #findJobsByTypeAndProcessDefinitionId(String, String)}, but by key and for a
   * specific tenantId.
   */
  List<TimerJobEntity> findJobsByTypeAndProcessDefinitionKeyAndTenantId(
      String type, String processDefinitionKey, String tenantId);

  /**
   * The same as {@link #findJobsByTypeAndProcessDefinitionId(String, String)}, but by key and
   * specifically for the 'no tenant' mode.
   */
  ...
  /**
   * Same as {@link #findJobsByQueryCriteria(TimerJobQueryImpl, Page)}, but only returns a count and
   * not the instances itself.
   */
  long findJobCountByQueryCriteria(TimerJobQueryImpl jobQuery);

  /**
   * Creates a new {@link TimerJobEntity}, typically when a timer is used in a repeating way. The
   * returns {@link TimerJobEntity} is not yet inserted.
   *
   * <p>Returns null if the timer has finished its repetitions.
   */
