...
public class DefaultMessageExecutionContext implements MessageExecutionContext {
  ...
  public MessageEventSubscriptionEntity createMessageEventSubscription(
    ...
    correlationKey.ifPresent(
        key -> assertNoExistingDuplicateEventSubscriptions(messageName, key, commandContext));
  ...
  protected String evaluateExpression(String expression, DelegateExecution execution) {
    return Optional.ofNullable(expressionManager.createExpression(expression))
        ...
        .map(Object::toString)
        .orElseThrow(
            () -> new ActivitiIllegalArgumentException("Expression '" + expression + "' is null"));
  }

  protected void assertNoExistingDuplicateEventSubscriptions(
      String messageName, String correlationKey, CommandContext commandContext) {

    List<EventSubscriptionEntity> existing =
        commandContext
            .getEventSubscriptionEntityManager()
