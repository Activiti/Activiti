...
public class DefaultAsyncJobExecutor implements AsyncExecutor {
    ...
    protected Runnable createRunnableForJob(final Job job) {
        if (executeAsyncRunnableFactory == null) {
            ...
            return executeAsyncRunnableFactory.createExecuteAsyncRunnable(job,
    ...
    protected void initAsyncJobExecutionThreadPool() {
        ...
        if (executorService == null) {
            ...
            BasicThreadFactory threadFactory = new BasicThreadFactory.Builder().namingPattern(
                "activiti-async-job-executor-thread-%d").build();
            ...
            try {
                if (!executorService.awaitTermination(secondsToWaitOnShutdown, TimeUnit.SECONDS)) {
                    log.warn("Timeout during shutdown of async job executor. "
                        + "The current running jobs could not end within " + secondsToWaitOnShutdown
    ...
    protected void stopResetExpiredJobsThread() {
        if (resetExpiredJobThread != null) {
            try {
                ...
                log.warn("Interrupted while waiting for the reset expired jobs thread to terminate",
                    e);
            }
