...
public class BpmnParse implements BpmnXMLConstants {
    ...
    /**
     * Mapping containing values stored during the first phase of parsing since other elements can
     * reference these messages.
     *
     * <p>All the map's elements are defined outside the process definition(s), which means that
    ...
    public BpmnParse execute() {
        try {
            ...
            if (encoding != null) {
                ...
                bpmnModel =
                        converter.convertToBpmnModel(
                                streamSource, validateSchema, enableSafeBpmnXml);
            }
            ...
            if (validateProcess) {
                ...
                if (processValidator == null) {
                    LOGGER.warn(
                            "Process should be validated, but no process validator is configured on"
    ...
    public void processFlowElements(Collection<FlowElement> flowElements) {
        ...
        for (FlowElement flowElement : flowElements) {
            ...
            // Sequence flow are also flow elements, but are only parsed once every activity is
            // found
            ...
            for (String bpmnReference : bpmnModel.getLocationMap().keySet()) {
                if (bpmnModel.getFlowElement(bpmnReference) == null) {
                    ...
                    if (bpmnModel.getArtifact(bpmnReference) == null) {
                        ...
                        if (bpmnModel.getPool(bpmnReference) == null
                                && bpmnModel.getLane(bpmnReference) == null) {
                            LOGGER.warn(
                                    "Invalid reference in diagram interchange definition: could not"
                                            + " find "
                                            + bpmnReference);
                        }
                    }
