 ...
 * <p>
 * For CRON strings DSTZONE is used as the time zone that the CRON schedule refers to. Leave it out
 * to use the server time zone.
 * <p>
 * For ISO strings the time zone offset for the date/time specified is part of the string itself.
 ...
 * behavior.
 * <p>
 * Schedule strings are versioned. Version 1 strings will use the original Activiti
...
public class AdvancedCycleBusinessCalendar extends CycleBusinessCalendar {
    ...
    public Date resolveDuedate(String duedateDescription, int maxIterations) {
        ...
        duedateDescription = removeValueFrom("VER",
            removeValueFrom("START", removeValueFrom("DSTZONE", duedateDescription))).trim();

        try {
            logger.info("Base Due Date: " + duedateDescription);

            Date date = resolvers.get(
                    version == null ? getDefaultScheduleVersion() : Integer.valueOf(version))
                .resolve(duedateDescription, clockReader,
                    timeZone == null ? clockReader.getCurrentTimeZone()
