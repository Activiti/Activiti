    ...
    protected void insertMessageEvent(MessageEventDefinition messageEventDefinition,
        ...
        for (EventSubscriptionEntity eventSubscriptionEntity : subscriptionsForSameMessageName) {
            ...
            if (eventSubscriptionEntity.getProcessInstanceId() == null
                || eventSubscriptionEntity.getProcessInstanceId()
                .isEmpty()) { // processInstanceId != null or not empty -> it's a message related to an execution
                // the event subscription has no instance-id, so it's a message start event
                throw new ActivitiException(
                    "Cannot deploy process definition '" + processDefinition.getResourceName()
                        + "': there already is a message event subscription for the message with name '"
                        + messageEventDefinition.getMessageRef() + "'.");
            }
        }

        MessageEventSubscriptionEntity newSubscription = commandContext.getEventSubscriptionEntityManager()
                ...
                if (element instanceof StartEvent) {
                    ...
                    if (CollectionUtil.isNotEmpty(startEvent.getEventDefinitions())) {
                        ...
                        if (eventDefinition instanceof SignalEventDefinition) {
                            ...
                            SignalEventSubscriptionEntity subscriptionEntity = commandContext.getEventSubscriptionEntityManager()
