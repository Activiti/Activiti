...
/**
 * Class that provides helper operations for use in the {@link JavaDelegate}, {@link
 * ActivityBehavior}, {@link ExecutionListener} and {@link TaskListener} interfaces.
 */
public class DelegateHelper {

    /**
     * To be used in an {@link ActivityBehavior} or {@link JavaDelegate}: leaves according to the
     * default BPMN 2.0 rules: all sequenceflow with a condition that evaluates to true are
     * followed.
     */
    ...
    /**
     * To be used in an {@link ActivityBehavior} or {@link JavaDelegate}: leaves the current
     * activity via one specific sequenceflow.
     */
        ...
        if (flowElement instanceof SequenceFlow) {
            ...
            Context.getAgenda()
                    .planTakeOutgoingSequenceFlowsOperation(
                            (ExecutionEntity) delegateExecution, false);
        } else {
    ...
    /**
     * Returns the {@link BpmnModel} matching the process definition bpmn model for the process
     * definition of the passed {@link DelegateExecution}.
     */
    ...
    /**
     * Returns whether or not the provided execution is being use for executing an {@link
     * ExecutionListener}.
     */
    ...
    /**
     * Returns for the activityId of the passed {@link DelegateExecution} the {@link Map} of {@link
     * ExtensionElement} instances. These represent the extension elements defined in the BPMN 2.0
     * XML as part of that particular activity.
     *
     * <p>If the execution is currently being used for executing an {@link ExecutionListener}, the
     * extension elements of the listener will be used. Use the {@link
     * #getFlowElementExtensionElements(DelegateExecution)} or {@link
     * #getListenerExtensionElements(DelegateExecution)} instead to specifically get the extension
     * elements of either the flow element or the listener.
     */
    public static Map<String, List<ExtensionElement>> getExtensionElements(
    ...
    public static Map<String, List<ExtensionElement>> getFlowElementExtensionElements(
            DelegateExecution execution) {
        return getFlowElement(execution).getExtensionElements();
    }

    public static Map<String, List<ExtensionElement>> getListenerExtensionElements(
            DelegateExecution execution) {
        return execution.getCurrentActivitiListener().getExtensionElements();
    }

    /**
     * Returns the list of field extensions, represented as instances of {@link FieldExtension}, for
     * the current activity of the passed {@link DelegateExecution}.
     *
     * <p>If the execution is currently being used for executing an {@link ExecutionListener}, the
     * fields of the listener will be returned. Use {@link #getFlowElementFields(DelegateExecution)}
     * or {@link #getListenerFields(DelegateExecution)} if needing the flow element of listener
     * fields specifically.
     */
    ...
    /**
     * Returns the {@link FieldExtension} matching the provided 'fieldName' which is defined for the
     * current activity of the provided {@link DelegateExecution}.
     *
     * <p>Returns null if no such {@link FieldExtension} can be found.
     *
     * <p>If the execution is currently being used for executing an {@link ExecutionListener}, the
     * field of the listener will be returned. Use {@link #getFlowElementField(DelegateExecution,
     * String)} or {@link #getListenerField(DelegateExecution, String)} for specifically getting the
     * field from either the flow element or the listener.
     */
    ...
    public static FieldExtension getFlowElementField(
        ...
        for (FieldExtension fieldExtension : fieldExtensions) {
            if (fieldExtension.getFieldName() != null
                    && fieldExtension.getFieldName().equals(fieldName)) {
    ...
    /**
     * Returns the {@link Expression} for the field defined for the current activity of the provided
     * {@link DelegateExecution}.
     *
     * <p>Returns null if no such field was found in the process definition xml.
     *
     * <p>If the execution is currently being used for executing an {@link ExecutionListener}, it
     * will return the field expression for the listener. Use {@link
     * #getFlowElementFieldExpression(DelegateExecution, String)} or {@link
     * #getListenerFieldExpression(DelegateExecution, String)} for specifically getting the flow
     * element or listener field expression.
     */
    ...
    /**
     * Similar to {@link #getFieldExpression(DelegateExecution, String)}, but for use within a
     * {@link TaskListener}.
     */
    public static Expression getFieldExpression(DelegateTask task, String fieldName) {
        if (task.getCurrentActivitiListener() != null) {
            List<FieldExtension> fieldExtensions =
                    task.getCurrentActivitiListener().getFieldExtensions();
            if (fieldExtensions != null && fieldExtensions.size() > 0) {
                for (FieldExtension fieldExtension : fieldExtensions) {
    ...
    public static Expression getFlowElementFieldExpression(
            DelegateExecution execution, String fieldName) {
    ...
    public static Expression getListenerFieldExpression(
            DelegateExecution execution, String fieldName) {
