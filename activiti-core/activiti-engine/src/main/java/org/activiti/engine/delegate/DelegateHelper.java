...
public class DelegateHelper {
    ...
    public static void leaveDelegate(DelegateExecution delegateExecution) {
        Context.getAgenda()
            .planTakeOutgoingSequenceFlowsOperation((ExecutionEntity) delegateExecution,
                true);
    }
     ...
     * <p>
     * If the execution is currently being used for executing an {@link ExecutionListener}, the
     * extension elements of the listener will be used. Use the {@link
     * #getFlowElementExtensionElements(DelegateExecution)} or {@link #getListenerExtensionElements(DelegateExecution)}
     * instead to specifically get the extension elements of either the flow element or the
     * listener.
     */
    ...
    public static FieldExtension getFlowElementField(DelegateExecution execution,
        ...
        for (FieldExtension fieldExtension : fieldExtensions) {
            if (fieldExtension.getFieldName() != null && fieldExtension.getFieldName()
                .equals(fieldName)) {
     ...
     * <p>
     * If the execution is currently being used for executing an {@link ExecutionListener}, it will
     * return the field expression for the listener. Use {@link #getFlowElementFieldExpression(DelegateExecution,
     * String)} or {@link #getListenerFieldExpression(DelegateExecution, String)} for specifically
    ...
    public static Expression getFieldExpression(DelegateTask task,
        ...
        if (task.getCurrentActivitiListener() != null) {
            List<FieldExtension> fieldExtensions = task.getCurrentActivitiListener()
                .getFieldExtensions();
            if (fieldExtensions != null && fieldExtensions.size() > 0) {
                for (FieldExtension fieldExtension : fieldExtensions) {
