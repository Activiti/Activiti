...
public interface HistoricDetailQuery extends Query<HistoricDetailQuery, HistoricDetail> {
    ...
    /**
     * Only select historic variable updates with the given process instance. {@link
    ...
    /**
     * Only select historic variable updates with the given execution. Note that {@link Execution}
    ...
    /**
     * Exclude all task-related {@link HistoricDetail}s, so only items which have no task-id set
     * will be selected. When used together with {@link #taskId(String)}, this call is ignored task
