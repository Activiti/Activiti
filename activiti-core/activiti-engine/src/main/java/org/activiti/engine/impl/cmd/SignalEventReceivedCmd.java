...
public class SignalEventReceivedCmd implements Command<Void> {
    ...
    public SignalEventReceivedCmd(String eventName, String executionId,
    ...
    public Void execute(CommandContext commandContext) {
        ...
        if (executionId == null) {
            signalEvents = eventSubscriptionEntityManager.findSignalEventSubscriptionsByEventName(
                eventName, tenantId);
        } else {
            ...
            if (execution.isSuspended()) {
                throw new ActivitiException(
                    "Cannot throw signal event '" + eventName + "' because execution '"
                        + executionId + "' is suspended");
            }

            signalEvents = eventSubscriptionEntityManager.findSignalEventSubscriptionsByNameAndExecution(
                eventName, executionId);

            if (signalEvents.isEmpty()) {
                throw new ActivitiException("Execution '" + executionId
                    + "' has not subscribed to a signal event with name '" + eventName + "'.");
            }
        }
        ...
        for (SignalEventSubscriptionEntity signalEventSubscriptionEntity : signalEvents) {
            ...
            if (signalEventSubscriptionEntity.isGlobalScoped()) {
                eventSubscriptionEntityManager.eventReceived(signalEventSubscriptionEntity, payload,
                    async);
            }
        }
