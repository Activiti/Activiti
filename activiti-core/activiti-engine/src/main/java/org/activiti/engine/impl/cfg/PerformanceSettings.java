...
public class PerformanceSettings {
    ...
    /**
     * Experimental setting: if true, whenever an execution is fetched from the data store, the
     * whole execution tree is fetched in the same roundtrip.
     *
     * <p>Less roundtrips to the database outweighs doing many, smaller fetches and often multiple
     * executions from the same tree are needed anyway when executing process instances.
     */
    protected boolean enableEagerExecutionTreeFetching;

    /**
     * Experimental setting: keeps a count on each execution that holds how many variables, jobs,
     * tasks, event subscriptions, etc. the execution has.
     *
     * <p>This makes the delete more performant as a query is not needed anymore to check if there
     * is related data. However, maintaining the count does mean more updates to the execution and
     * potentially more optimistic locking opportunities. Typically keeping the counts lead to
     * better performance as deletes are a large part of the execution tree maintenance.
     */
    ...
    /**
     * Experimental setting: in certain places in the engine (execution/process instance/historic
     * process instance/ tasks/data objects) localization is supported. When this setting is false,
