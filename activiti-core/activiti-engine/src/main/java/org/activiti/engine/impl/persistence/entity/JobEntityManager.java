...
public interface JobEntityManager extends EntityManager<JobEntity> {
     ...
     * <p>
     * For example used by the default {@link AcquireJobsCmd} command used by the default {@link
    ...
    /**
     * Returns all {@link JobEntity} instance which are expired, which means that the lock time of
     * the {@link JobEntity} is past a certain configurable date and is deemed to be in error.
     */
    ...
    /**
     * Same as {@link #findJobsByQueryCriteria(JobQueryImpl, Page)}, but only returns a count and
     * not the instances itself.
     */
    long findJobCountByQueryCriteria(JobQueryImpl jobQuery);

    /**
     * Resets an expired job. These are jobs that were locked, but not completed. Resetting these
     * will make them available for being picked up by other executors.
     */
