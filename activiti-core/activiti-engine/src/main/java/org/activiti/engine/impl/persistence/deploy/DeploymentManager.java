...
public class DeploymentManager {
    ...
    public ProcessDefinition findDeployedLatestProcessDefinitionByKeyAndTenantId(
        ...
        if (processDefinition == null) {
            throw new ActivitiObjectNotFoundException(
                "no processes deployed with key '" + processDefinitionKey
                    + "' for tenant identifier '" + tenantId + "'", ProcessDefinition.class);
        }
        processDefinition = resolveProcessDefinition(processDefinition).getProcessDefinition();
        return processDefinition;
    }
    ...
    public ProcessDefinition findDeployedProcessDefinitionByKeyAndVersionAndTenantId(
        ...
        ProcessDefinition processDefinition = processDefinitionEntityManager
            .findProcessDefinitionByKeyAndVersionAndTenantId(processDefinitionKey,
                processDefinitionVersion, tenantId);
        if (processDefinition == null) {
            throw new ActivitiObjectNotFoundException(
                "no processes deployed with key = '" + processDefinitionKey + "' and version = '"
    ...
    public ProcessDefinitionCacheEntry resolveProcessDefinition(
        ...
        if (cachedProcessDefinition == null) {
            ...
            cachedProcessDefinition = resolveProcessDefinitionInternal(commandContext,
                processDefinition, deploymentId, processDefinitionId);
        }
        return cachedProcessDefinition;
    }

    protected ProcessDefinitionCacheEntry resolveProcessDefinitionInternal(
        CommandContext commandContext, ProcessDefinition processDefinition, String deploymentId,
        ...
        if (cachedProcessDefinition == null) {
            throw new ActivitiException(
                "deployment '" + deploymentId + "' didn't put process definition '"
                    + processDefinitionId + "' in the cache");
        }
        return cachedProcessDefinition;
    }
    ...
    protected void removeDeploymentInternal(String deploymentId, boolean cascade,
