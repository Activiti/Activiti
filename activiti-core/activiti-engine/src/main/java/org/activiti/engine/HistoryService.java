...
public interface HistoryService {
    ...
    /**
     * Deletes historic task instance. This might be useful for tasks that are {@link
     * TaskService#newTask() dynamically created} and then {@link TaskService#complete(String)
     * completed}. If the historic task instance doesn't exist, no exception is thrown and the
    ...
    /**
     * Retrieves the {@link HistoricIdentityLink}s associated with the given task. Such an {@link
     * IdentityLink} informs how a certain identity (eg. group or user) is associated with a certain
     * task (eg. as candidate, assignee, etc.), even if the task is completed as opposed to {@link
     * IdentityLink}s which only exist for active tasks.
     */
    List<HistoricIdentityLink> getHistoricIdentityLinksForTask(String taskId);

    /**
     * Retrieves the {@link HistoricIdentityLink}s associated with the given process instance. Such
     * an {@link IdentityLink} informs how a certain identity (eg. group or user) is associated with
     * a certain process instance, even if the instance is completed as opposed to {@link
     * IdentityLink}s which only exist for active instances.
     */
