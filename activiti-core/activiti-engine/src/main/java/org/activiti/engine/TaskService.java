...
public interface TaskService {
     ...
     * persistent store, it is updated. After a new task has been saved, the task instance passed
     * into this method is updated with the id of the newly created task.
     *
     ...
     * @throws ActivitiException               when an error occurs while deleting the task or in
     *                                         case the task is part of a running process.
     */
     ...
     * to these tasks.
     *
     * @param taskIds The id's of the tasks that will be deleted, cannot be null. All id's in the
     *                list that don't have an existing task will be ignored.
     * @throws ActivitiObjectNotFoundException when one of the task does not exist.
     * @throws ActivitiException               when an error occurs while deleting the tasks or in
     *                                         case one of the tasks is part of a running process.
     */
     ...
     *
     * @param taskId  The id of the task that will be deleted, cannot be null. If no task exists
     *                with the given taskId, the operation is ignored.
     ...
     * to these tasks.
     *
     * @param taskIds      The id's of the tasks that will be deleted, cannot be null. All id's in
     *                     the list that don't have an existing task will be ignored.
     * @param deleteReason reason the task is deleted. Is recorded in history, if enabled.
     * @throws ActivitiObjectNotFoundException when one of the tasks does not exist.
     * @throws ActivitiException               when an error occurs while deleting the tasks or in
     *                                         case one of the tasks is part of a running process.
     ...
     * @throws ActivitiException               when an error occurs while deleting the tasks or in
     *                                         case one of the tasks is part of a running process.
     * @oaramn cancel mark the task as cancelled
     */
     ...
     * @throws ActivitiTaskAlreadyClaimedException when the task is already claimed by another
     *                                             user.
     */
     ...
     * @throws ActivitiException               when this task is {@link DelegationState#PENDING}
     *                                         delegation.
     */
     ...
     * method returns, the {@link Task#getDelegationState() delegationState} is set to {@link
    ...
    /**
     * Marks that the assignee is done with this task providing the required variables and that it
     * can be sent back to the owner. Can only be called when this task is {@link
     * DelegationState#PENDING} delegation. After this method returns, the {@link
     * Task#getDelegationState() delegationState} is set to {@link DelegationState#RESOLVED}.
     *
     * @param taskId
     * @param variables
     ...
     *                   process instance wide (which is the default for {@link #complete(String,
     *                   Map)}).
     * @throws ActivitiObjectNotFoundException when no task exists with the given id.
     */
    ...
    /**
     * Retrieves the {@link IdentityLink}s associated with the given task. Such an {@link
     * IdentityLink} informs how a certain identity (eg. group or user) is associated with a certain
     * task (eg. as candidate, assignee, etc.)
     */
     ...
     * task (including parent scopes). Returns null when no variable value is found with the given
     * name.
     *
     ...
     * and if available also the execution scopes.
     */
    ...
    /**
     * checks whether or not the task has a variable defined with the given name, local task scope
     * only.
     */
     ...
     * you have many variables and you only need a few, consider using {@link #getVariables(String,
     ...
     * (Does not searching parent scopes).
     *
     * @param taskId        id of taskId, cannot be null.
     * @param variableNames the collection of variable names that should be retrieved.
     * @return the variables or an empty map if no such variables are found.
     ...
     */
    Map<String, DataObject> getDataObjects(String taskId, String locale,
     ...
     * given task (including parent scopes). Returns null when no DataObject value is found with the
     * given name.
     *
     * @param taskId         id of task, cannot be null.
     ...
     * @throws ActivitiObjectNotFoundException when no task is found for the given taskId.
     */
    DataObject getDataObject(String taskId, String dataObjectName, String locale,
        boolean withLocalizationFallback);
    ...
    /**
     * Returns an individual event with the given id. Returns null if no event exists with the given
     * id.
     */
    ...
    /**
     * Add a new attachment to a task and/or a process instance and use an input stream to provide
     * the content
     */
