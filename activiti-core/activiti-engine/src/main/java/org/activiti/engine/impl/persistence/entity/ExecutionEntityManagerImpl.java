...
public class ExecutionEntityManagerImpl extends AbstractEntityManager<ExecutionEntity>
  ...
  /**
   * Processes a collection of {@link ExecutionEntity} instances, which form on execution tree. All
   * the executions share the same rootProcessInstanceId (which is provided). The return value will
   * be the root {@link ExecutionEntity} instance, with all child {@link ExecutionEntity} instances
   * populated and set using the {@link ExecutionEntity} instances from the provided collections
   */
  ...
  public ExecutionEntity createProcessInstanceExecution(
    ...
    if (authenticatedUserId != null) {
      getIdentityLinkEntityManager()
          .addIdentityLink(
              processInstanceExecution, authenticatedUserId, null, IdentityLinkType.STARTER);
    }
  ...
  public void deleteProcessInstancesByProcessDefinition(
    ...
    List<String> processInstanceIds =
        executionDataManager.findProcessInstanceIdsByProcessDefinitionId(processDefinitionId);
  ...
  protected void deleteProcessInstanceCascade(
        ...
        for (ExecutionEntity miExecutionEntity : subExecutionEntity.getExecutions()) {
          if (miExecutionEntity.getSubProcessInstance() != null) {
            deleteProcessInstanceCascade(
                miExecutionEntity.getSubProcessInstance(), deleteReason, deleteHistory);
          }
        }
    ...
    getHistoryManager()
        .recordProcessInstanceEnd(processInstanceExecutionEntity.getId(), deleteReason, null);
    ...
    if (getEventDispatcher().isEnabled()) {
      if (!cancel) {
        getEventDispatcher()
            .dispatchEvent(
                ActivitiEventBuilder.createEntityEvent(
                    ActivitiEventType.PROCESS_COMPLETED, processInstanceEntity));
      } else {
        getEventDispatcher()
            .dispatchEvent(
  ...
  private void deleteExecutionEntity(ExecutionEntity executionEntity, String deleteReason) {
        ...
        if (variableInstance instanceof VariableInstanceEntity) {
          ...
          if (variableInstanceEntity.getByteArrayRef() != null
            ...
            getByteArrayEntityManager()
                .deleteByteArrayById(variableInstanceEntity.getByteArrayRef().getId());
          }
        }
      }
    }
    ...
    if (!enableExecutionRelationshipCounts
      ...
      List<EventSubscriptionEntity> eventSubscriptions =
          eventSubscriptionEntityManager.findEventSubscriptionsByExecution(executionEntity.getId());
