...
public class ExecutionEntityManagerImpl
    ...
    public ExecutionEntity createProcessInstanceExecution(
        ...
        if (getEventDispatcher().isEnabled()) {
            getEventDispatcher()
                .dispatchEvent(
                    ActivitiEventBuilder.createEntityEvent(ActivitiEventType.ENTITY_CREATED, processInstanceExecution)
                );
        }
    ...
    public ExecutionEntity createChildExecution(ExecutionEntity parentExecutionEntity) {
        ...
        if (getEventDispatcher().isEnabled()) {
            getEventDispatcher()
                .dispatchEvent(
                    ActivitiEventBuilder.createEntityEvent(ActivitiEventType.ENTITY_CREATED, childExecution)
                );
            getEventDispatcher()
                .dispatchEvent(
                    ActivitiEventBuilder.createEntityEvent(ActivitiEventType.ENTITY_INITIALIZED, childExecution)
                );
        }
    ...
    private String getStartUserId(ExecutionEntity executionEntity) {
        return Optional
            .ofNullable(executionEntity)
            .map(ExecutionEntity::getStartUserId)
            .or(() -> Optional.ofNullable(executionEntity).map(ExecutionEntity::getParent).map(this::getStartUserId))
            .orElse(null);
    }
    ...
    public ExecutionEntity createSubprocessInstance(
        ...
        String authenticatedUserId = Optional
            .ofNullable(Authentication.getAuthenticatedUserId())
            .orElseGet(() -> getStartUserId(superExecutionEntity));
        ...
        if (authenticatedUserId != null) {
            getIdentityLinkEntityManager()
                .addIdentityLink(subProcessInstance, authenticatedUserId, null, IdentityLinkType.STARTER);
        }
    ...
    protected void deleteProcessInstanceCascade(ExecutionEntity execution, String deleteReason, boolean deleteHistory) {
                    ...
                    if (miExecutionEntity.getSubProcessInstance() != null) {
                        ...
                        for (ExecutionEntity miSubExecutionEntity : miExecutionEntity.getExecutions()) {
                            if (miSubExecutionEntity.getSubProcessInstance() != null) {
                                deleteProcessInstanceCascade(
                                    miSubExecutionEntity.getSubProcessInstance(),
                                    deleteReason,
                                    deleteHistory
                                );
                            }
        ...
        if (process != null && CollectionUtil.isNotEmpty(process.getExecutionListeners())) {
            executeExecutionListeners(process, processInstanceExecutionEntity, ExecutionListener.EVENTNAME_END);
        }
    ...
    private void cancelDataForExecution(ExecutionEntity executionEntity, String deleteReason) {
        ...
        if (
            ...
            getEventDispatcher()
                .dispatchEvent(ActivitiEventBuilder.createActivityCancelledEvent(executionEntity, deleteReason));
        }
    }
