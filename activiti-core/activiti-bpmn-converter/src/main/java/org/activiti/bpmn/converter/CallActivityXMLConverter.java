...
public class CallActivityXMLConverter extends BaseBpmnXMLConverter {
    ...
    protected BaseElement convertXMLToElement(XMLStreamReader xtr, BpmnModel model)
        ...
        callActivity.setBusinessKey(
                xtr.getAttributeValue(
                        ACTIVITI_EXTENSIONS_NAMESPACE, ATTRIBUTE_CALL_ACTIVITY_BUSINESS_KEY));
        callActivity.setInheritBusinessKey(
                Boolean.parseBoolean(
                        xtr.getAttributeValue(
                                ACTIVITI_EXTENSIONS_NAMESPACE,
                                ATTRIBUTE_CALL_ACTIVITY_INHERIT_BUSINESS_KEY)));
        callActivity.setInheritVariables(
                Boolean.valueOf(
                        xtr.getAttributeValue(
                                ACTIVITI_EXTENSIONS_NAMESPACE,
    ...
    protected void writeAdditionalAttributes(
        ...
        if (StringUtils.isNotEmpty(callActivity.getCalledElement())) {
            xtw.writeAttribute(
                    ATTRIBUTE_CALL_ACTIVITY_CALLEDELEMENT, callActivity.getCalledElement());
        }
        if (StringUtils.isNotEmpty(callActivity.getBusinessKey())) {
            writeQualifiedAttribute(
                    ATTRIBUTE_CALL_ACTIVITY_BUSINESS_KEY, callActivity.getBusinessKey(), xtw);
        }
    ...
    protected boolean writeExtensionChildElements(
        ...
        didWriteExtensionStartElement =
                writeIOParameters(
                        ELEMENT_CALL_ACTIVITY_IN_PARAMETERS,
                        callActivity.getInParameters(),
                        didWriteExtensionStartElement,
                        xtw);
        didWriteExtensionStartElement =
                writeIOParameters(
                        ELEMENT_CALL_ACTIVITY_OUT_PARAMETERS,
    ...
    private boolean writeIOParameters(
            String elementName,
        ...
        for (IOParameter ioParameter : parameterList) {
            ...
            xtw.writeStartElement(
                    ACTIVITI_EXTENSIONS_PREFIX, elementName, ACTIVITI_EXTENSIONS_NAMESPACE);
            ...
            if (StringUtils.isNotEmpty(ioParameter.getSourceExpression())) {
                writeDefaultAttribute(
                        ...
                        ioParameter.getSourceExpression(),
                        xtw);
            }
    ...
    public class InParameterParser extends BaseChildElementParser {
        ...
        public void parseChildElement(
            ...
            String sourceExpression =
                    xtr.getAttributeValue(null, ATTRIBUTE_IOPARAMETER_SOURCE_EXPRESSION);
