    ...
    public void parseChildElement(XMLStreamReader xtr, BaseElement parentElement, BpmnModel model)
        ...
        eventDefinition.setCorrelationKey(
                xtr.getAttributeValue(
                        ACTIVITI_EXTENSIONS_NAMESPACE, ATTRIBUTE_MESSAGE_CORRELATION_KEY));
        ...
        if (!StringUtils.isEmpty(eventDefinition.getMessageRef())) {
            ...
            if (indexOfP != -1) {
                ...
                if (resolvedNamespace == null) {
                    // if it's an invalid prefix will consider this is not a namespace prefix so
                    // will be used
                    // as part of the stringReference
                    messageRef = prefix + ":" + messageRef;
                } else if (!resolvedNamespace.equalsIgnoreCase(model.getTargetNamespace())) {
                    // if it's a valid namespace prefix but it's not the targetNamespace then we'll
                    // use it as
                    // a valid namespace
                    // (even out editor does not support defining namespaces it is still a valid xml
                    // file)
        ...
        BpmnXMLUtil.parseChildElements(
                ELEMENT_EVENT_MESSAGEDEFINITION, eventDefinition, xtr, model);
