...
public class StartEventJsonConverter
    ...
    protected String getStencilId(BaseElement baseElement) {
        ...
        if (event.getEventDefinitions().size() > 0) {
            EventDefinition eventDefinition = event
                .getEventDefinitions()
                .get(0);
    ...
    protected void convertElementToJson(
        ...
        if (StringUtils.isNotEmpty(startEvent.getInitiator())) {
            propertiesNode.put(
                PROPERTY_NONE_STARTEVENT_INITIATOR,
        ...
        if (StringUtils.isNotEmpty(startEvent.getFormKey())) {
            if (
                formKeyMap != null &&
                formKeyMap.containsKey(startEvent.getFormKey())
            ) {
    ...
    protected FlowElement convertJsonToElement(
        ...
        if (STENCIL_EVENT_START_NONE.equals(stencilId)) {
            ...
            if (StringUtils.isNotEmpty(formKey)) {
                ...
                if (
                    formReferenceNode != null &&
                    formReferenceNode.get("id") != null
                ) {
                    if (
                        formMap != null &&
                        formMap.containsKey(
                            formReferenceNode.get("id").asText()
                        )
                    ) {
