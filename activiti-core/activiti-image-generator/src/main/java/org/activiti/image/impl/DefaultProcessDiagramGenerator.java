...
/**
 * Class to generate a svg based the diagram interchange information in a BPMN 2.0 process.
 */
public class DefaultProcessDiagramGenerator implements ProcessDiagramGenerator {
  ...
  public DefaultProcessDiagramGenerator() {
    ...
    activityDrawInstructions.put(StartEvent.class,
      (processDiagramCanvas, bpmnModel, flowNode) -> {
        var graphicInfo = bpmnModel.getGraphicInfo(flowNode.getId());
        var startEvent = (StartEvent) flowNode;
        if (startEvent.getEventDefinitions() != null && !startEvent.getEventDefinitions()
          .isEmpty()) {
          var eventDefinition = startEvent.getEventDefinitions().getFirst();
          switch (eventDefinition) {
            case TimerEventDefinition ignored ->
            ...
            case MessageEventDefinition messageEventDefinition ->
              processDiagramCanvas.drawMessageStartEvent(flowNode.getId(),
                graphicInfo);
            case null, default -> processDiagramCanvas.drawNoneStartEvent(flowNode.getId(),
              graphicInfo);
          }
        } else {
    ...
    activityDrawInstructions.put(IntermediateCatchEvent.class,
      (processDiagramCanvas, bpmnModel, flowNode) -> {
        var graphicInfo = bpmnModel.getGraphicInfo(flowNode.getId());
        var intermediateCatchEvent = (IntermediateCatchEvent) flowNode;
    ...
    activityDrawInstructions.put(ThrowEvent.class,
      (processDiagramCanvas, bpmnModel, flowNode) -> {
        var graphicInfo = bpmnModel.getGraphicInfo(flowNode.getId());
        var throwEvent = (ThrowEvent) flowNode;
    ...
    activityDrawInstructions.put(EndEvent.class,
      (processDiagramCanvas, bpmnModel, flowNode) -> {
        var graphicInfo = bpmnModel.getGraphicInfo(flowNode.getId());
        var endEvent = (EndEvent) flowNode;
    ...
    activityDrawInstructions.put(ServiceTask.class,
      (processDiagramCanvas, bpmnModel, flowNode) -> {
        var graphicInfo = bpmnModel.getGraphicInfo(flowNode.getId());
        var serviceTask = (ServiceTask) flowNode;
    ...
    activityDrawInstructions.put(BoundaryEvent.class,
      (processDiagramCanvas, bpmnModel, flowNode) -> {
        var graphicInfo = bpmnModel.getGraphicInfo(flowNode.getId());
        var boundaryEvent = (BoundaryEvent) flowNode;
        if (boundaryEvent.getEventDefinitions() != null && !boundaryEvent.getEventDefinitions()
          .isEmpty()) {
          if (boundaryEvent.getEventDefinitions().getFirst() instanceof TimerEventDefinition) {

            processDiagramCanvas.drawCatchingTimerEvent(flowNode.getId(),
              flowNode.getName(),
              graphicInfo,
              boundaryEvent.isCancelActivity());
          } else if (boundaryEvent.getEventDefinitions()
            .getFirst() instanceof ErrorEventDefinition) {
            ...
            .getFirst() instanceof CompensateEventDefinition) {
    ...
    artifactDrawInstructions.put(TextAnnotation.class,
      (processDiagramCanvas, bpmnModel, artifact) -> {
        var graphicInfo = bpmnModel.getGraphicInfo(artifact.getId());
        var textAnnotation = (TextAnnotation) artifact;
    ...
    artifactDrawInstructions.put(Association.class,
      (processDiagramCanvas, bpmnModel, artifact) -> {
        var association = (Association) artifact;
        var sourceRef = association.getSourceRef();
        var targetRef = association.getTargetRef();

        // source and target can be an instance of FlowElement or Artifact
        ...
        var graphicInfoList = bpmnModel.getFlowLocationGraphicInfo(artifact.getId());
        ...
        for (int i = 1; i < graphicInfoList.size(); i++) {
          var graphicInfo = graphicInfoList.get(i);
          var previousGraphicInfo = graphicInfoList.get(i - 1);
        ...
        var associationDirection = association.getAssociationDirection();
  ...
  protected InputStream getDefaultDiagram(String diagramImageFileName) {
    var imageFileName = diagramImageFileName != null ?
      diagramImageFileName :
      getDefaultDiagramImageFileName();

    var imageStream = getClass().getResourceAsStream(imageFileName);
    if (imageStream == null) {
      throw new ActivitiImageException(
        "Error occurred while getting default diagram image from file: " + imageFileName);
    }
    return imageStream;
  }
  ...
  protected DefaultProcessDiagramCanvas generateProcessDiagram(BpmnModel bpmnModel,
    ...
    for (Pool pool : bpmnModel.getPools()) {
      var graphicInfo = bpmnModel.getGraphicInfo(pool.getId());
    ...
    for (Process process : bpmnModel.getProcesses()) {
      for (Lane lane : process.getLanes()) {
        var graphicInfo = bpmnModel.getGraphicInfo(lane.getId());
    ...
    // Draw artifacts
    for (var process : bpmnModel.getProcesses()) {

      for (var artifact : process.getArtifacts()) {
        drawArtifact(processDiagramCanvas,
          bpmnModel,
          artifact);
      }

      var subProcesses = process.findFlowElementsOfType(SubProcess.class,
        true);
      if (subProcesses != null) {
        for (var subProcess : subProcesses) {
          for (var subProcessArtifact : subProcess.getArtifacts()) {
            drawArtifact(processDiagramCanvas,
              bpmnModel,
              subProcessArtifact);
          }
  ...
  protected void prepareBpmnModel(BpmnModel bpmnModel) {
    ...
    var allGraphicInfos = new ArrayList<GraphicInfo>();
    ...
    if (bpmnModel.getFlowLocationMap() != null) {
      for (var flowGraphicInfos : bpmnModel.getFlowLocationMap().values()) {
  ...
  protected void drawActivity(DefaultProcessDiagramCanvas processDiagramCanvas,
    ...
    var drawInstruction = activityDrawInstructions.get(flowNode.getClass());
    if (drawInstruction != null) {
      ...
      if (flowNode instanceof Activity activity) {
        var multiInstanceLoopCharacteristics = activity.getLoopCharacteristics();
      ...
      if (highLightedActivities.contains(flowNode.getId())) {
        var flowNodeClass = flowNode.getClass();
        if (Event.class.isAssignableFrom(flowNodeClass)) {
          ...
          || CallActivity.class.isAssignableFrom(flowNodeClass)) {
          processDiagramCanvas
    ...
    for (SequenceFlow sequenceFlow : flowNode.getOutgoingFlows()) {
      ...
      var sourceRef = sequenceFlow.getSourceRef();
      var targetRef = sequenceFlow.getTargetRef();
      var sourceElement = bpmnModel.getFlowElement(sourceRef);
      var targetElement = bpmnModel.getFlowElement(targetRef);
      var graphicInfoList = bpmnModel.getFlowLocationGraphicInfo(
        sequenceFlow.getId());
      if (graphicInfoList != null && !graphicInfoList.isEmpty()) {
        ...
        var labelGraphicInfo = bpmnModel.getLabelGraphicInfo(sequenceFlow.getId());
   ...
   * @return
   */
  protected static List<GraphicInfo> connectionPerfectionizer(
  ...
  /**
   * This method returns shape type of base element.<br> Each element can be presented as rectangle,
   * rhombus, or ellipse.
   *
   * @param baseElement
  ...
  protected static DefaultProcessDiagramCanvas initProcessDiagramCanvas(BpmnModel bpmnModel,
    ...
    for (FlowNode flowNode : flowNodes) {
      ...
      for (SequenceFlow sequenceFlow : flowNode.getOutgoingFlows()) {
        List<GraphicInfo> graphicInfoList = bpmnModel.getFlowLocationGraphicInfo(
          sequenceFlow.getId());
