...
public class DefaultProcessDiagramCanvas {
    ...
    protected static final Logger LOGGER =
            LoggerFactory.getLogger(DefaultProcessDiagramCanvas.class);
     ...
     *
     * <p>Allows to specify minimal boundaries on the left and upper side of the canvas. This is
     * useful for diagrams that have white space there (eg Signavio). Everything beneath these
     * minimum values will be cropped.
     *
    ...
    public void drawCatchingEvent(
        ...
        Ellipse2D innerCircle =
                new Ellipse2D.Double(
                        innerCircleX, innerCircleY, innerCircleWidth, innerCircleHeight);
    ...
    protected void drawMultilineText(
        ...
        LineBreakMeasurer measurer =
                new LineBreakMeasurer(characterIterator, g.getFontRenderContext());
        ...
        while (measurer.getPosition() < characterIterator.getEndIndex()
            ...
            int height =
                    ((Float) (layout.getDescent() + layout.getAscent() + layout.getLeading()))
                            .intValue();

            if (currentHeight + height > boxHeight) {
                // The line we're about to add should NOT be added anymore, append three dots to
                // previous
                // one instead
    ...
    public void drawParallelGateway(String id, GraphicInfo graphicInfo) {
        ...
        Line2D.Double line =
                new Line2D.Double(
                        x + 10, y + height / 2, x + width - 10, y + height / 2); // horizontal
        g.draw(line);
    ...
    private static Point getEllipseIntersection(Shape shape, Line2D.Double line) {
        double angle = Math.atan2(line.y2 - line.y1, line.x2 - line.x1);
        double x =
                shape.getBounds2D().getWidth() / 2 * Math.cos(angle)
                        + shape.getBounds2D().getCenterX();
        double y =
                shape.getBounds2D().getHeight() / 2 * Math.sin(angle)
