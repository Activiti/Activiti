...
public class DefaultProcessDiagramCanvas {
  ...
  protected static Color HIGHLIGHT_CURRENT_COLOR = new Color(87, 255, 174);
  protected static Color HIGHLIGHT_COMPLETED_ACTIVITY_COLOR = new Color(51, 153, 255);
  protected static Color HIGHLIGHT_ERRORED_ACTIVITY_COLOR = new Color(255, 55, 87);
  ...
  protected static Stroke EVENT_SUBPROCESS_STROKE =
      new BasicStroke(
          1.0f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER, 1.0f, new float[] {1.0f}, 0.0f);
  protected static Stroke NON_INTERRUPTING_EVENT_STROKE =
      new BasicStroke(
          1.0f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER, 1.0f, new float[] {4.0f, 3.0f}, 0.0f);
  ...
  protected static Stroke ASSOCIATION_STROKE =
      new BasicStroke(
          2.0f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_MITER, 1.0f, new float[] {2.0f, 2.0f}, 0.0f);
   ...
   *
   * <p>Allows to specify minimal boundaries on the left and upper side of the canvas. This is
   * useful for diagrams that have white space there. Everything beneath these minimum values will
   * be cropped. It's also possible to pass a specific font name and a class loader for the icon
   * images.
   */
   ...
   *
   * <p>Allows to specify minimal boundaries on the left and upper side of the canvas. This is
   * useful for diagrams that have white space there (eg Signavio). Everything beneath these minimum
   * values will be cropped.
   *
   * @param minX Hint that will be used when generating the image. Parts that fall below minX on the
   *     horizontal scale will be cropped.
   * @param minY Hint that will be used when generating the image. Parts that fall below minX on the
   *     horizontal scale will be cropped.
   */
   ...
   *
   * <p>Throws an {@link ActivitiImageException} when {@link #close()} is already called.
   */
  ...
  public void drawStartEvent(String id, GraphicInfo graphicInfo, IconType icon) {
    ...
    if (icon != null) {
      int imageX =
          (int)
              Math.round(graphicInfo.getX() + (graphicInfo.getWidth() / 2) - (icon.getWidth() / 2));
      int imageY =
          (int)
              Math.round(
                  graphicInfo.getY() + (graphicInfo.getHeight() / 2) - (icon.getHeight() / 2));
  ...
  public void drawCatchingEvent(
      String id, GraphicInfo graphicInfo, boolean isInterrupting, IconType icon, String eventType) {
    ...
    if (icon != null) {
      // calculate coordinates to center image
      int imageX =
          (int) (graphicInfo.getX() + (graphicInfo.getWidth() / 2) - (icon.getWidth() / 2));
      int imageY =
          (int) (graphicInfo.getY() + (graphicInfo.getHeight() / 2) - (icon.getHeight() / 2));
  ...
  public void drawCatchingCompensateEvent(
  ...
  public void drawCatchingTimerEvent(
      String id, String name, GraphicInfo graphicInfo, boolean isInterrupting) {
  ...
  public void drawCatchingErrorEvent(
      String id, String name, GraphicInfo graphicInfo, boolean isInterrupting) {
  ...
  public void drawCatchingSignalEvent(
      String id, String name, GraphicInfo graphicInfo, boolean isInterrupting) {
  ...
  public void drawSequenceflow(
      int srcX, int srcY, int targetX, int targetY, boolean conditional, boolean highLighted) {
    Paint originalPaint = g.getPaint();
    if (highLighted) {
  ...
  public void drawAssociation(
    ...
    drawConnection(
        xPoints, yPoints, conditional, isDefault, "association", associationDirection, highLighted);
  }

  public void drawSequenceflow(
      int[] xPoints, int[] yPoints, boolean conditional, boolean isDefault, boolean highLighted) {
    drawConnection(
        xPoints,
        yPoints,
        conditional,
  ...
  public void drawSequenceflowWithoutArrow(
      int srcX, int srcY, int targetX, int targetY, boolean conditional) {
  ...
  public void drawTask(TaskIconType icon, String id, String name, GraphicInfo graphicInfo) {
    ...
    icon.drawIcon((int) graphicInfo.getX(), (int) graphicInfo.getY(), ICON_PADDING, g);
  }
  ...
  protected void drawMultilineAnnotationText(
      String text, int x, int y, int boxWidth, int boxHeight) {
    drawMultilineText(text, x, y, boxWidth, boxHeight, false);
  }

  protected void drawMultilineText(
      String text, int x, int y, int boxWidth, int boxHeight, boolean centered) {
    ...
    for (TextLayout textLayout : layouts) {
      ...
      currentX =
          x
              + (centered
                  ? ((boxWidth - ((Double) textLayout.getBounds().getWidth()).intValue()) / 2)
                  : 0);
  ...
  public void drawExpandedSubProcess(
      String id, String name, GraphicInfo graphicInfo, Class<?> type) {
  ...
  public void drawCollapsedSubProcess(
      String id, String name, GraphicInfo graphicInfo, Boolean isTriggeredByEvent) {
  ...
  protected void drawCollapsedTask(
      String id, String name, GraphicInfo graphicInfo, boolean thickBorder) {
  ...
  public void drawActivityMarkers(
      int x,
      int y,
      int width,
      int height,
  ...
  public void drawParallelGateway(String id, GraphicInfo graphicInfo) {
    ...
    Line2D.Double line =
        new Line2D.Double(x + 10, y + height / 2, x + width - 10, y + height / 2); // horizontal
    g.draw(line);
  ...
  public void drawInclusiveGateway(String id, GraphicInfo graphicInfo) {
    ...
    Ellipse2D.Double circle =
        new Ellipse2D.Double(
            ((width - diameter) / 2) + x, ((height - diameter) / 2) + y, diameter, diameter);
    g.draw(circle);
    g.setStroke(orginalStroke);
  ...
  private static Point getEllipseIntersection(Shape shape, Line2D.Double line) {
    double angle = Math.atan2(line.y2 - line.y1, line.x2 - line.x1);
    double x =
        shape.getBounds2D().getWidth() / 2 * Math.cos(angle) + shape.getBounds2D().getCenterX();
    double y =
        shape.getBounds2D().getHeight() / 2 * Math.sin(angle) + shape.getBounds2D().getCenterY();
    Point p = new Point();
    p.setLocation(x, y);
    return p;
  }
  ...
  private static Point getLinesIntersection(Line2D a, Line2D b) {
    double d =
        (a.getX1() - a.getX2()) * (b.getY2() - b.getY1())
            - (a.getY1() - a.getY2()) * (b.getX2() - b.getX1());
    double da =
        (a.getX1() - b.getX1()) * (b.getY2() - b.getY1())
            - (a.getY1() - b.getY1()) * (b.getX2() - b.getX1());
    double ta = da / d;
    Point p = new Point();
    p.setLocation(
        a.getX1() + ta * (a.getX2() - a.getX1()), a.getY1() + ta * (a.getY2() - a.getY1()));
    return p;
  }
}
