...
public class EventSubprocessValidator extends ProcessLevelValidator {
    ...
    protected void executeValidation(
        ...
        List<EventSubProcess> eventSubprocesses =
                process.findFlowElementsOfType(EventSubProcess.class);
        for (EventSubProcess eventSubprocess : eventSubprocesses) {

            List<StartEvent> startEvents =
                    process.findFlowElementsInSubProcessOfType(eventSubprocess, StartEvent.class);
                ...
                if (startEvent.getEventDefinitions() != null
                    ...
                    if (!(eventDefinition instanceof org.activiti.bpmn.model.ErrorEventDefinition)
                        ...
                        addError(
                                ...
                                process,
                                eventSubprocess,
                                "start event of event subprocess must be of type 'error', 'message'"
                                        + " or 'signal'");
                    }
                }
            }
