...
public class FlowElementValidator extends ProcessLevelValidator {
    ...
    protected void handleConstraints(
            Process process, Activity activity, List<ValidationError> errors) {
        if (activity.getId() != null && activity.getId().length() > ID_MAX_LENGTH) {
            addError(
                    errors,
                    Problems.FLOW_ELEMENT_ID_TOO_LONG,
                    process,
                    activity,
                    "The id of a flow element must not contain more than "
                            + ID_MAX_LENGTH
                            + " characters");
        }
    }
        ...
        if (multiInstanceLoopCharacteristics != null) {
            ...
            if (StringUtils.isEmpty(multiInstanceLoopCharacteristics.getLoopCardinality())
                ...
                addError(
                        ...
                        process,
                        activity,
                        "Either loopCardinality or loopDataInputRef/activiti:collection must been"
                                + " set");
            }
        }
    }

    protected void handleDataAssociations(
            Process process, Activity activity, List<ValidationError> errors) {
