...
public class SendTaskValidator extends ExternalInvocationTaskValidator {
        ...
        for (SendTask sendTask : sendTasks) {
            ...
            if (StringUtils.isNotEmpty(sendTask.getType())) {
                ...
                if (!sendTask.getType().equalsIgnoreCase("mail")
                    ...
                    addError(
                            errors,
                            Problems.SEND_TASK_INVALID_TYPE,
                            process,
                            sendTask,
                ...
                if (sendTask.getType().equalsIgnoreCase("mail")) {
                    validateFieldDeclarationsForEmail(
                            process, sendTask, sendTask.getFieldExtensions(), errors);
                }
            }
    ...
    protected void verifyWebservice(
            BpmnModel bpmnModel, Process process, SendTask sendTask, List<ValidationError> errors) {
        if (ImplementationType.IMPLEMENTATION_TYPE_WEBSERVICE.equalsIgnoreCase(
                    ...
                    if (bpmnInterface.getOperations() != null
                        ...
                        for (Operation operation : bpmnInterface.getOperations()) {
                            if (operation.getId() != null
                                    && operation.getId().equals(sendTask.getOperationRef())) {
                                operationFound = true;
                            }
                        }
                    }
            ...
            if (!operationFound) {
                addError(
                        ...
                        process,
                        sendTask,
                        "Invalid operation reference for send task");
            }
        }
    }
}
