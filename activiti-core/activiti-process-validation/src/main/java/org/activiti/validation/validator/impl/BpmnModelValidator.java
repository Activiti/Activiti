...
public class BpmnModelValidator extends ValidatorImpl {
    ...
    public void validate(BpmnModel bpmnModel, List<ValidationError> errors) {
        ...
        // If at least one process definition is executable, show a warning for each of the
        // none-executables
            ...
            for (Process process : bpmnModel.getProcesses()) {
                if (!process.isExecutable()) {
                    addWarning(
                            ...
                            process,
                            process,
                            "Process definition is not executable. Please verify that this is"
                                    + " intentional.");
                }
    ...
    protected void handleProcessConstraints(
        ...
        if (process.getId() != null
                && process.getId().length() > Constraints.PROCESS_DEFINITION_ID_MAX_LENGTH) {
            addError(
                    errors,
        ...
        if (process.getName() != null
                && process.getName().length() > Constraints.PROCESS_DEFINITION_NAME_MAX_LENGTH) {
            addError(
                    errors,
    ...
    protected boolean validateAtLeastOneExecutable(
        ...
        if (nrOfExecutableDefinitions == 0) {
            addError(
                    ...
                    "All process definition are set to be non-executable (property 'isExecutable'"
                            + " on process). This is not allowed.");
        }
