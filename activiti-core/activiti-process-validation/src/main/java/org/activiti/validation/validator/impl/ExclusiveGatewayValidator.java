        ...
        if (exclusiveGateway.getOutgoingFlows().isEmpty()) {
            ...
            if (StringUtils.isNotEmpty(sequenceFlow.getConditionExpression())) {
                addError(
                        ...
                        process,
                        exclusiveGateway,
                        "Exclusive gateway has only one outgoing sequence flow. This is not allowed"
                                + " to have a condition.");
            }
        } else {
            ...
            for (SequenceFlow flow : exclusiveGateway.getOutgoingFlows()) {
                ...
                boolean isDefaultFlow =
                        flow.getId() != null && flow.getId().equals(defaultSequenceFlow);
            ...
            if (!flowsWithoutCondition.isEmpty()) {
                addWarning(
                        ...
                        process,
                        exclusiveGateway,
                        "Exclusive gateway has at least one outgoing sequence flow without a"
                                + " condition (which isn't the default one)");
            }
        }
    }
}
