...
public class ProcessRuntimeAutoConfiguration {
  ...
  /**
   * Creates default SignalPayloadEventListener bean if no existing bean found in
   * ApplicationContext.
   */
  @Bean
  ...
  /**
   * Creates default MessagePayloadEventListener bean if no existing bean found in
   * ApplicationContext.
   */
  @Bean
  ...
  public ReceiveMessagePayloadEventListener receiveMessagePayloadEventListener(
      RuntimeService runtimeService, ManagementService managementService) {
    ...
    return () ->
        runtimeService.addEventListener(
            new ProcessStartedListenerDelegate(
                getInitializedListeners(listeners), processStartedEventConverter),
    ...
    return () ->
        runtimeService.addEventListener(
            new ProcessSuspendedListenerDelegate(
                getInitializedListeners(eventListeners), converter),
    ...
    return () ->
        runtimeService.addEventListener(
            new SequenceFlowTakenListenerDelegate(
                getInitializedListeners(eventListeners), new ToSequenceFlowTakenConverter()),
