...
public class ToAPITaskCandidateUserAddedEventConverterTest {

    @InjectMocks
    private ToAPITaskCandidateUserAddedEventConverter converter;

    @Mock
    private APITaskCandidateUserConverter candidateConverter;
    ...
    public void from_should_propagateProcessInstanceAndDefinitionIds_when_taskBelongsToProcessInstance() {
        ...
        given(identityLink.getUserId()).willReturn("user1");
        ...
        TaskCandidateUser candidate = new TaskCandidateUserImpl("user1", "task1");
        given(candidateConverter.from(identityLink)).willReturn(candidate);

        TaskCandidateUserAddedEvent event = converter.from(internalEvent).orElse(null);
