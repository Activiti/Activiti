...
public class TaskAdminRuntimeImpl implements TaskAdminRuntime {
        ...
        if (task == null) {
            throw new IllegalStateException(
                    "Task with id: "
                            ...
                            + " cannot be completed because it cannot be found.");
        }

        taskRuntimeHelper.handleCompleteTaskPayload(completeTaskPayload);

        TaskImpl competedTaskData =
                new TaskImpl(task.getId(), task.getName(), Task.TaskStatus.COMPLETED);
    ...
    public Task assign(AssignTaskPayload assignTaskPayload) {
        ...
        taskService.claim(assignTaskPayload.getTaskId(), assignTaskPayload.getAssignee());
