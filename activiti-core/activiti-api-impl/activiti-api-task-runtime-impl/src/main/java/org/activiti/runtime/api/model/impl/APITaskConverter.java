...
public class APITaskConverter extends ListConverter<org.activiti.engine.task.Task, Task>
  ...
  private TaskImpl buildFromInternalTask(
      org.activiti.engine.task.Task internalTask, Task.TaskStatus status) {
  ...
  public Task fromWithCompletedBy(
      org.activiti.engine.task.Task internalTask, Task.TaskStatus status, String completedBy) {
  ...
  private void extractCandidateUsersAndGroups(
      org.activiti.engine.task.Task source, TaskImpl destination) {
  ...
  private List<String> extractCandidatesBy(
      List<IdentityLink> candidates, Function<IdentityLink, String> extractor) {
