...
public class ProcessDefinitionUserTask extends JsonDeserializer<List<ProcessDefinitionUserTask>> {
	...
	@JsonProperty("taskDocumentation") private String taskDocumentation;
	...
	@Override
	public List<ProcessDefinitionUserTask> deserialize(JsonParser jp, DeserializationContext ctxt)
	    ...
	    for(int i=0; i<nodes.size(); i++)
	    {
	    	ProcessDefinitionUserTask task = new ProcessDefinitionUserTask(
	    			nodes.get(i).get("taskName").asText(), nodes.get(i).get("taskDocumentation").asText());
	    	tasks.add(task);
	    }
