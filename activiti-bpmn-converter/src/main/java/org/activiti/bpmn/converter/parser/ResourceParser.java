        ...
        if (model.containsResourceId(resourceId)) {
            ...
            for ( org.activiti.bpmn.model.Process process : model.getProcesses() ) {
                for ( FlowElement flowElement : process.getFlowElements() ) {
                    if (flowElement instanceof UserTask && ((UserTask) flowElement).getCandidateGroups().contains(resourceId)) {
                        ((UserTask) flowElement).getCandidateGroups().remove(resourceId);
                        ((UserTask) flowElement).getCandidateGroups().add(resourceName);
                    }
                }
            }
        } else {
