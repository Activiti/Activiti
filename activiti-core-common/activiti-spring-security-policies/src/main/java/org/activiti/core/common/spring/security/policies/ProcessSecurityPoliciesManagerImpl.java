...
public class ProcessSecurityPoliciesManagerImpl extends BaseSecurityPoliciesManagerImpl
    implements ProcessSecurityPoliciesManager {

  private final SecurityPoliciesRestrictionApplier<GetProcessDefinitionsPayload>
      processDefinitionRestrictionApplier;

  private final SecurityPoliciesRestrictionApplier<GetProcessInstancesPayload>
  ...
  private Set<String> definitionKeysAllowedForApplicationPolicy(
    ...
    for (String appName : restrictions.keySet()) {
      ...
      if (appName != null
          && appName.replace("-", "").equalsIgnoreCase(applicationName.replace("-", ""))) {
  ...
  protected boolean anEntryInSetStartsKey(Set<String> keys, String processDefinitionKey) {
    for (String key : keys) {
      // override the base class with exact matching as startsWith is only preferable for audit
      // where id might be used that would start with key
