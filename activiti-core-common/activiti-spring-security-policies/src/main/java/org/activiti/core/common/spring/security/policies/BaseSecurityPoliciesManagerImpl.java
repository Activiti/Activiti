...
public abstract class BaseSecurityPoliciesManagerImpl
    ...
    public Map<String, Set<String>> getAllowedKeys(
        SecurityPolicyAccess... securityPoliciesAccess
    ) {
        ...
        for (SecurityPolicy ssp : policies) {
            ...
            if (
                isUserInPolicy(ssp, authenticatedUserId) ||
                isGroupInPolicy(ssp, groups)
            ) {
                // Here if securityPolicyAccess is READ, it should also include WRITES, if it is NONE nothing, and if it is WRITE only WRITE
                List<SecurityPolicyAccess> securityPolicyAccesses = asList(
                    securityPoliciesAccess
                );
                if (
                    securityPolicyAccesses.contains(SecurityPolicyAccess.WRITE)
                ) {
                    ...
                    securityPolicyAccesses.contains(SecurityPolicyAccess.READ)
                ) {
                    if (
    ...
    public boolean hasPermission(
        ...
        Map<String, Set<String>> policiesMap = getAllowedKeys(
        ...
        if (
            appName != null &&
            policiesMap.get(appName.replaceAll("-", "").toLowerCase()) != null
        ) {
            keys.addAll(
                policiesMap.get(appName.replaceAll("-", "").toLowerCase())
            );
        }

        return (
            anEntryInSetStartsKey(keys, processDefinitionKey) ||
