...
public abstract class BaseSecurityPoliciesManagerImpl implements SecurityPoliciesManager {
    ...
    public Map<String, Set<String>> getAllowedKeys(SecurityPolicyAccess... securityPoliciesAccess) {
        ...
        for (SecurityPolicy ssp : policies) {
            definitionKeysAllowedByPolicy.computeIfAbsent(
                    ssp.getServiceName(), k -> new HashSet<>());

            // I need to check that the user is listed in the user lists or that at least one of the
            // user
            // groups is in the group list
            if (isUserInPolicy(ssp, authenticatedUserId) || isGroupInPolicy(ssp, groups)) {

                // Here if securityPolicyAccess is READ, it should also include WRITES, if it is
                // NONE
                // nothing, and if it is WRITE only WRITE
                ...
                if (securityPolicyAccesses.contains(SecurityPolicyAccess.WRITE)) {
                    if (ssp.getAccess().equals(SecurityPolicyAccess.WRITE)) {
                        definitionKeysAllowedByPolicy
                                .get(ssp.getServiceName())
                                .addAll(ssp.getKeys());
                    }
    ...
    private boolean isUserInPolicy(SecurityPolicy ssp, String userId) {
        return (ssp.getUsers() != null
                && !ssp.getUsers().isEmpty()
                && ssp.getUsers().contains(userId));
    }
    ...
    public boolean hasPermission(
        ...
        // also factor for case sensitivity and hyphens (which are stripped when specified through
        // env
        // var)
    ...
    // startsWith logic supports the case of audit where only definition id might be available and
    // it
    // would start with the key
    // protected scope means we can override where exact matching more appropriate (consider keys
    // ProcessWithVariables and ProcessWithVariables2)
    // even for audit would be better if we had a known separator which cant be part of key - this
    // seems best we can do for now
