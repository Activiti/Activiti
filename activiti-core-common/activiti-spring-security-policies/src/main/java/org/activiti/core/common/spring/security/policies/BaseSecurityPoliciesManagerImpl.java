...
public abstract class BaseSecurityPoliciesManagerImpl implements SecurityPoliciesManager {
  ...
  public BaseSecurityPoliciesManagerImpl(
      SecurityManager securityManager, SecurityPoliciesProperties securityPoliciesProperties) {
  ...
  public Map<String, Set<String>> getAllowedKeys(SecurityPolicyAccess... securityPoliciesAccess) {
    ...
    for (SecurityPolicy ssp : policies) {
      ...
      // I need to check that the user is listed in the user lists or that at least one of the user
      // groups is in the group list
      if (isUserInPolicy(ssp, authenticatedUserId) || isGroupInPolicy(ssp, groups)) {

        // Here if securityPolicyAccess is READ, it should also include WRITES, if it is NONE
  ...
  public boolean hasPermission(
      String processDefinitionKey, SecurityPolicyAccess securityPolicyAccess, String appName) {
    ...
    // also factor for case sensitivity and hyphens (which are stripped when specified through env
    // var)
  ...
  // startsWith logic supports the case of audit where only definition id might be available and it
