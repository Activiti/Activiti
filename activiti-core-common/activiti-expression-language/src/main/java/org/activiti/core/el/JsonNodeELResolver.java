...
public class JsonNodeELResolver extends ELResolver {
   ...
   * the property argument. Otherwise, returns null. Assuming the base is not null, this method will
   * always return Object.class. This is because any object is accepted as a key and is coerced into
   * a string.
   *
  ...
  /**
   * If the base object is not null, returns an Iterator containing the set of JavaBeans properties
   * available on the given object. Otherwise, returns null. The Iterator returned must contain zero
   * or more instances of java.beans.FeatureDescriptor. Each info object contains information about
   * a property in the bean, as obtained by calling the BeanInfo.getPropertyDescriptors method. The
   ...
   *   <li>{@link ELResolver#RESOLVABLE_AT_DESIGN_TIME} - true.
   * </ul>
   *
   * @param context The context of this evaluation.
   * @param base The bean to analyze.
   ...
   * If the base is a Map, the propertyResolved property of the ELContext object must be set to true
   * by this resolver, before returning. If this property is not true after this method is called,
   * the caller should ignore the return value. Assuming the base is a Map, this method will always
   * return Object.class. This is because Maps accept any object as the value for a given key.
   *
   * @param context The context of this evaluation.
   ...
   * propertyResolved property of the ELContext object must be set to true by this resolver, before
   * returning. If this property is not true after this method is called, the caller should ignore
   * the return value. Just as in java.util.Map.get(Object), just because null is returned doesn't
   * mean there is no mapping for the key; it's also possible that the Map explicitly maps the key
   * to null.
   *
   * @param context The context of this evaluation.
   * @param base The map to analyze. Only bases of type Map are handled by this resolver.
   ...
   *     thrown by the underlying Map).
   * @throws NullPointerException if context is null, or if the key is null and this map does not
   *     permit null keys (the latter is optionally thrown by the underlying Map).
   * @throws ELException if an exception was thrown while performing the property or variable
   *     resolution. The thrown exception must be included as the cause property of this exception,
   *     if available.
   */
  ...
  /**
   * Returns the {@link ObjectMapper} used internally to convert {@link List} properties. Subclasses
   * may override this method to provide a specific one
   */
  ...
  /**
   * If the base object is a map, returns whether a call to {@link #setValue(ELContext, Object,
   * Object, Object)} will always fail. If the base is a Map, the propertyResolved property of the
   * ELContext object must be set to true by this resolver, before returning. If this property is
   * not true after this method is called, the caller should ignore the return value. If this
   * resolver was constructed in read-only mode, this method will always return true. If a Map was
   * created using java.util.Collections.unmodifiableMap(Map), this method must return true.
   * Unfortunately, there is no Collections API method to detect this. However, an implementation
   * can create a prototype unmodifiable Map and query its runtime type to see if it matches the
   * runtime type of the base object as a workaround.
   *
   * @param context The context of this evaluation.
   * @param base The map to analyze. Only bases of type Map are handled by this resolver.
   * @param property The key to return the acceptable type for. Ignored by this resolver.
   * @return If the propertyResolved property of ELContext was set to true, then true if calling the
   *     setValue method will always fail or false if it is possible that such a call may succeed;
   *     otherwise undefined.
   * @throws NullPointerException if context is null.
   * @throws ELException if an exception was thrown while performing the property or variable
   *     resolution. The thrown exception must be included as the cause property of this exception,
   ...
   * specified by the property argument. If the base is a Map, the propertyResolved property of the
   * ELContext object must be set to true by this resolver, before returning. If this property is
   * not true after this method is called, the caller can safely assume no value was set. If this
   * resolver was constructed in read-only mode, this method will always throw
   * PropertyNotWritableException. If a Map was created using
   * java.util.Collections.unmodifiableMap(Map), this method must throw
   * PropertyNotWritableException. Unfortunately, there is no Collections API method to detect this.
   * However, an implementation can create a prototype unmodifiable Map and query its runtime type
   * to see if it matches the runtime type of the base object as a workaround.
   *
   * @param context The context of this evaluation.
   * @param base The map to analyze. Only bases of type Map are handled by this resolver.
   * @param property The key to return the acceptable type for. Ignored by this resolver.
   ...
   *     stored in this map.
   * @throws NullPointerException if context is null, or if this map does not permit null keys or
