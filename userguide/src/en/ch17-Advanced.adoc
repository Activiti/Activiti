...
When working with more complicated statements XML Mapped Statements can be helpful. Suppose that for some use case the ability to query attachments data is needed based on id, name, type, userId, etc!. Since Activiti uses	XML Mapped Statements internally, it's possible to make use of the underlying capabilities. 

To fulfill the use case create a sub-class of  _org.activiti.engine.impl.AbstractQuery_ that can look as follows:   
...
Note that when extending _AbstractQuery_ extended classes should pass an instance of _ManagementService_ to super constructor and that methods _executeCount_ and _executeList_ need to be overrided to call the mapped statements. 
...
Capabilities such as pagination, ordering, table name prefixing are available and can be used in the statements (since the parameterType is a subclass of _AbstractQuery_). Note that to map results the internal resultMap (_org.activiti.engine.impl.persistence.entity.AttachmentEntity.attachmentResultMap_) can be used.

The mapper XML file must be provided to the Process Engine configuration as follows:
...
<property name="customMybatisXMLMappers">
  <set>
    <value>org/activiti/standalone/cfg/custom-mappers/AttachmentMapper.xml</value>
  </set>
</property>
...
---- 

Finally, the custom query can be used as follows:
