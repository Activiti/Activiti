...
* A in-memory datasource is created automatically (since the H2 driver is on the classpath) and passed to the Activiti process engine configuration
...
* The _startProcess_ now gets and assignee username in, which is used to look up the Person, and put the Person JPA object as a process variable in the process instance.
...
And lastly, to try out the Spring-JPA-Activiti integration, we assigne the task using the id of the Person JPA object in the process definition:
