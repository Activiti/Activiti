...
If booth are specified then the endDate specified as attribute will be used by the system.
...
What mentioned in the above tables, only holds for start side of the variable transfer in both directions from Camel to Activiti and vice versa. +
...
Just as an example in a route like
----
from("direct:start").to("activiti:process?var.return.exampleVar").to("mock:result");
----

Activiti variable with the name of exampleVar will be considered as output and will be copied back as a property in camel with the same name.
