...
<serviceTask id="javaService"
  ...
  <extensionElements>
    <activiti:field name="text">
        <activiti:string>
          This is a very long string with a lot of words and potentially way longer even!
      </activiti:string>
    </activiti:field>
  </extensionElements>
</serviceTask>
...
*Important: the JavaDelegateHelper cannot be used for an _ExecutionListener_ or _TaskListener_. To make thread-safe instances of those listeners, use either an expression or make sure a new instance is created every time the delegate expression is resolved.*
