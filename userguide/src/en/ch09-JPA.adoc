...
The next example configuration below provides a +EntityManagerFactory+ which we define ourselves (in this case, an Hibernate entity manager). Note that the snippet only contains the beans that are relevant for the example, the others are omitted. Full working example with Hibernate entity manager can be found in the activiti-spring tests (++/activiti-spring/src/test/java/org/activiti/spring/test/jpa/JPASpringTest.java++)
