...
Next, we need to add context.xml files that contain the default H2 configuration. These will be overridden by your JNDI configuration, if it exists.  For the Activiti UI app, add activiti-app/src/main/webapp/META-INF/context.xml with the following:
...
<Context antiJARLocking="true" path="/activiti-rest">
    <Resource auth="Container"
              name="jdbc/activitiDB"
              type="javax.sql.DataSource"
              scope="Shareable"
...
As an optional step, you can now delete the unused db.properties files in the Activiti REST webapp project.
...
The aysnc executor of Activiti 5 is the only available job executor in Activiti 5 as it was a more performant and more database friendly way of executing asynchronous jobs in the Activiti Engine.
The old job executor of Activiti 5 is removed. More information can be found in the advanced section of the user guide.
...
The property asyncExecutorActivate instructs the Activiti Engine to startup the Async executor thread pool at startup.
...
As of version 5.13, Activiti supports Mapped Diagnostic Contexts feature of SLF4j. These basic information are passed to the underlying logger along with what is going to be logged:
