...
/**
 * Parses a String into a {@link ValueExpression} or {@link MethodExpression}
 ...
 * {@link #createValueExpression(ELContext, String, Class)} method is used to
 * parse expressions that evaluate to values (both l-values and r-values are
 * supported). The
 * {@link #createMethodExpression(ELContext, String, Class, Class[])} method is
 ...
 * evaluation time, via the {@link ELResolver} associated with the
 * {@link ELContext} passed to the ValueExpression or MethodExpression. The
 * ELContext object also provides access to the {@link FunctionMapper} and
 ...
 * <ul>
 * <li>Single expressions using the ${} delimiter (e.g. "${employee.lastName}").
 * </li>
 * <li>Single expressions using the #{} delimiter (e.g. "#{employee.lastName}").
 * </li>
...
public abstract class ExpressionFactory {
     ...
     * <li>Use the properties file "lib/el.properties" in the JRE directory. If
     * this file exists and it is readable by the
     * java.util.Properties.load(InputStream) method, and it contains an entry
     * whose key is "javax.el.ExpressionFactory", then the value of that entry
     * is used as the name of the implementation class.</li>
     * <li>Use the javax.el.ExpressionFactory system property. If a system
     * property with this name is defined, then its value is used as the name of
     ...
     * newInstance(). If the argument properties is not null, and if the
     * implementation contains a constructor with a single parameter of type
     * java.util.Properties, then the constructor is used to create the
     * instance. Properties are optional and can be ignored by an
     * implementation. The name of a property should start with "javax.el." The
     * following are some suggested names for properties.
     * <ul>
     ...
     * @return An instance of ExpressionFactory.
     * @throws ELException
     *             if the class could not be found or if it is not a subclass of
     ...
     * rules. An {@link ELException} is thrown if an error results from applying
     ...
     * evaluation. Use this method for expressions that refer to methods. If the
     * expression is a String literal, a MethodExpression is created, which when
     * invoked, returns the String literal, coerced to expectedReturnType. An
     * ELException is thrown if expectedReturnType is void or if the coercion of
     * the String literal to the expectedReturnType yields an error (see Section
     * "1.16 Type Conversion"). This method should perform syntactic validation
     * of the expression. If in doing so it detects errors, it should raise an
     * ELException.
     * 
     * @param context
     *            The EL context used to parse the expression. The
     *            FunctionMapper and VariableMapper stored in the ELContext are
     *            used to resolve functions and variables found in the
     *            expression. They can be null, in which case functions or
     *            variables are not supported for this expression. The object
     *            returned must invoke the same functions and access the same
     *            variable mappings regardless of whether the mappings in the
     *            provided FunctionMapper and VariableMapper instances change
     *            between calling ExpressionFactory.createMethodExpression() and
     *            any method on MethodExpression. Note that within the EL, the
     *            ${} and #{} syntaxes are treated identically. This includes
     *            the use of VariableMapper and FunctionMapper at expression
     *            creation time. Each is invoked if not null, independent of
     *            whether the #{} or ${} syntax is used for the expression.
     * @param expression
     *            The expression to parse
     * @param expectedReturnType
     *            The expected return type for the method to be found. After
     *            evaluating the expression, the MethodExpression must check
     *            that the return type of the actual method matches this type.
     *            Passing in a value of null indicates the caller does not care
     *            what the return type is, and the check is disabled.
     * @param expectedParamTypes
     *            The expected parameter types for the method to be found. Must
     *            be an array with no elements if there are no parameters
     *            expected. It is illegal to pass null.
    ...
    /**
     * Parses an expression into a {@link ValueExpression} for later evaluation.
     ...
     *            variable mappings regardless of whether the mappings in the
     *            provided FunctionMapper and VariableMapper instances change
     *            between calling ExpressionFactory.createValueExpression() and
     *            any method on ValueExpression. Note that within the EL, the
     *            ${} and #{} syntaxes are treated identically. This includes
     *            the use of VariableMapper and FunctionMapper at expression
     *            creation time. Each is invoked if not null, independent of
     *            whether the #{} or ${} syntax is used for the expression.
    ...
    /**
     * Creates a ValueExpression that wraps an object instance. This method can
     * be used to pass any object as a ValueExpression. The wrapper
     * ValueExpression is read only, and returns the wrapped object via its
