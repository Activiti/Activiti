...
/**
 * Simple (thread-safe) LRU cache. After the cache size reached a certain limit,
 * the least recently used entry is removed, when adding a new entry.
 * 
...
public final class Cache implements TreeCache {
    ...
    /**
     * Constructor. Use a {@link WeakHashMap} as secondary map.
     * 
     * @param size
    ...
    /**
     * Constructor. If the least recently used entry is removed from the primary
     * cache, it is added to the secondary map.
     * 
     * @param size
     *            maximum primary cache size
     * @param secondary
     *            the secondary map (may be <code>null</code>)
     */
    ...
    public Cache(final int size, Map<String, Tree> secondary) {
        this.primary = Collections.synchronizedMap(new LinkedHashMap<String, Tree>(16, 0.75f, true) {
            ...
            protected boolean removeEldestEntry(Entry<String, Tree> eldest) {
                if (size() > size) {
                    if (Cache.this.secondary != null) { // move to secondary
                                                        // cache
