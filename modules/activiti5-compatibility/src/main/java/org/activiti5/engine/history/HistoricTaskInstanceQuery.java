...
import org.activiti5.engine.task.TaskInfoQuery;
...
public interface HistoricTaskInstanceQuery extends TaskInfoQuery<HistoricTaskInstanceQuery, HistoricTaskInstance> {
    ...
    /**
     * Only select historic task instances with a task description like the
     * given value. The syntax that should be used is the same as in SQL, eg.
     * %activiti%.
     */
    ...
    /**
     * Only select historic task instances which are part of a process instance
    ...
    /**
     * Only select select historic task instances which are completed on the
     * given date
     */
    HistoricTaskInstanceQuery taskCompletedOn(Date endDate);

    /**
     * Only select select historic task instances which are completed before the
     * given date
     */
    HistoricTaskInstanceQuery taskCompletedBefore(Date endDate);

    /**
     * Only select select historic task instances which are completed after the
     * given date
     */
    ...
    /**
     * Order by the historic activity instance id this task was used in (needs
    ...
    /**
     * Order by end time (needs to be followed by {@link #asc()} or
     * {@link #desc()}).
     */
    HistoricTaskInstanceQuery orderByHistoricTaskInstanceEndTime();

    /**
     * Order by start time (needs to be followed by {@link #asc()} or
     * {@link #desc()}).
     * 
    ...
    /**
     * Order by task delete reason (needs to be followed by {@link #asc()} or
     * {@link #desc()}).
     */
