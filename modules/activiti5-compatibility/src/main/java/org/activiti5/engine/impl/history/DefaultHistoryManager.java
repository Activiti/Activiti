...
import org.activiti5.engine.impl.persistence.entity.HistoricFormPropertyEntity;
...
public class DefaultHistoryManager extends AbstractManager implements HistoryManager {
    ...
    public void recordExecutionReplacedBy(ExecutionEntity execution, InterpretableExecution replacedBy) {
        if (isHistoryLevelAtLeast(HistoryLevel.ACTIVITY)) {
            ...
            List<HistoricActivityInstanceEntity> historicActivityInstances = (List) new HistoricActivityInstanceQueryImpl(Context.getCommandContext()).executionId(execution.getId()).unfinished()
                    .list();
