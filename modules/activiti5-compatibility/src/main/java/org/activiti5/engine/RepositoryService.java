...
public interface RepositoryService {
    ...
    /**
     * Sets the category of the deployment. Deployments can be queried by
     * category: see {@link DeploymentQuery#deploymentCategory(String)}.
     * 
     ...
     * 
     * Changes the tenant identifier of a deployment to match the given tenant
     * identifier. This change will cascade to any related entity: - process
     * definitions related to the deployment - process instances related to
     * those process definitions - executions related to those process instances
     * - tasks related to those process instances - jobs related to the process
     * definitions and process instances
     * 
     * This method can be used in the case that there was no tenant identifier
     * set on the deployment or those entities before.
     * 
     * This method can be used to remove a tenant identifier from the deployment
     * and related entities (simply pass null).
     * 
     * Important: no optimistic locking will be done while executing the tenant
     * identifier change!
     * 
     * This is an experimental feature, mainly because it WILL NOT work properly
     * in a clustered environment without special care: suppose some process
     * instance is in flight. The process definition is in the process
     * definition cache. When a task or job is created when continuing the
     * process instance, the process definition cache will be consulted to get
     * the process definition and from it the tenant identifier. Since it's
     * cached, it will not be the new tenant identifier. This method does clear
     * the cache for this engineinstance , but it will not be cleared on other
     * nodes in a cluster (unless using a shared process definition cache).
     * 
     * @param deploymentId
     *            The id of the deployment of which the tenant identifier will
     *            be changed.
     * @param newTenantId
    ...
    /**
     * Returns a new {@link org.activiti5.engine.query.NativeQuery} for
     * deployment.
     */
     ...
     * 
     * If a process definition is in state suspended, it will not be possible to
     * start new process instances based on the process definition.
     * 
     * <strong>Note: all the process instances of the process definition will
     * still be active (ie. not suspended)!</strong>
     * 
     * @throws ActivitiObjectNotFoundException
     ...
     *            If true, all the process instances of the provided process
     *            definition will be suspended too.
     * @param suspensionDate
     *            The date on which the process definition will be suspended. If
    ...
    /**
     * Suspends the <strong>all<strong> process definitions with the given key
     * (= id in the bpmn20.xml file).
     * 
     * If a process definition is in state suspended, it will not be possible to
     * start new process instances based on the process definition.
     * 
    ...
    /**
     * Similar to {@link #suspendProcessDefinitionByKey(String)}, but only
     * applicable for the given tenant identifier.
     */
    void suspendProcessDefinitionByKey(String processDefinitionKey, String tenantId);

    /**
     * Similar to {@link #suspendProcessDefinitionByKey(String, boolean, Date)},
     ...
     *            The date on which the process definition will be activated. If
     *            null, the process definition is suspended immediately. Note:
     *            The job executor needs to be active to use this!
     * 
     * @throws ActivitiObjectNotFoundException
    ...
    /**
     * Similar to {@link #activateProcessDefinitionByKey(String)}, but only
     * applicable for the given tenant identifier.
     */
    void activateProcessDefinitionByKey(String processDefinitionKey, String tenantId);

    /**
     * Similar to {@link #activateProcessDefinitionByKey(String, boolean, Date)}
     * , but only applicable for the given tenant identifier.
     */
    ...
    /**
     * Sets the category of the process definition. Process definitions can be
     * queried by category: see
     * {@link ProcessDefinitionQuery#processDefinitionCategory(String)}.
     * 
     * @throws ActivitiObjectNotFoundException
    ...
    /**
     * Returns the {@link BpmnModel} corresponding with the process definition
     * with the provided process definition id. The {@link BpmnModel} is a pojo
     * versions of the BPMN 2.0 xml and can be used to introspect the process
     * definition using regular Java.
     */
     ...
     * @param processDefinitionId
     *            id of a {@link ProcessDefinition}, cannot be null.
     * @return Map with process element ids as keys and positions and dimensions
     *         as values.
     ...
     * definition. Such an {@link IdentityLink} informs how a certain identity
     * (eg. group or user) is authorized for a certain process definition
     */
    List<IdentityLink> getIdentityLinksForProcessDefinition(String processDefinitionId);

    /**
     * Validates the given process definition against the rules for executing a
     * process definition on the Activiti engine.
     * 
     * To create such a {@link BpmnModel} from a String, following code may be
     * used:
     * 
     * XMLInputFactory xif = XMLInputFactory.newInstance(); InputStreamReader in
     * = new InputStreamReader(new ByteArrayInputStream(myProcess.getBytes()),
     * "UTF-8"); // Change to other streams for eg from classpath
     * XMLStreamReader xtr = xif.createXMLStreamReader(in); bpmnModel = new
