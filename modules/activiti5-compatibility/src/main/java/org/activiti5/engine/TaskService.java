...
public interface TaskService {
     ...
     * 
     * The returned task is transient and must be saved with
     * {@link #saveTask(Task)} 'manually'.
     */
    Task newTask();
     ...
     * present in the persistent store, it is updated. After a new task has been
     * saved, the task instance passed into this method is updated with the id
     ...
     * 
     * @param taskId
     *            The id of the task that will be deleted, cannot be null. If no
     *            task exists with the given taskId, the operation is ignored.
     * @throws ActivitiObjectNotFoundException
     *             when the task with given id does not exist.
     * @throws ActivitiException
     *             when an error occurs while deleting the task or in case the
     *             task is part of a running process.
     */
    void deleteTask(String taskId);

    /**
     * Deletes all tasks of the given collection, not deleting historic
     * information that is related to these tasks.
     * 
     * @param taskIds
     *            The id's of the tasks that will be deleted, cannot be null.
     *            All id's in the list that don't have an existing task will be
     *            ignored.
     * @throws ActivitiObjectNotFoundException
     *             when one of the task does not exist.
     * @throws ActivitiException
     *             when an error occurs while deleting the tasks or in case one
     *             of the tasks is part of a running process.
     */
     ...
     *            task exists with the given taskId, the operation is ignored.
     * @param cascade
     *            If cascade is true, also the historic information related to
     ...
     *            task exists with the given taskId, the operation is ignored.
     * @param deleteReason
     *            reason the task is deleted. Is recorded in history, if
     *            enabled.
     * @throws ActivitiObjectNotFoundException
     *             when the task with given id does not exist.
     * @throws ActivitiException
     ...
     * here a check is done if the task already has a user assigned to it. No
     * check is done whether the user is known by the identity component.
     * 
     * @param taskId
    ...
    /**
     * Marks that the assignee is done with this task and that it can be send
     * back to the owner. Can only be called when this task is
     * {@link DelegationState#PENDING} delegation. After this method returns,
     * the {@link Task#getDelegationState() delegationState} is set to
     ...
     *            task parameters. May be null or empty.
     * @param localScope
     *            If true, the provided variables will be stored task-local,
     *            instead of process instance wide (which is the default for
     *            {@link #complete(String, Map)}).
     * @throws ActivitiObjectNotFoundException
     *             when no task exists with the given id.
     */
    ...
    /**
     * Changes the assignee of the given task to the given userId. No check is
     * done whether the user is known by the identity component.
     * 
     * @param taskId
    ...
    /**
     * Transfers ownership of this task to another user. No check is done
     * whether the user is known by the identity component.
     * 
     * @param taskId
     *            id of the task, cannot be null.
     * @param userId
    ...
    /**
     * Retrieves the {@link IdentityLink}s associated with the given task. Such
     * an {@link IdentityLink} informs how a certain identity (eg. group or
     * user) is associated with a certain task (eg. as candidate, assignee,
     * etc.)
     */
    ...
    /**
     * Removes the association between a user and a task for the given
     * identityLinkType.
     * 
     * @param taskId
     *            id of the task, cannot be null.
    ...
    /**
     * Removes the association between a group and a task for the given
     * identityLinkType.
     * 
     * @param taskId
     *            id of the task, cannot be null.
    ...
    /**
     * Returns a new {@link TaskQuery} that can be used to dynamically query
     * tasks.
     */
    ...
    /**
     * set variable on a task. If the variable is not already existing, it will
     * be created in the most outer scope. This means the process instance in
     * case this task is related to an execution.
     */
    void setVariable(String taskId, String variableName, Object value);

    /**
     * set variables on a task. If the variable is not already existing, it will
     * be created in the most outer scope. This means the process instance in
     * case this task is related to an execution.
     */
    ...
    /**
     * checks whether or not the task has a variable defined with the given
     * name, in the task scope and if available also the execution scopes.
     */
    ...
    /**
     * get all variables and search only in the task scope. If you have many
     * task local variables and you only need a few, consider using
    ...
    /**
     * Removes the variable from the task. When the variable does not exist,
     * nothing happens.
     */
    void removeVariable(String taskId, String variableName);

    /**
     * Removes the variable from the task (not considering parent scopes). When
     * the variable does not exist, nothing happens.
     */
    void removeVariableLocal(String taskId, String variableName);

    /**
     * Removes all variables in the given collection from the task. Non existing
     * variable names are simply ignored.
     */
    void removeVariables(String taskId, Collection<String> variableNames);

    /**
     * Removes all variables in the given collection from the task (not
     * considering parent scopes). Non existing variable names are simply
     * ignored.
     */
    ...
    /**
     * Returns an individual comment with the given id. Returns null if no
     * comment exists with the given id.
     */
    ...
    /**
     * Returns an individual event with the given id. Returns null if no event
    ...
    /**
     * Add a new attachment to a task and/or a process instance and use an input
     * stream to provide the content
     */
    ...
    /**
     * Add a new attachment to a task and/or a process instance and use an url
     * as the content
     */
