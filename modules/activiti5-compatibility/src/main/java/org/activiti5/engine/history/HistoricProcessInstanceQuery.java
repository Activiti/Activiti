...
public interface HistoricProcessInstanceQuery extends Query<HistoricProcessInstanceQuery, HistoricProcessInstance> {
     ...
     * {@link ProcessInstance) ids and {@link HistoricProcessInstance} ids
     * match.
     */
    HistoricProcessInstanceQuery processInstanceId(String processInstanceId);

    /**
     * Only select historic process instances whose id is in the given set of
     * ids. {@link ProcessInstance) ids and {@link HistoricProcessInstance} ids
     * match.
     */
    ...
    /**
     * Only select historic process instances that don't have a
     * process-definition of which the key is present in the given list
     */
    ...
    /**
     * Only select the historic process instances with which the user with the
     * given id is involved.
     */
    HistoricProcessInstanceQuery involvedUser(String userId);

    /**
     * Only select process instances which had a global variable with the given
     * value when they ended. The type only applies to already ended process
     * instances, otherwise use a {@link ProcessInstanceQuery} instead! of
    ...
    /**
     * Only select process instances which had at least one global variable with
     * the given value when they ended. The type only applies to already ended
     * process instances, otherwise use a {@link ProcessInstanceQuery} instead!
    ...
    /**
     * Only select historic process instances which have a local string variable
    ...
    /**
     * Only select process instances which had a global variable with the given
     * name, but with a different value than the passed value when they ended.
     * Only select process instances which have a variable value greater than
     * the passed value. Byte-arrays and {@link Serializable} objects (which are
    ...
    /**
     * Only select process instances which had a global variable value greater
     * than the passed value when they ended. Booleans, Byte-arrays and
     * {@link Serializable} objects (which are not primitive type wrappers) are
    ...
    /**
     * Only select process instances which had a global variable value greater
     * than or equal to the passed value when they ended. Booleans, Byte-arrays
     * and {@link Serializable} objects (which are not primitive type wrappers)
     * are not supported. Only applies to already ended process instances,
     * otherwise use a {@link ProcessInstanceQuery} instead!
     * 
     * @param name
     *            cannot be null.
     * @param value
    ...
    /**
     * Only select process instances which had a global variable value less than
     * the passed value when the ended. Only applies to already ended process
     * instances, otherwise use a {@link ProcessInstanceQuery} instead!
     * Booleans, Byte-arrays and {@link Serializable} objects (which are not
    ...
    /**
     * Only select process instances which has a global variable value less than
     * or equal to the passed value when they ended. Only applies to already
     * ended process instances, otherwise use a {@link ProcessInstanceQuery}
     * instead! Booleans, Byte-arrays and {@link Serializable} objects (which
    ...
    /**
     * Only select process instances which had global variable value like the
     * given value when they ended. Only applies to already ended process
     * instances, otherwise use a {@link ProcessInstanceQuery} instead! This can
     * be used on string variables only.
     * 
     * @param name
     *            cannot be null.
     * @param value
    ...
    /**
     * Only select historic process instances that were started before the given
     * date.
     */
    HistoricProcessInstanceQuery startedBefore(Date date);

    /**
     * Only select historic process instances that were started after the given
     * date.
     */
    ...
    /**
     * Begin an OR statement. Make sure you invoke the endOr method at the end
     * of your OR statement. Only one OR statement is allowed, for the second
     * call to this method an exception will be thrown.
     */
    HistoricProcessInstanceQuery or();

    /**
     * End an OR statement. Only one OR statement is allowed, for the second
     * call to this method an exception will be thrown.
     */
    HistoricProcessInstanceQuery endOr();

    /**
     * Order by the process instance id (needs to be followed by {@link #asc()}
     * or {@link #desc()}).
     */
    ...
    /**
     * Order by the business key (needs to be followed by {@link #asc()} or
     * {@link #desc()}).
     */
    HistoricProcessInstanceQuery orderByProcessInstanceBusinessKey();

    /**
     * Order by the start time (needs to be followed by {@link #asc()} or
     * {@link #desc()}).
     */
    HistoricProcessInstanceQuery orderByProcessInstanceStartTime();

    /**
     * Order by the end time (needs to be followed by {@link #asc()} or
     * {@link #desc()}).
     */
     ...
     * instance. {@link ProcessInstance) ids and
     * {@link HistoricProcessInstance} ids match.
     */
    ...
    /**
     * Only select process instances with a name like the given value, ignoring
     * upper/lower case.
     */
