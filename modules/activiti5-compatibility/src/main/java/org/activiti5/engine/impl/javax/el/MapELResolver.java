...
public class MapELResolver extends ELResolver {
   ...
   * <ul>
   * <li>displayName - The return value of calling the toString method on this key, or "null" if the key is null</li>
   ...
   * <ul>
   * <li>{@link ELResolver#TYPE} - The return value of calling the getClass() method on this key, or null if the key is null.</li>
   ...
   * @param base
   *          The map to analyze. Only bases of type Map are handled by this resolver.
   * @return An Iterator containing zero or more (possibly infinitely more) FeatureDescriptor objects, each representing a key in this map, or null if the base object is not a map.
   */
  @Override
  ...
  /**
   * If the base object is a map, returns whether a call to {@link #setValue(ELContext, Object, Object, Object)} will always fail. If the base is a Map, the propertyResolved property of the ELContext
   * object must be set to true by this resolver, before returning. If this property is not true after this method is called, the caller should ignore the return value. If this resolver was
   * constructed in read-only mode, this method will always return true. If a Map was created using java.util.Collections.unmodifiableMap(Map), this method must return true. Unfortunately, there is no
   * Collections API method to detect this. However, an implementation can create a prototype unmodifiable Map and query its runtime type to see if it matches the runtime type of the base object as a
   * workaround.
   * 
   * @param context
   *          The context of this evaluation.
