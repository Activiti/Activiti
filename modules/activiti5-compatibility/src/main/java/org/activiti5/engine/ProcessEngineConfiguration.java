 ...
 * first option is {@link #createStandaloneProcessEngineConfiguration()}
 * 
 * <pre>
 ...
 * {@link #createStandalonInMemeProcessEngineConfiguration()}
 * 
 * <pre>
 ...
 * DB schema strategy default is in this case <code>create-drop</code>.
 * Standalone mode means that Activiti will manage the transactions on the JDBC
 * connections that it creates. One transaction per service method.
 * </p>
 * 
 * <p>
 * On all forms of creating a process engine, you can first customize the
 * configuration before calling the {@link #buildProcessEngine()} method by
 * calling any of the setters like this:
 * 
 * <pre>
...
public abstract class ProcessEngineConfiguration implements EngineServices {
    ...
    /**
     * Define the default lock time for an async job in seconds. The lock time
     * is used when creating an async job and when it expires the async executor
    ...
    /**
     * Allows configuring a database table prefix which is used for all runtime
     * operations of the process engine. For example, if you specify a prefix
     * named 'PRE1.', activiti will query for executions in a table named
     * 'PRE1.ACT_RU_EXECUTION_'.
     * 
     * <p />
     * <strong>NOTE: the prefix is not respected by automatic database schema
     * management. If you use
     * {@link ProcessEngineConfiguration#DB_SCHEMA_UPDATE_CREATE_DROP} or
     * {@link ProcessEngineConfiguration#DB_SCHEMA_UPDATE_TRUE}, activiti will
     * create the database tables using the default names, regardless of the
     * prefix configured here.</strong>
     * 
     * @since 5.9
     */
    ...
    /**
     * Set to true in case the defined databaseTablePrefix is a schema-name,
     * instead of an actual table name prefix. This is relevant for checking if
     * Activiti-tables exist, the databaseTablePrefix will not be used here -
     * since the schema is taken into account already, adding a prefix for the
     * table-check will result in wrong table-names.
     * 
     * @since 5.15
     */
     ...
     * http://forums.activiti.org/content/reflectutilloadclass-and-custom-
     * classloader
     */
