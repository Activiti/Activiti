...
public class TaskQueryTest extends PluggableActivitiTestCase {
    ...
    public void testProcessCategoryInOr() throws Exception {
        ...
        final Task task = taskService.createTaskQuery().or().taskId("invalid").processCategoryIn(Collections.singletonList("Examples")).singleResult();
        assertNotNull(task);
        assertEquals("theTask", task.getTaskDefinitionKey());
        assertEquals(processInstance.getId(), task.getProcessInstanceId());
    ...
    public void testNativeQuery() {
        ...
        assertEquals(
                ...
                taskService
                        ...
                        .sql("SELECT count(*) FROM " + managementService.getTableName(Task.class) + " T1, " + managementService.getTableName(VariableInstanceEntity.class)
        ...
        // use parameters
        assertEquals(1,
                taskService.createNativeTaskQuery().sql("SELECT count(*) FROM " + managementService.getTableName(Task.class) + " T WHERE T.NAME_ = #{taskName}").parameter("taskName", "gonzoTask")
                        .count());
    }
    ...
    /**
     * Generates some test tasks. - 6 tasks where kermit is a candidate - 1
     * tasks where gonzo is assignee - 2 tasks assigned to management group - 2
     * tasks assigned to accountancy group - 1 task assigned to both the
     * management and accountancy group
     */
