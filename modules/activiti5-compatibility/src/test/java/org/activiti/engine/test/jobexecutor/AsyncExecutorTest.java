 ...
 * 
 * Note that all tests with jobs already use this async executor, so this is
 * really all the 'edgy' cases here.
 * 
 * @author jbarrez
 */
public class AsyncExecutorTest {
    ...
    public void testAsyncScriptExecutionOnTwoEngines() {
        ...
        try {
            ...
            secondProcessEngine = createProcessEngine(true, now); // Same
                                                                  // timestamp
                                                                  // as first
                                                                  // engine
            Assert.assertEquals(0, firstProcessEngine.getTaskService().createTaskQuery().taskName("Task after script").count());
    ...
    private ProcessEngine createProcessEngine(boolean enableAsyncExecutor, Date time) {
        ...
        if (enableAsyncExecutor) {
            ...
            countingAsyncExecutor.setDefaultAsyncJobAcquireWaitTimeInMillis(50); // To
                                                                                 // avoid
                                                                                 // waiting
                                                                                 // too
                                                                                 // long
