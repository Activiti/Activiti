 ...
 * with a particular Execution / Task, then perform some changes (retrieve / set
 * process variables) and then end the unit of work. This bean makes sure that
 * our changes are only "flushed" to the process engine when we successfully
 ...
 * </pre>
 * <p />
 * <strong>NOTE:</strong> in the absence of a conversation, (non faces request,
 * i.e. when processing a JAX-RS, JAX-WS, JMS, remote EJB or plain Servlet
 * requests), the {@link BusinessProcess} bean associates with the current
 * Request (see {@link RequestScoped @RequestScoped}).
 * <p />
 * <strong>NOTE:</strong> in the absence of a request, ie. when the activiti
 * JobExecutor accesses {@link BusinessProcessScoped @BusinessProcessScoped}
 * beans, the execution is associated with the current thread.
 * 
 * @author Daniel Meyer
 * @author Falko Menge
 */
...
public class BusinessProcess implements Serializable {
     ...
     * @throw ActivitiCdiException if no such execution exists
     */
    ...
    /**
     * Associates the task with the provided taskId with the current
     * conversation.
     * <p/>
     * 
     * @param taskId
     ...
     * 
     *      this method allows to start a conversation if no conversation is
     *      active
     */
     ...
     * <p />
     * 
     * <strong>NOTE:</strong> If no execution is currently associated, the value
     * is temporarily cached and flushed to the process instance at the end of
     * the unit of work
     * 
     * @param variableName
     *            the name of the process variable for which a value is to be
     *            set
     * @param value
     *            the value to be set
     * 
     ...
     *             {@link #isTaskAssociated()} to check whether an association
     *             exists.
     * 
     */
    public Task getTask() {
     ...
     *             {@link #isAssociated()} to check whether an association
     *             exists.
     */
