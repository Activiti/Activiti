...
public class ProcessDefinitionQueryEscapeClauseTest extends AbstractEscapeClauseTestCase {
  ...
  protected void setUp() throws Exception {
    deploymentOneId = repositoryService
      ...
      deploymentTwoId = repositoryService
        ...
        .tenantId("Two_")
        .name("two_")
        .addClasspathResource("org/activiti5/engine/test/repository/two_.bpmn20.xml")
        .deploymentProperty(DeploymentProperties.DEPLOY_AS_ACTIVITI5_PROCESS_DEFINITION, Boolean.TRUE)
        .deploy()
        .getId();
  ...
  public void testQueryByResourceNameLike() {
    ...
    assertEquals("org/activiti5/engine/test/repository/one%.bpmn20.xml", query.singleResult().getResourceName());
    assertEquals(1, query.list().size());
    assertEquals(1, query.count());
    
    query = repositoryService.createProcessDefinitionQuery().processDefinitionResourceNameLike("%\\_%");
    assertEquals("org/activiti5/engine/test/repository/two_.bpmn20.xml", query.singleResult().getResourceName());
    assertEquals(1, query.list().size());
    assertEquals(1, query.count());
  }
