...
/**
 * A {@link ProcessEngineConfigurator} that integrates a LDAP system with the
 * Activiti process engine. The LDAP system will be consulted primarily for
 * getting user information and in particular for fetching groups of a user.
 * 
 * This class is extensible and many methods can be overriden when the default
 * behavior is not fitting your use case.
 * 
 * Check the docs (speficifally the setters) to see how this class can be
 * tweaked.
 * 
 * @author Joram Barrez
 */
public class LDAPConfigurator extends AbstractProcessEngineConfigurator {
    ...
    /**
     * The server on which the LDAP system can be reached. For example
    ...
    /**
     * The {@link InitialContextFactory} name used to connect to the LDAP
     * system.
     * 
    ...
    /**
     * The value that is used for the 'java.naming.security.authentication'
     * property used to connect to the LDAP system.
     * 
    ...
    /**
     * Allows to set all LDAP connection parameters which do not have a
     * dedicated setter. See for example
     ...
     * pooling, specific security settings, etc.
     * 
     * All the provided parameters will be provided when creating a
     * {@link InitialDirContext}, ie when a connection to the LDAP system is
     * established.
     */
    ...
    /**
     * The base 'distinguished name' (DN) from which the searches for users and
     * groups are started.
     * 
     * Use {@link #setUserBaseDn(String)} or {@link #setGroupBaseDn(String)}
     * when needing to differentiate between user and group base DN.
     */
    ...
    /**
     * The base 'distinguished name' (DN) from which the searches for users are
     * started.
     */
    ...
    /**
     * The base 'distinguished name' (DN) from which the searches for groups are
     * started.
     */
    ...
    /**
     * The timeout that is used when doing a search in LDAP. By default set to
     * '0', which means 'wait forever'.
     */
     ...
     * 
     * Here, all the objects in LDAP with the class 'inetOrgPerson' and who have
     * the matching 'uid' attribute value will be returned.
     * 
     * As shown in the example, the user id is injected by the typical
     * {@link MessageFormat}, ie by using <i>{0}</i>
     * 
     * If setting the query alone is insufficient for your specific LDAP setup,
     * you can alternatively plug in a different {@link LDAPQueryBuilder}, which
     * allows for more customization than only the query.
     */
     ...
     * 
     * Here, all the objects in LDAP with the class 'inetOrgPerson' and who have
     * the matching first name or last name will be returned
     * 
     * Several things will be injected in the expression: {0} : the first name
     * attribute {1} : the search text {2} : the last name attribute {3} : the
     * search text
     * 
     * If setting the query alone is insufficient for your specific LDAP setup,
     * you can alternatively plug in a different {@link LDAPQueryBuilder}, which
    ...
    /**
     * The query that is executed when searching for the groups of a specific
     * user.
     * 
     * For example: (&amp;(objectClass=groupOfUniqueNames)(uniqueMember={0}))
     * 
     * Here, all the objects in LDAP with the class 'groupOfUniqueNames' and
     * where the provided DN is a 'uniqueMember' are returned.
     * 
     * As shown in the example, the user id is injected by the typical
     * {@link MessageFormat}, ie by using <i>{0}</i>
     * 
     ...
     * 
     * This property is used when looking for a {@link User} object and the
     * mapping between the LDAP object and the Activiti {@link User} object is
     * done.
     * 
     * This property is optional and is only needed if searching for
     * {@link User} objects using the Activiti API.
     */
     ...
     * 
     * This property is used when looking for a {@link Group} object and the
     * mapping between the LDAP object and the Activiti {@link Group} object is
     * done.
     */
    ...
    /**
     * Set a custom implementation of the {@link LDAPUserManagerFactory} if the
    ...
    /**
     * Set a custom implementation of the {@link LDAPGroupManagerFactory} if the
     * default implementation is not suitable.
     */
    ...
    /**
     * Set a custom {@link LDAPQueryBuilder} if the default implementation is
     * not suitable. The {@link LDAPQueryBuilder} instance is used when the
     * {@link LDAPUserManager} or {@link LDAPGroupManager} does an actual query
     * against the LDAP system.
     * 
     * The default implementation uses the properties as set on this instance
     * such as {@link #setQueryGroupsForUser(String)} and
     * {@link #setQueryUserByUserId(String)}.
     */
    ...
    /**
     * Allows to set the size of the {@link LDAPGroupCache}. This is an LRU
     * cache that caches groups for users and thus avoids hitting the LDAP
     * system each time the groups of a user needs to be known.
     * 
     * The cache will not be instantiated if the value is less then zero. By
     * default set to -1, so no caching is done.
     * 
     * Note that the group cache is instantiated on the
     * {@link LDAPGroupManagerFactory}. As such, if you have a custom
     * implementation of the {@link LDAPGroupManagerFactory}, do not forget to
     * add the group cache functionality.
     */
     ...
     * When groups for a specific user are fetched, and if the group cache
     * exists (see {@link #setGroupCacheSize(int)}), the groups will be stored
     * in this cache for the time set in this property. ie. when the groups were
     * fetched at 00:00 and the expiration time is 30 mins, any fetch of the
     * groups for that user after 00:30 will not come from the cache, but do a
     * fetch again from the LDAP system. Likewise, everything group fetch for
     * that user done between 00:00 - 00:30 will come from the cache.
     * 
