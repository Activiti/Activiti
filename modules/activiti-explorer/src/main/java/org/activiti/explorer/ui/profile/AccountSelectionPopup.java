...
public class AccountSelectionPopup extends TabbedSelectionWindow {
  ...
  public AccountSelectionPopup(String title) {
    ...
    // TODO: components are eager loaded. For performance they should be
    // lazy loaded (eg through factory)
  ...
  protected void initImapComponent() {
    ...
    imapClickListener = new ClickListener() {
      public void buttonClick(ClickEvent event) {
        Map<String, Object> accountDetails = createAccountDetails("imap", imapUserName.getValue().toString(), imapPassword.getValue().toString(), "server", imapServer.getValue().toString(), "port",
            imapPort.getValue().toString(), "ssl", imapPort.getValue().toString());
  ...
  protected void initAlfrescoComponent() {
    ...
    alfrescoClickListener = new ClickListener() {
      public void buttonClick(ClickEvent event) {
        Map<String, Object> accountDetails = createAccountDetails("alfresco", alfrescoUserName.getValue().toString(), alfrescoPassword.getValue().toString(), "server", alfrescoServer.getValue()
            .toString());
        fireEvent(new SubmitEvent(AccountSelectionPopup.this, SubmitEvent.SUBMITTED, accountDetails));
      }
    };
