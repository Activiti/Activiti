...
/**
 * A simplified version of the {@Link TaskDetailPanel}, displaying read-only historic task details.
 * 
 * @author Joram Barrez
 */
public class HistoricTaskDetailPanel extends DetailPanel {
  ...
  protected void populateRelatedContent(Table table, List<Attachment> attachments) {
    ...
    for (Attachment attachment : attachments) {
      ...
      // Simple renderer that just shows a popup window with the
      // attachment
  ...
  /**
   * Returns a numeric priority - low (1), medium (2) or high (3) - based on the given numeric priority value.
   */
