...
public class ChartGenerator {
    ...
    protected static Component createChart(JsonNode dataNode, String[] names, Number[] values) {
        ...
        if (CHART_TYPE_BAR_CHART.equals(type)) {
            ...
            Axes axes = new Axes().addAxis(new XYaxis().setLabel(xAxis != null ? xAxis : "").setMin(names[0]).setMax(names[values.length - 1]).setDrawMajorTickMarks(true)).addAxis(
                    new XYaxis(XYaxes.Y).setLabel(yAxis != null ? yAxis : "").setDrawMajorTickMarks(true));
