...
public class AdminRunningInstancesPanel extends DetailPanel {
  ...
  protected void initDefinitionsTable() {
    if (instanceList == null || instanceList.isEmpty()) {
      ...
      for (ManagementProcessDefinition managementDefinition : runningDefinitions.values()) {
        definitionsTable.addItem(
            new String[] { managementDefinition.processDefinition.getId(), managementDefinition.processDefinition.getName(), String.valueOf(managementDefinition.runningInstances.size()) },
      ...
      definitionsTable.addListener(new Property.ValueChangeListener() {
        ...
        public void valueChange(ValueChangeEvent event) {
          Item item = definitionsTable.getItem(event.getProperty().getValue()); // the
                                                                                // value
                                                                                // of
                                                                                // the
                                                                                // property
  ...
  protected void initInstancesTable() {
    ...
    instancesTable.addListener(new Property.ValueChangeListener() {
      ...
      public void valueChange(ValueChangeEvent event) {
        Item item = instancesTable.getItem(event.getProperty().getValue()); // the
                                                                            // value
                                                                            // of
                                                                            // the
                                                                            // property
  ...
  protected void refreshInstancesTable() {
    ...
    for (HistoricProcessInstance instance : selectedManagementDefinition.runningInstances) {
      instancesTable.addItem(new String[] { instance.getId(), instance.getBusinessKey(), instance.getStartUserId(), instance.getStartActivityId(), instance.getStartTime().toString() },
          instance.getId());
    }
  }
  ...
  protected void addProcessImage(ProcessDefinition processDefinition, HistoricProcessInstance processInstance) {
    ...
    if (processDefinitionEntity != null && processDefinitionEntity.isGraphicalNotationDefined()) {
      ...
      StreamResource diagram = new ProcessDefinitionImageStreamResourceBuilder().buildStreamResource(processInstance.getId(), processInstance.getProcessDefinitionId(), repositoryService,
  ...
  protected void addTasks(HistoricProcessInstance processInstance) {
    ...
    // Fetch all tasks
    List<HistoricTaskInstance> tasks = historyService.createHistoricTaskInstanceQuery().processInstanceId(processInstance.getId()).orderByHistoricTaskInstanceEndTime().desc().orderByTaskCreateTime()
        .desc().list();
