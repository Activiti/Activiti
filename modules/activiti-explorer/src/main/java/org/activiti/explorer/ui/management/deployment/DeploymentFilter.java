...
/**
 * Class used in {@link DeploymentListQuery} for filtering and consulted right before a new {@link Deployment} is about to be deployed through the UI.
 * 
...
public interface DeploymentFilter {
  ...
  /**
   * Return a query that filters deployments, paging info is applied later on and should not be altered.
   */
  ...
  /**
   * @param a
   *          {@link Deployment} resulting from the query
   * @return item representing the deployment
   */
  DeploymentListitem createItem(Deployment deployment);

  /**
   * Called right before the deployment, created in the UI, is deployed to the API. This allows to ad additional artifacts or alter deployment properties.
   */
