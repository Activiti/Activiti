...
public class DefaultViewManager implements ViewManager, Serializable {
  ...
  protected void showNavigationError(String taskId) {
    ExplorerApp.get().getNotificationManager()
        .showErrorNotification(Messages.NAVIGATION_ERROR_NOT_INVOLVED_TITLE, ExplorerApp.get().getI18nManager().getMessage(Messages.NAVIGATION_ERROR_NOT_INVOLVED, taskId));
  }
  ...
  public void showProcessInstancePage() {
    throw new UnsupportedOperationException(); // Only for alfresco admin
                                               // app
  }
