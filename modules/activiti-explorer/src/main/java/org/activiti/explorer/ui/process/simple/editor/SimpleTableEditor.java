...
public class SimpleTableEditor extends AbstractPage {
  ...
  protected void showDiagram() {
    StreamResource.StreamSource streamSource = new StreamSource() {
      ...
      public InputStream getStream() {
        ...
        return diagramGenerator.generateDiagram(workflowDefinitionConversion.getBpmnModel(), "png", processEngineConfiguration.getActivityFontName(), processEngineConfiguration.getLabelFontName(),
  ...
  protected void save() {
    ...
    try {
      ...
      // Store process image
      // TODO: we should really allow the service to take an inputstream
      // as input. Now we load it into memory ...
      repositoryService.addModelEditorSourceExtra(model.getId(), IOUtils.toByteArray(diagramGenerator.generateDiagram(conversion.getBpmnModel(), "png",
          processEngineConfiguration.getActivityFontName(), processEngineConfiguration.getLabelFontName(), processEngineConfiguration.getClassLoader())));
    } catch (IOException e) {
  ...
  // Following are required by superclass, but not implemented since we do the
  // addMainLayout ourselves
