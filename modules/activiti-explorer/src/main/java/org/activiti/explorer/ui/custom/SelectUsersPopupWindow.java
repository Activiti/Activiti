...
/**
 * A popup window that is used to select people. Two possible modes: - multiselect: displays two tables that allow to select users from the left table to the table on the right - non-multiselect: one
 * table where only one user can be chosen from.
 * 
 * {@link SubmitEventListener} can be attached to listen to completion of the selection. The selected user(s) can be retrieved using {@link #getSelectedUserId()} ,{@link #getSelectedUserIds()} and
...
public class SelectUsersPopupWindow extends PopupWindow {
  ...
  protected void selectUser(String userId, String userName) {
    if (!selectedUsersTable.containsId(userId)) {
      ...
      if (showRoles) {
        ComboBox comboBox = new ComboBox(null, Arrays.asList(i18nManager.getMessage(Messages.TASK_ROLE_CONTRIBUTOR), i18nManager.getMessage(Messages.TASK_ROLE_IMPLEMENTER),
            i18nManager.getMessage(Messages.TASK_ROLE_MANAGER), i18nManager.getMessage(Messages.TASK_ROLE_SPONSOR)));
