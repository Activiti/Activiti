...
public class DatabasePage extends ManagementPage {
    ...
    public DatabasePage() {
        ExplorerApp.get().setCurrentUriFragment(new UriFragment(DatabaseNavigator.TABLE_URI_PART));
    }
    ...
    protected void initUi() {
        ...
        populateTableList(); // tablelist is NOT lazy loaded, since there are
                             // only a few tables
        if (tableName == null) {
            selectElement(0);
        } else {
    ...
    protected void populateTableList() {
        TreeMap<String, Long> tables = new TreeMap<String, Long>(managementService.getTableCount()); // treemap
                                                                                                     // because
                                                                                                     // we
                                                                                                     // want
                                                                                                     // to
