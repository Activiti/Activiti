...
public abstract class AbstractPage extends CustomComponent {
  ...
  /**
   * Returns an implementation of {@link AbstractSelect}, which will be displayed on the left side of the page, allowing to select elements from eg. a list, tree, etc.
   */
  protected abstract AbstractSelect createSelectComponent();

  /**
   * Refreshes the elements of the list, and selects the next one (useful when the selected element is deleted).
   */
  ...
  /**
   * Override to get the search component to display above the table. Return null when no search should be displayed.
   */
   ...
   * 
   * Return null by default: no event-component will be used, in that case the main UI will be two columns instead of three.
   * 
   * Override in case the event component must be shown: three columns will be used then.
   */
