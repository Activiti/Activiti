    ...
    public void testPostNewDeploymentDMNFile() throws Exception {
        ...
        try {
            ...
            httpPost.setEntity(HttpMultipartHelper.getMultiPartEntity("simple.dmn", "application/xml",
                    ReflectUtil.getResourceAsStream("org/activiti/rest/dmn/service/api/repository/simple.dmn"), null));
    ...
    public void testGetDeployments() throws Exception {
        ...
        try {
            ...
            DmnDeployment secondDeployment = dmnRepositoryService.createDeployment().name("Deployment 2").category("ABC")
                    .addClasspathResource("org/activiti/rest/dmn/service/api/repository/simple.dmn").tenantId("myTenant").deploy();
            ...
            // Check paging
            response = executeRequest(new HttpGet(SERVER_URL_PREFIX + DmnRestUrls.createRelativeResourceUrl(DmnRestUrls.URL_DEPLOYMENT_COLLECTION) + "?sort=deployTime&order=asc&start=1&size=1"), HttpStatus.SC_OK);
            JsonNode responseNode = objectMapper.readTree(response.getEntity().getContent());
            closeResponse(response);
