...
public class BpmnAutoLayout {
    ...
    protected void layout(FlowElementsContainer flowElementsContainer) {
        ...
        sequenceFlows = new HashMap<String, SequenceFlow>(); // Sequence flow
                                                             ...
                                                             // source and
                                                             // target
        boundaryEvents = new ArrayList<BoundaryEvent>(); // Boundary events are
                                                         // gathered and
                                                         // processed
                                                         // afterwards, because
                                                         // we must be sure we
        ...
        for (int i = 0; i < unoptimizedPointsList.size(); i++) {
            ...
            if (i > 0 && i != unoptimizedPointsList.size() - 1) {
                ...
                if (currentPoint.getX() >= previousPoint.getX() && currentPoint.getX() <= nextPoint.getX() && currentPoint.getY() == previousPoint.getY() && currentPoint.getY() == nextPoint.getY()) {
                    keepPoint = false;
                } else if (currentPoint.getY() >= previousPoint.getY() && currentPoint.getY() <= nextPoint.getY() && currentPoint.getX() == previousPoint.getX()
     ...
     * {@link #handleSubProcess(FlowElement)}), the elements have x and y
     * coordinates relative to the bounds of the subprocess (thinking the
     * subprocess is on (0,0). This however, does not work for nested
     * subprocesses, as they need to take in account the x and y coordinates for
     * each of the parent subproceses.
     * 
     * This method is to be called after fully layouting one process, since ALL
     * elements need to have x and y.
     */
