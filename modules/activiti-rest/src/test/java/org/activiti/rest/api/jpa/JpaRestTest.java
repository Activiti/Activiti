...
public class JpaRestTest extends BaseJPARestTestCase {
    ...
    public void testGetJpaVariableViaTaskCollection() throws Exception {
        ...
        // Request all variables (no scope provides) which include global and
        // local
        HttpResponse response = executeHttpRequest(new HttpGet(SERVER_URL_PREFIX + RestUrls.createRelativeResourceUrl(RestUrls.URL_TASK_COLLECTION) + "?includeProcessVariables=true"),
                HttpStatus.SC_OK);
    ...
    public void testGetJpaVariableViaHistoricProcessCollection() throws Exception {
        ...
        // Request all variables (no scope provides) which include global and
        // local
        HttpResponse response = executeHttpRequest(new HttpGet(SERVER_URL_PREFIX + RestUrls.createRelativeResourceUrl(RestUrls.URL_HISTORIC_PROCESS_INSTANCES) + "?processInstanceId="
                + processInstance.getId() + "&includeProcessVariables=true"), HttpStatus.SC_OK);
    ...
    public void testGetJpaVariableViaHistoricVariablesCollections() throws Exception {
        ...
        // Request all variables (no scope provides) which include global and
        // local
        HttpResponse response = executeHttpRequest(new HttpGet(SERVER_URL_PREFIX + RestUrls.createRelativeResourceUrl(RestUrls.URL_HISTORIC_VARIABLE_INSTANCES) + "?processInstanceId="
