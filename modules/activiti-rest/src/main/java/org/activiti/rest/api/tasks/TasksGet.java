...
public class TasksGet extends ActivitiWebScript
{
  ...
  protected void executeWebScript(WebScriptRequest req, Status status, Cache cache, Map<String, Object> model)
  {
    ...
    int startIndex = getInt(req, "startIndex", 0);
    int pageSize = getInt(req, "pageSize", 10);
    Page p = new Page(startIndex, pageSize);
    ...
    if (personalTaskUserId != null) {
      ...
      throw new WebScriptException(Status.STATUS_BAD_REQUEST, "Tasks must be filtered with at least one filter parameter");
    }
    model.put("tasks", tq.pagedList(startIndex, pageSize));
    model.put("startIndex", startIndex);
    model.put("totalRecords", tq.count());
