...
/**
 * "thread" has to be driven by simulation time. In fact new thread is not started. Simulation is driven by simulation time. In case of thread for acquiring jobs, synchronization will be over
 * complicated.
 * 
...
public class SimulationAcquireJobsRunnable extends AcquireJobsRunnableImpl {
  ...
  public synchronized void run() {
    ...
    if ((millisToWait > 0) && (!isJobAdded)) {
        ...
        synchronized (MONITOR) {
          if (!isInterrupted) {
            ...
            SimulationEvent event = new SimulationEvent.Builder(SimulationEvent.TYPE_ACQUIRE_JOB_NOTIFICATION_EVENT)
                .simulationTime(SimulationRunContext.getClock().getCurrentTime().getTime() + millisToWait).property(this).build();
      ...
      // schedule run now
      SimulationEvent event = new SimulationEvent.Builder(SimulationEvent.TYPE_ACQUIRE_JOB_NOTIFICATION_EVENT).simulationTime(SimulationRunContext.getClock().getCurrentTime().getTime())
          .property(this).build();
      SimulationRunContext.getEventCalendar().addEvent(event);
    }
    // }
