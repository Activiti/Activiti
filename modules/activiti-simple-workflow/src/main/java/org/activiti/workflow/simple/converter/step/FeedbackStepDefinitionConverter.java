...
public class FeedbackStepDefinitionConverter extends BaseStepDefinitionConverter<FeedbackStepDefinition, Map<String, BaseElement>> {
  ...
  protected Map<String, BaseElement> createProcessArtifact(FeedbackStepDefinition feedbackStepDefinition, WorkflowDefinitionConversion conversion) {
    ...
    // See feedback-step.png in the resource folder to get a graphical
    // understanding of the conversion below
    ...
    // Boundary signal catch to shut down all tasks if the 'gather feedback'
    // task is completed
    BoundaryEvent boundarySignalCatch = createBoundarySignalCatch(conversion, signal, feedbackTask);

    // Exclusive gateway after the feedback task, needed to correctly merge
    // the sequence flow
    // such that the joining parallel gateway has exactly two incoming
    // sequence flow
  ...
  protected UserTask createSelectPeopleUserTask(FeedbackStepDefinition feedbackStepDefinition, WorkflowDefinitionConversion conversion, Map<String, BaseElement> processElements) {
    ...
    // TODO: work out form such that it can be used in Activiti Explorer,
    // ie. add correct form properties
    ...
    feedbackProvidersProperty.setType("string"); // TODO: we need some kind
                                                 // of 'people' property
                                                 // type here

    selectPeopleUserTask.setFormProperties(Arrays.asList(feedbackProvidersProperty));

    // When the list of feedback providers is fixed up front, we need to add
    // a script listener
    // that injects these variables into the process (instead of having it
    // provided by the end user in a form)
