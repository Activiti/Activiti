 ...
 * 
 * When added to a {@link WorkflowDefinitionConversionFactory}, this class will make sure a start event and end event is created for the {@link Process}. Further, it will generate a correct incoming
 * and outgoing sequence flow list for each {@link Process} element, as required by some toolings (eg Modeler).
 * 
 * @author Joram Barrez
 */
public class DefaultWorkflowDefinitionConversionListener implements WorkflowDefinitionConversionListener {
  ...
  public void afterStepsConversion(WorkflowDefinitionConversion conversion) {
    ...
    // We must add the ingoing and outgoing sequence flow to each of the
    // flow
    // nodes
