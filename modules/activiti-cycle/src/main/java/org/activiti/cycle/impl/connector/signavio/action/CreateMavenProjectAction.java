...
public class CreateMavenProjectAction extends CreateTechnicalBpmnXmlAction {
	...
	public void execute(RepositoryConnector connector, RepositoryArtifact artifact, Map<String, Object> parameters) throws Exception {
		...
		RepositoryConnector targetConnector = (RepositoryConnector) getParameter(parameters, PARAM_TARGET_CONNECTOR, true, null,
	...
	/**
	 * create a project from the Maven template and return the
	 * RepositoryArtifact representing the bpmn process model
	 */
	public RepositoryArtifact createProject(RepositoryConnector connector, String rootFolderId, String projectName,
		...
		try {
			...
			while ((zipEntry = projectTemplateInputStream.getNextEntry()) != null) {
				...
				if (zipEntry.isDirectory()) {
					...
					log.log(Level.INFO, "Create new artifact from zip entry '" + zipEntry.getName() + "' in folder '" + absolutePath
							+ "' with name '" + name + "'");
