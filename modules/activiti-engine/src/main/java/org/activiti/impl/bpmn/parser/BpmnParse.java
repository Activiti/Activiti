...
public class BpmnParse extends Parse {
  ...
  /**
   * The class to instantiate during parsing where all the runtime data will be stored.
   */
  ...
  /**
   * Map containing the the {@link BpmnInterface}s defined in the XML file.
   * The key is the id of the interface.
   * 
   * Interfaces are defined outside the process definition(s), which means
   * that this map doesn't need to be re-initialized for each new process
   * definition.
   */
  ...
  /**
   * Map containing the {@link Operation}s defined in the XML file.
   * The key is the id of the operations.
   * 
   * Operations are defined outside the process definition(s), which means
   * that this map doesn't need to be re-initialized for each new process
   * definition.
   */
  ...
  /**
   * The current {@link CommandContext} that is used during the {@link DeployCmd} .
  ...
  public BpmnParse execute() {
    ...
    // Item definitions and interfaces/operations need to be 
    // parsed before the processes since the processes refer to them
   ...
   * Item definitions are not contained within a process element, 
   * but they can be referenced from inner process elements.
   * 
   * @param definitionsElement The root element of the XML file.
   */
  ...
  /**
   * Parses all the process definitions defined within
   * the 'definitions' root element.
   * 
   * @param definitionsElement The root element of the XML file.
   */
   ...
   * @param scopeElement The {@link ScopeElementImpl} to which the start events must be added.
   */
   ...
   * @param scopeElement The {@link ScopeElementImpl} to which the activities must be added.
   */
  ...
  /**
   * Generic parsing method for most flow elements: parsing of the documentation sub-element.
   */
   ...
   */
  public ActivityImpl parseAndCreateActivityOnScopeElement(Element activityElement, 
...
//    }
   ...
   * @param scopeElement The {@link ScopeElementImpl} to which the end events must be added.
   */
   ...
   * bpmn activities, since the semantics are different (boundaryEvent needs to be added
   * as nested activity to the reference activity on PVM level).
   * 
   * @param parentElement The 'parent' element that contains the activities (process, subprocess).
   * @param scopeElement The {@link ScopeElementImpl} to which the activities must be added.
   */
  public void parseBoundaryEvents(Element parentElement, ScopeElementImpl scopeElement) {
    for (Element boundaryEventElement : parentElement.elements("boundaryEvent")) {
      ...
      ProcessDefinitionImpl procDef = findContextualObject(ProcessDefinitionImpl.class);
      ActivityImpl parentActivity = procDef.findActivity(attachedToRef);
      if (parentActivity == null) {
        throw new ActivitiException("Invalid reference in boundary timer event: " + attachedToRef);
      }
  ...
  public void parseBoundaryTimerEventDefinition(Element timerEventDefinition, boolean interrupting, ActivityImpl timerActivity) {
    ...
    TimerDeclarationImpl timerDeclaration = timerActivity.getParent()
      .createTimerDeclaration(timeDurationText, TimerExecuteNestedActivityJobHandler.TYPE);
    timerDeclaration.setBusinessCalendarRef(DurationBusinessCalendar.NAME);
  ...
  /** 
   * Parses the properties of an element (if any) that can
   * contain properties (processes, activities, etc.)
   * 
   * Returns true if property subelemens are found.
   * 
   * @param element The element that can contain properties.
   * @param activity The activity where the property declaration is done. 
   */
   ...
   * 
   * @param propertyElement The 'property' element that defines how a 
   *                        property looks like and is handled.
   */
   ...
   * 
   * @param activity The activity where the property declaration is done.
   * @param propertyElement The 'property' element defining the property.
   * @param propertyName The name of the property.
   * @param propertyType The type of the property.
   */
  public void parsePropertyCustomExtensions(ActivityImpl activity, Element propertyElement, 
  ...
  /**
   * Parses all sequence flow of a process.
   * 
   * @param processElement The 'process' element wherein the sequence flow are defined.
   * @param processDefinition The processDefinition to which the sequence flow must be added.
   */
  public void parseSequenceFlow(Element processElement, ProcessDefinitionImpl processDefinition) {
   ...
   * 
   * @param seqFlowElement The 'sequenceFlow' element that can contain a condition.
   * @param seqFlow The sequenceFlow object representation to which the condition must be added.
   */
  ...
  /**
   * Retrieves the {@link Operation} corresponding with the given operation identifier.  
   */
