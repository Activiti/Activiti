...
public interface UserQuery extends Query<UserQuery, User> {
    ...
    /**
     * Only select {@link User}s where the first name matches the given
     * parameter. The syntax is that of SQL, eg. %activivi%.
     */
    ...
    /**
     * Only select {@link User}s where the last name matches the given
     * parameter. The syntax is that of SQL, eg. %activivi%.
     */
    UserQuery userLastNameLike(String lastNameLike);

    /**
     * Only select {@link User}s where the full name matches the given
     * parameters. Both the first name and last name will be tried, ie in
     * semi-sql: where firstName like xxx or lastname like xxx
     */
    ...
    /**
     * Only select {@link User}S that are potential starter for the given
     * process definition.
     */
    ...
    /**
     * Order by user id (needs to be followed by {@link #asc()} or
     * {@link #desc()}).
     */
    UserQuery orderByUserId();

    /**
     * Order by user first name (needs to be followed by {@link #asc()} or
     * {@link #desc()}).
     */
    UserQuery orderByUserFirstName();

    /**
     * Order by user last name (needs to be followed by {@link #asc()} or
     * {@link #desc()}).
     */
    UserQuery orderByUserLastName();

    /**
     * Order by user email (needs to be followed by {@link #asc()} or
     * {@link #desc()}).
     */
