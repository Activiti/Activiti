...
public interface ModelQuery extends Query<ModelQuery, Model> {
    ...
    /**
     * Only select models where the category matches the given parameter. The
     * syntax that should be used is the same as in SQL, eg. %activiti%
     */
    ...
    /**
     * Only select models where the name matches the given parameter. The syntax
     ...
     * 
     * Note: if modelKey(key) is not used in this query, all the models with the
     * highest version for each key will be returned (similar to process
     * definitions)
     */
    ...
    /**
     * Order by the id of the models (needs to be followed by {@link #asc()} or
     * {@link #desc()}).
     */
    ModelQuery orderByModelId();

    /**
     * Order by the key of the models (needs to be followed by {@link #asc()} or
     * {@link #desc()}).
     */
    ...
    /**
     * Order by the name of the models (needs to be followed by {@link #asc()}
     * or {@link #desc()}).
     */
