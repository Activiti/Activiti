...
public class StartEventParseHandler extends AbstractActivityBpmnParseHandler<StartEvent> {
    ...
    // ActivityImpl startEventActivity = createActivityOnCurrentScope(
    // bpmnParse, startEvent, BpmnXMLConstants.ELEMENT_EVENT_START);
    //
    // ScopeImpl scope = bpmnParse.getCurrentScope();
    // if (scope instanceof ProcessDefinitionEntity) {
    // createProcessDefinitionStartEvent(bpmnParse, startEventActivity,
    // startEvent, (ProcessDefinitionEntity) scope);
    // selectInitial(bpmnParse, startEventActivity, startEvent,
    // (ProcessDefinitionEntity) scope);
    // createStartFormHandlers(bpmnParse, startEvent,
    // (ProcessDefinitionEntity) scope);
    // } else {
    ...
    // } else {
    // // validate that there is a single none start event / timer start
    // // event:
    // if (!startEventActivity.getProperty("type").equals(
    // "messageStartEvent")) {
    // String currentInitialType = (String) processDefinition
    // .getInitial().getProperty("type");
    ...
    // startEvent.getFormKey(), bpmnParse.getDeployment(),
    // processDefinition);
    ...
    // EventSubProcessStartEventActivityBehavior activityBehavior = bpmnParse
    // .getActivityBehaviorFactory()
    ...
    // eventDefinition);
    // } else {
    // logger.warn("start event of event subprocess must be of type 'error', 'message' or 'signal' for start event "
    // + startEvent.getId());
    // }
    // }
    //
    ...
    //
    // if (!startEvent.getEventDefinitions().isEmpty()) {
    // logger.warn("event definitions only allowed on start event if subprocess is an event subprocess "
