...
public class EndEventParseHandler extends AbstractActivityBpmnParseHandler<EndEvent> {
    ...
    // protected void executeParse(BpmnParse bpmnParse, EndEvent endEvent) {
    //
    // ActivityImpl endEventActivity = createActivityOnCurrentScope(bpmnParse,
    // endEvent, BpmnXMLConstants.ELEMENT_EVENT_END);
    ...
    // if
    // (bpmnParse.getBpmnModel().containsErrorRef(errorDefinition.getErrorCode()))
    // {
    // String errorCode =
    // bpmnParse.getBpmnModel().getErrors().get(errorDefinition.getErrorCode());
    // if (StringUtils.isEmpty(errorCode)) {
    // logger.warn("errorCode is required for an error event " +
    // endEvent.getId());
    // }
    // endEventActivity.setProperty("type", "errorEndEvent");
    // errorDefinition.setErrorCode(errorCode);
    // }
    // endEventActivity.setActivityBehavior(bpmnParse.getActivityBehaviorFactory().createErrorEndEventActivityBehavior(endEvent,
    // errorDefinition));
    //
    // // Cancel end event
    ...
    // logger.warn("end event with cancelEventDefinition only supported inside transaction subprocess (id="
    // + endEvent.getId() + ")");
    // } else {
