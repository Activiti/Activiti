...
public abstract class AbstractBpmnParseHandler<T extends BaseElement> implements BpmnParseHandler {
    ...
    protected void createAssociation(BpmnParse bpmnParse, Association association) {
        ...
        /*
         * } else { if
         * (sourceActivity.getProperty("type").equals("compensationBoundaryCatch"
         * )) { Object isForCompensation =
         * targetActivity.getProperty(PROPERTYNAME_IS_FOR_COMPENSATION); if
         * (isForCompensation == null || !(Boolean) isForCompensation) {
         * logger.warn(
         * "compensation boundary catch must be connected to element with isForCompensation=true"
         * ); } else { ActivityImpl compensatedActivity =
         * sourceActivity.getParentActivity();
         * compensatedActivity.setProperty(BpmnParse
         * .PROPERTYNAME_COMPENSATION_HANDLER_ID, targetActivity.getId()); } } }
         */
    }
