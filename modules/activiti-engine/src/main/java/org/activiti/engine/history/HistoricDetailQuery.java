...
public interface HistoricDetailQuery extends Query<HistoricDetailQuery, HistoricDetail> {
    ...
    /**
     * Only select historic variable updates with the given execution. Note that
     * {@link Execution} ids are not stored in the history as first class
     * citizen, only process instances are.
     */
    HistoricDetailQuery executionId(String executionId);

    /**
     * Only select historic variable updates associated to the given
     * {@link HistoricActivityInstance activity instance}.
     */
    HistoricDetailQuery activityInstanceId(String activityInstanceId);

    /**
     * Only select historic variable updates associated to the given
     * {@link HistoricTaskInstance historic task instance}.
     */
    ...
    /**
     * Exclude all task-related {@link HistoricDetail}s, so only items which
     * have no task-id set will be selected. When used togheter with
     * {@link #taskId(String)}, this call is ignored task details are NOT
     * excluded.
     */
