...
public abstract class AbstractSetProcessDefinitionStateCmd implements Command<Void> {
    ...
    /**
     * Subclasses should return the type of the {@link JobHandler} here. it will
     * be used when the user provides an execution date on which the actual
     * state change will happen.
     */
    protected abstract String getDelayedExecutionJobHandlerType();

    /**
     * Subclasses should return a {@link Command} implementation that matches
     * the process definition state change.
     */
