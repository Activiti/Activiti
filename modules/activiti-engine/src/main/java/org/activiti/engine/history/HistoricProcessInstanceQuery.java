...
public interface HistoricProcessInstanceQuery extends Query<HistoricProcessInstanceQuery, HistoricProcessInstance> {
  ...
  /**
   * Only select historic process instances that were started on the provided
   * date.
   */
   ...
   * provided date. (Date will be adjusted to reflect one second before
   * midnight)
   */
  ...
  /**
   * Order by the process instance id (needs to be followed by {@link #asc()} or
   * {@link #desc()}).
   */
  HistoricProcessInstanceQuery orderByProcessInstanceId();

  /**
   * Order by the process definition id (needs to be followed by {@link #asc()}
   * or {@link #desc()}).
   */
