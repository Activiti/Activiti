...
//	    activity.setMapExceptions(serviceTask.getMapExceptions());
		...
		if (StringUtils.isNotEmpty(serviceTask.getType())) {
			...
			if (serviceTask.getType().equalsIgnoreCase("mail")) {
				...
				logger.warn("Invalid service task type: '" + serviceTask.getType() + "' " 
						+ " for service task " + serviceTask.getId());
			}
		...
		// Webservice
			...
			if (!bpmnParse.getOperations().containsKey(serviceTask.getOperationRef())) {
				...
				WebServiceActivityBehavior webServiceActivityBehavior = bpmnParse
				        .getActivityBehaviorFactory().createWebServiceActivityBehavior(serviceTask);
