...
/**
 * Interface containing shared methods between the {@link TaskQuery} and the
 * {@link HistoricTaskInstanceQuery}.
 * 
 * @author Joram Barrez
 */
public interface TaskInfoQuery<T extends TaskInfoQuery<?, ?>, V extends TaskInfo> extends Query<T, V> {
     ...
     * Only select tasks with a name that is in the given list
     * 
     * This method, unlike the {@link #taskNameIn(List)} method will not take in
     * account the upper/lower case: both the input parameters as the column
    ...
    /**
     * Only select tasks with a name matching the parameter. The syntax is that
     * of SQL: for example usage: nameLike(%activiti%)
     */
     ...
     * of SQL: for example usage: nameLike(%activiti%)
     * 
     * This method, unlike the {@link #taskNameLike(String)} method will not
     * take in account the upper/lower case: both the input parameter as the
     * column value are lowercased when the query is executed.
     */
    ...
    /**
     * Only select tasks with a description matching the parameter . The syntax
     * is that of SQL: for example usage: descriptionLike(%activiti%)
     */
     ...
     * is that of SQL: for example usage: descriptionLike(%activiti%)
     * 
     * This method, unlike the {@link #taskDescriptionLike(String)} method will
     * not take in account the upper/lower case: both the input parameter as the
     * column value are lowercased when the query is executed.
     */
    ...
    /**
     * Only select tasks which were last assigned to an assignee like the given
     * value. The syntax that should be used is the same as in SQL, eg.
     * %activiti%.
     */
     ...
     * %activiti%.
     * 
     * This method, unlike the {@link #taskAssigneeLike(String)} method will not
     * take in account the upper/lower case: both the input parameter as the
     * column value are lowercased when the query is executed.
     */
    ...
    /**
     * Only select tasks which were last assigned to an owner like the given
     * value. The syntax that should be used is the same as in SQL, eg.
     * %activiti%.
     */
     ...
     * %activiti%.
     * 
     * This method, unlike the {@link #taskOwnerLike(String)} method will not
     * take in account the upper/lower case: both the input parameter as the
     * column value are lowercased when the query is executed.
     */
    ...
    /**
     * Only select tasks for which there exist an {@link IdentityLink} with the
     * given user, including tasks which have been assigned to the given user
     * (assignee) or owned by the given user (owner).
     */
    ...
    /**
     * Only select tasks for which the 'candidateGroup' is one of the given
     * groups.
     * 
     * @throws ActivitiIllegalArgumentException
     *             When query is executed and
     *             {@link #taskCandidateGroup(String)} or
     *             {@link #taskCandidateUser(String)} has been executed on the
     *             query instance. When passed group list is empty or
     *             <code>null</code>.
     */
    ...
    /**
     * Only select tasks with a business key like the given value The syntax is
     * that of SQL: for example usage:
     * processInstanceBusinessKeyLike("%activiti%").
     */
     ...
     * processInstanceBusinessKeyLike("%activiti%").
     * 
     * This method, unlike the {@link #processInstanceBusinessKeyLike(String)}
     * method will not take in account the upper/lower case: both the input
     * parameter as the column value are lowercased when the query is executed.
     */
    ...
    /**
     * Only select tasks with the given taskDefinitionKey. The task definition
     * key is the id of the userTask: &lt;userTask id="xxx" .../&gt;
     **/
    T taskDefinitionKey(String key);

    /**
     * Only select tasks with a taskDefinitionKey that match the given
     * parameter. The syntax is that of SQL: for example usage:
     * taskDefinitionKeyLike("%activiti%"). The task definition key is the id of
    ...
    /**
     * Only select tasks which are part of a process instance which has the
     ...
     * process definition key like the given value. The syntax that should be
     * used is the same as in SQL, eg. %activiti%.
     */
     ...
     * used is the same as in SQL, eg. %activiti%.
     * 
     * This method, unlike the {@link #processDefinitionKeyLike(String)} method
     * will not take in account the upper/lower case: both the input parameter
     * as the column value are lowercased when the query is executed.
     */
    ...
    /**
     * Only select tasks which are part of a process instance which has the
     * given process definition name.
     */
    T processDefinitionName(String processDefinitionName);

    /**
     * Only select tasks which are part of a process instance which has a
     * process definition name like the given value. The syntax that should be
     * used is the same as in SQL, eg. %activiti%.
     */
     ...
     * 
     * @throws ActivitiIllegalArgumentException
     *             When passed category list is empty or <code>null</code> or
     *             contains <code>null String</code>.
    ...
    /**
     * Only select tasks which are part of a process instance whose definition
     * does not belong to the category which is present in the given list.
     * 
     * @throws ActivitiIllegalArgumentException
     *             When passed category list is empty or <code>null</code> or
     *             contains <code>null String</code>.
    ...
    /**
     * Only select tasks which have at least one local task variable with the
     * given value.
     */
    T taskVariableValueEquals(Object variableValue);

    /**
     * Only select tasks which have a local string variable with the given
     * value, case insensitive.
     * <p>
     * This method only works if your database has encoding/collation that
     * supports case-sensitive queries. For example, use "collate UTF-8" on
     * MySQL and for MSSQL, select one of the case-sensitive Collations
     * available (<a
     * href="http://msdn.microsoft.com/en-us/library/ms144250(v=sql.105).aspx"
    ...
    /**
     * Only select tasks which have a local task variable with the given name,
     * but with a different value than the passed value. Byte-arrays and
    ...
    /**
     * Only select tasks which have a local string variable with is not the
    ...
    /**
     * Only select tasks which have a local variable value greater than or equal
     * to the passed value when they ended. Booleans, Byte-arrays and
     * {@link Serializable} objects (which are not primitive type wrappers) are
     * not supported.
     * 
     * @param name
    ...
    /**
     * Only select tasks which have a local variable value less than the passed
     * value when the ended.Booleans, Byte-arrays and {@link Serializable}
    ...
    /**
     * Only select tasks which have a local variable value less than or equal to
     * the passed value when they ended. Booleans, Byte-arrays and
     * {@link Serializable} objects (which are not primitive type wrappers) are
     * not supported.
     * 
     * @param name
     ...
     *            cannot be null.
     * @param value
     *            cannot be null. The string can include the wildcard character
     *            '%' to express like-strategy: starts with (string%), ends with
     *            (%string) or contains (%string%).
     */
    T taskVariableValueLike(String name, String value);

    /**
     * Only select tasks which are part of a process that has a variable with
     * the given name set to the given value.
     */
    T processVariableValueEquals(String variableName, Object variableValue);

    /**
     * Only select tasks which are part of a process that has at least one
     * variable with the given value.
     */
    T processVariableValueEquals(Object variableValue);

    /**
     * Only select tasks which are part of a process that has a local string
     * variable which is not the given value, case insensitive.
     * <p>
     * This method only works if your database has encoding/collation that
     * supports case-sensitive queries. For example, use "collate UTF-8" on
     * MySQL and for MSSQL, select one of the case-sensitive Collations
    ...
    /**
     * Only select tasks which have a variable with the given name, but with a
     * different value than the passed value. Byte-arrays and
     * {@link Serializable} objects (which are not primitive type wrappers) are
     * not supported.
     */
    ...
    /**
     * Only select tasks which are part of a process that has a string variable
     * with the given value, case insensitive.
     * <p>
     * This method only works if your database has encoding/collation that
     * supports case-sensitive queries. For example, use "collate UTF-8" on
     * MySQL and for MSSQL, select one of the case-sensitive Collations
    ...
    /**
     * Only select tasks which have a global variable value less than the passed
     * value when the ended.Booleans, Byte-arrays and {@link Serializable}
     * objects (which are not primitive type wrappers) are not supported.
     * 
     * @param name
    ...
    /**
     * All query clauses called will be added to a single or-statement. This
     * or-statement will be included with the other already existing clauses in
     * the query, joined by an 'and'.
     * 
     * Calling endOr() will add all clauses to the regular query again. Calling
     * or() after endOr() has been called will result in an exception.
     */
    T or();
    ...
    /**
     * Order by task id (needs to be followed by {@link #asc()} or
     * {@link #desc()}).
     */
    T orderByTaskId();

    /**
     * Order by task name (needs to be followed by {@link #asc()} or
     * {@link #desc()}).
     */
    T orderByTaskName();

    /**
     * Order by description (needs to be followed by {@link #asc()} or
     * {@link #desc()}).
     */
    T orderByTaskDescription();

    /**
     * Order by priority (needs to be followed by {@link #asc()} or
     * {@link #desc()}).
     */
    T orderByTaskPriority();

    /**
     * Order by assignee (needs to be followed by {@link #asc()} or
     * {@link #desc()}).
     */
    T orderByTaskAssignee();

    /**
     * Order by the time on which the tasks were created (needs to be followed
    ...
    /**
     * Order by process definition id (needs to be followed by {@link #asc()} or
     * {@link #desc()}).
     */
    T orderByProcessDefinitionId();

    /**
     * Order by task due date (needs to be followed by {@link #asc()} or
     * {@link #desc()}).
     */
    T orderByTaskDueDate();

    /**
     * Order by task owner (needs to be followed by {@link #asc()} or
     * {@link #desc()}).
     */
    T orderByTaskOwner();

    /**
     * Order by task definition key (needs to be followed by {@link #asc()} or
     * {@link #desc()}).
     */
    ...
    /**
     * Order by due date (needs to be followed by {@link #asc()} or
     * {@link #desc()}). If any of the tasks have null for the due date, these
     * will be first in the result.
     */
