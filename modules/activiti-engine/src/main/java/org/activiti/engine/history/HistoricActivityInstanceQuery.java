...
public interface HistoricActivityInstanceQuery extends Query<HistoricActivityInstanceQuery, HistoricActivityInstance> {
    ...
    /**
     * Only select historic activity instances with the given id (primary key
     * within history tables).
     */
    ...
    /**
     * Only select historic activity instances for the given activity (id from
     * BPMN 2.0 XML)
     */
    HistoricActivityInstanceQuery activityId(String activityId);

    /**
     * Only select historic activity instances for activities with the given
     * name
     */
    ...
    /**
     * Only select historic activity instances for userTask activities assigned
     * to the given user
     */
    ...
    /**
     * Only select historic activity instances with a tenant id like the given
     * one.
     */
    ...
    /**
     * Order by processInstanceId (needs to be followed by {@link #asc()} or
     * {@link #desc()}).
     */
    HistoricActivityInstanceQuery orderByProcessInstanceId();

    /**
     * Order by executionId (needs to be followed by {@link #asc()} or
     * {@link #desc()}).
     */
    HistoricActivityInstanceQuery orderByExecutionId();

    /**
     * Order by activityId (needs to be followed by {@link #asc()} or
     * {@link #desc()}).
     */
    HistoricActivityInstanceQuery orderByActivityId();

    /**
     * Order by activityName (needs to be followed by {@link #asc()} or
     * {@link #desc()}).
     */
    HistoricActivityInstanceQuery orderByActivityName();

    /**
     * Order by activityType (needs to be followed by {@link #asc()} or
     * {@link #desc()}).
     */
    ...
    /**
     * Order by duration (needs to be followed by {@link #asc()} or
     * {@link #desc()}).
     */
    HistoricActivityInstanceQuery orderByHistoricActivityInstanceDuration();

    /**
     * Order by processDefinitionId (needs to be followed by {@link #asc()} or
     * {@link #desc()}).
     */
