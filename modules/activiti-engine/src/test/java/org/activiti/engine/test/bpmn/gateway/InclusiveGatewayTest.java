...
public class InclusiveGatewayTest extends PluggableActivitiTestCase {
    ...
    /**
     * Test for ACT-1216: When merging a concurrent execution the parent is not
     * activated correctly
     */
    @Deployment
    ...
    /**
     * Test for bug ACT-10: whitespaces/newlines in expressions lead to
     * exceptions
     */
    @Deployment
    ...
    /**
     * This test the isReachable() check thaty is done to check if upstream
     * tokens can reach the inclusive gateway.
     * 
     * In case of loops, special care needs to be taken in the algorithm, or
     * else stackoverflows will happen very quickly.
     */
    @Deployment
    ...
    /*
     * @Deployment public void testAsyncBehavior() { for (int i = 0; i < 100;
     * i++) { ProcessInstance processInstance =
     * runtimeService.startProcessInstanceByKey("async"); } assertEquals(200,
     * managementService.createJobQuery().count());
     * waitForJobExecutorToProcessAllJobs(120000, 5000); assertEquals(0,
     * managementService.createJobQuery().count()); assertEquals(0,
