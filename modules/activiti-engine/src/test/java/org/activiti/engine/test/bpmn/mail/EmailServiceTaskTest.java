...
public class EmailServiceTaskTest extends EmailTestCase {
  ...
  public void testSimpleTextMailWhenMultiTenant() throws Exception {
    ...
    WiserMessage message = messages.get(0);
    assertEmailSend(message, false, "Hello Kermit!", "This a text only e-mail.", "activiti@myTenant.com", Collections.singletonList("kermit@activiti.org"), null);
    assertProcessEnded(procId);
  ...
  public void testTextMailExpressions() throws Exception {
    ...
    WiserMessage message = messages.get(0);
    assertEmailSend(message, false, subject, "Hello " + recipientName + ", this is an e-mail", sender, Collections.singletonList(recipient), null);
  }
  ...
  public void testCcAndBcc() throws Exception {
    ...
    List<WiserMessage> messages = wiser.getMessages();
    assertEmailSend(messages.get(0), false, "Hello world", "This is the content", "activiti@localhost", Collections.singletonList("kermit@activiti.org"), Collections.singletonList("fozzie@activiti.org"));
