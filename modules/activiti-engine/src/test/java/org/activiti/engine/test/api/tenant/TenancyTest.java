...
/**
 * A test case for the various implications of the tenancy support (tenant id
 * column to entities + query support)
 * 
 * @author jbarrez
 */
public class TenancyTest extends PluggableActivitiTestCase {
    ...
    public void testProcessDefinitionTenancy() {
        ...
        assertEquals(processDefinitionIdWithTenant2, repositoryService.createProcessDefinitionQuery().processDefinitionKey("oneTaskProcess").processDefinitionTenantId(TEST_TENANT_ID).latestVersion()
                .singleResult().getId());
        ...
        assertEquals(processDefinitionIdWithoutTenant, repositoryService.createProcessDefinitionQuery().processDefinitionKey("oneTaskProcess").processDefinitionWithoutTenantId().latestVersion()
                .singleResult().getId());
    }
    ...
    public void testProcessDefinitionKeyClashBetweenTenants() {
        ...
        try {
            ...
            fail(); // shouldnt happen, there is no process definition with that
    ...
    public void testSignalFromProcessTenancy() {
        ...
        // tenant context), it should obly continue those without tenant
    ...
    public void testStartProcessInstanceBySignalTenancy() {
        ...
        repositoryService.createDeployment().addClasspathResource("org/activiti/engine/test/api/tenant/TenancyTest.testStartProcessInstanceBySignalTenancy.bpmn20.xml").tenantId(TEST_TENANT_ID)
                .deploy();
