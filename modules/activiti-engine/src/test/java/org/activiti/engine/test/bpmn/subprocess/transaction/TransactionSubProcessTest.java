...
public class TransactionSubProcessTest extends PluggableActivitiTestCase {
    ...
    public void testSimpleCaseTxSuccessful() {
        ...
        // now the process instance execution is sitting in the 'afterSuccess'
        // task
        ...
        // there is a compensate event subscription for the transaction under
        ...
        // there is an event-scope execution associated with the
        // event-subscription:
        ...
        // we still have compensate event subscriptions for the compensation
        // handlers, only now they are part of the event scope
    ...
    public void testNestedCancelInner() {
        ...
        // end the process instance (signal the execution still sitting in
        // afterInnerCancellation)
    ...
    /*
     * The cancel end event cancels all instances, compensation is performed for
     * all instances
     * 
     * see spec page 470: "If the cancelActivity attribute is set, the Activity
     * the Event is attached to is then cancelled (in case of a multi-instance,
     * all its instances are cancelled);"
     */
    @Deployment
    public void testMultiInstanceTx() {
        ...
        // the event subscriptions are all under the same execution (the
        // execution of the multi-instance wrapper)
    ...
    public void testMultipleCancelBoundaryFails() {
        try {
            repositoryService.createDeployment().addClasspathResource("org/activiti/engine/test/bpmn/subprocess/transaction/TransactionSubProcessTest.testMultipleCancelBoundaryFails.bpmn20.xml")
                    .deploy();
    ...
    public void testCancelEndNoTransactionFails() {
        try {
            repositoryService.createDeployment().addClasspathResource("org/activiti/engine/test/bpmn/subprocess/transaction/TransactionSubProcessTest.testCancelEndNoTransactionFails.bpmn20.xml")
                    .deploy();
            fail("exception expected");
        } catch (Exception e) {
    ...
    public void testParseWithDI() {
        ...
        // this test simply makes sure we can parse a transaction subprocess
        // with DI information
