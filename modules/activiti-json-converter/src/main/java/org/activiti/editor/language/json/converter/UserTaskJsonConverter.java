...
public class UserTaskJsonConverter extends BaseBpmnJsonConverter {
    ...
    protected void convertElementToJson(ObjectNode propertiesNode, BaseElement baseElement) {
        ...
        if (StringUtils.isNotEmpty(userTask.getFormKey())) {
            List<ExtensionElement> formIdExtensions = userTask.getExtensionElements().get("form-reference-id");
            List<ExtensionElement> formNameExtensions = userTask.getExtensionElements().get("form-reference-name");
    ...
    protected FlowElement convertJsonToElement(JsonNode elementNode, JsonNode modelNode, Map<String, JsonNode> shapeMap) {
        ...
        if (StringUtils.isNotEmpty(formKey)) {
            ...
            if (formReferenceNode != null && formReferenceNode.get("id") != null && formReferenceNode.get("name") != null) {
                task.setFormKey(formReferenceNode.get("id").asText());
                addExtensionElement("form-reference-id", formReferenceNode.get("id").asText(), task);
                addExtensionElement("form-reference-name", formReferenceNode.get("name").asText(), task);
            }
        }
            ...
            if (assignmentDefNode != null) {
                ...
                if (typeNode == null || "static".equalsIgnoreCase(typeNode.asText())) {
                    ...
                    if (idmDefNode != null && idmDefNode.has("type")) {
                        ...
                        } else if (idmTypeNode != null && "users".equalsIgnoreCase(idmTypeNode.asText()) && 
