...
public class DefaultProcessDiagramGenerator implements ProcessDiagramGenerator {
      ...
      public void draw(DefaultProcessDiagramCanvas processDiagramCanvas, BpmnModel bpmnModel, FlowNode flowNode) {
        ...
        if (startEvent.getEventDefinitions() != null && !startEvent.getEventDefinitions().isEmpty()) {
          ...
          if (eventDefinition instanceof TimerEventDefinition) {
            processDiagramCanvas.drawTimerStartEvent(flowNode.getName(), graphicInfo, scaleFactor);
          } else if (eventDefinition instanceof ErrorEventDefinition) {
            processDiagramCanvas.drawErrorStartEvent(flowNode.getName(), graphicInfo, scaleFactor);
          } else if (eventDefinition instanceof SignalEventDefinition) {
            processDiagramCanvas.drawSignalStartEvent(flowNode.getName(), graphicInfo, scaleFactor);
          } else if (eventDefinition instanceof MessageEventDefinition) {
            processDiagramCanvas.drawMessageStartEvent(flowNode.getName(), graphicInfo, scaleFactor);
          } else {
            processDiagramCanvas.drawNoneStartEvent(flowNode.getName(), graphicInfo);
          }
        } else {
          processDiagramCanvas.drawNoneStartEvent(flowNode.getName(), graphicInfo);
        }
  ...
  protected static DefaultProcessDiagramCanvas initProcessDiagramCanvas(BpmnModel bpmnModel, String imageType,
    ...
    return new DefaultProcessDiagramCanvas((int) maxX + 30,(int) maxY + 30, (int) minX, (int) minY, 
