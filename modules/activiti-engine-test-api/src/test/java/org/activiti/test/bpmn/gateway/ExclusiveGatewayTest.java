...
public class ExclusiveGatewayTest extends ActivitiTestCase {
  ...
  public void testDivergingExclusiveGateway() {
    ...
    for (int i =1 ; i<=3; i++) {
      ProcessInstance pi = processService.startProcessInstanceByKey("exclusiveGwDiverging", singletonMap("input", i));
  ...
  public void testMultipleValidConditions() {
    deployProcessForThisTestMethod();
    processService.startProcessInstanceByKey("exclusiveGwMultipleValidConditions", singletonMap("input", 5));
  ...
  public void testNoSequenceFlowSelected() {
    ...
    try {
      processService.startProcessInstanceByKey("exclusiveGwNoSeqFlowSelected", singletonMap("input", 4));
      fail();
